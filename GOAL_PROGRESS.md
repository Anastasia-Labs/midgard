...
| F02-R | F02 audit | parent | `docs/exec-plans/evidence/canonical-v1-format-registry-v1.json`, `demo/scripts/verify-canonical-v1-format-registry.mjs` | IN_PROGRESS | pending | Registry contains all 132 prose IDs in canonical order and unknowns fail closed. Structural mode passes; default release mode rejects 124 open rows. C01/C02/C04/C05/C10/D19/D20/A22 are source-verified PASS; every remaining row must still supply exact source/symbol, wire field/tag/arity, parser/encoder, positive/rejection, cross-language/N/A, and obsolete-branch evidence. |
