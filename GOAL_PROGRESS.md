...
| F02-R | F02 audit | parent | `docs/exec-plans/evidence/canonical-v1-format-registry-v1.json`, `demo/scripts/verify-canonical-v1-format-registry.mjs` | IN_PROGRESS | `47a93b1a` plus worktree | Registry contains all 132 prose IDs in canonical order and unknowns fail closed. Structural mode passes; default release mode rejects 122 open rows. C01/C02/C03/C04/C05/C09/C10/D19/D20/A22 are source-verified PASS; every remaining row must still supply exact source/symbol, wire field/tag/arity, parser/encoder, positive/rejection, cross-language/N/A, and obsolete-branch evidence. |
...
| F02-C08 | F02 audit | parent | registry row C08; implementation lease not yet assigned | TODO | n/a | Fail-closed source audit: TS and Aiken both define genesis sentinel `0`, but neither production path consumes it; Aiken state-queue initialization writes ordinary protocol version `1`. C08 cannot pass until genesis is an authenticated distinct variant/sentinel without making the first ordinary V1 header inherit `0`. |
