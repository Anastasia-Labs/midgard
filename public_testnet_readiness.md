...
- [ ] Complete the public fraud-proof/proof-data-availability milestone.
  ...
  - Prerequisite (recorded per [#606](https://github.com/Anastasia-Labs/midgard/issues/606)'s acceptance, owner rulings 2026-08-15/16): the §8.3 erratum E2 limit-3 repair (constant certificate asset name + mint-welded `field_hash` + door equality) and the pexcludes empty-trie-sentinel translation are pre-launch obligations. **Delivered in-tree by the #606 batch**; any deployment MUST bind against the post-#606 regenerated identity (testnet blueprint md5 `5e38d7c6ccb7987d0aca710307dcaea7`, 398 validators) — a deployment of the pre-#606 identity re-opens the E2 dispute-lane operator-escape window and the first-block non-membership liveness gap. Scope caveat on the second obligation: #606's rider authorised the sentinel translation at exactly one placement, `validators/pexcludes.ak`, so it closes the gap on the redeemer-carried (direct) route only. Through published-chunk carriage the gap remains open — `lib/midgard/mpf-proof-v1.ak`'s `does_not_have` still compares against the untranslated root, so a first block over a genesis confirmed state cannot be proved by that route (liveness-only, nothing false accepted; recorded in `chunked-inclusion-v1.ak`'s module header and `mpf-proof-v1.ak`'s `does_not_have` doc-comment; repair is owner-approved, 2026-08-17, to land inside the one remaining blueprint regeneration event, [#510](https://github.com/Anastasia-Labs/midgard/issues/510), together with the matching `insert_root` fold-in).
