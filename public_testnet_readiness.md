...
- [ ] Public readiness blockers remain: complete and correctly bound fraud proofs, permissionless DA/retrieval guarantees, production timing/economics, L1 finality and rollback policy, ingress/ops hardening, clean public acceptance, and operator-halt/fund-recovery behavior.
...
- [ ] Complete the public fraud-proof/proof-data-availability milestone.
  ...
  - Prerequisite (recorded per [#606](https://github.com/Anastasia-Labs/midgard/issues/606)'s acceptance, owner rulings 2026-08-15/16): the §8.3 erratum E2 limit-3 repair (constant certificate asset name + mint-welded `field_hash` + door equality) and the pexcludes empty-trie-sentinel translation are pre-launch obligations. **Delivered in-tree by the #606 batch**; any deployment MUST bind against the post-#606 regenerated identity (testnet blueprint md5 `5e38d7c6ccb7987d0aca710307dcaea7`, 398 validators) — a deployment of the pre-#606 identity re-opens the E2 dispute-lane operator-escape window and the first-block non-membership liveness gap. The second obligation's route gap is now closed as well: #606's rider authorised the sentinel translation at exactly one placement, `validators/pexcludes.ak`, which closed the redeemer-carried (direct) route only, leaving published-chunk carriage unable to prove a first block over a genesis confirmed state (liveness-only, nothing false accepted). That residual is repaired — `lib/midgard/mpf-proof-v1.ak`'s `does_not_have` translates the sentinel before comparing, with the matching `insert_root` fold-in, so the direct, transition-trace and published-chunk routes now answer the same question. Owner-approved 2026-08-17 and ruled into the single blueprint regeneration event as R5 item 6 of [decision 0005](docs/midgard/decisions/0005-owner-rulings-2026-08-18.md) by the 2026-08-23 owner ruling on [#625](https://github.com/Anastasia-Labs/midgard/issues/625); it is delivered inside the [#617](https://github.com/Anastasia-Labs/midgard/issues/617) regeneration wave, so the binding identity a deployment must use is the wave's re-pinned one, not the post-#606 identity cited above.
...
- [x] Align package-level CI with the public gate's relevant code surfaces.
  ...
  - Evidence: `.github/workflows/aiken-ci.yml` and `.github/workflows/midgard-node-ci.yml` use the patched Aiken fork `v1.1.23+2a78108` (`Anastasia-Labs/aiken`, tag `midgard-2a78108c`), matching the `v1.1.23` release `onchain/aiken/aiken.toml` declares; node CI builds/tests core, validation, SDK, fault-proof tooling, DA committee, Lucid, node, native MPF, and offline throughput gates.
...
| `.github/workflows/aiken-ci.yml` and `.github/workflows/midgard-node-ci.yml` use the patched Aiken fork `v1.1.23+2a78108` (`Anastasia-Labs/aiken`, tag `midgard-2a78108c`), matching the `v1.1.23` release `onchain/aiken/aiken.toml` declares, and both pin the fork by rev; but the workflows still use mutable runner/action references for everything else.                                                                                                                                                                                                                                                           | Compiler-version alignment is fixed, but release CI is not yet supply-chain pinned.                                                                                                         |
...
| `onchain/aiken/env/testnet.ak` has zero bond/slash/reward values and defines no outbox identifiers at all; its delegated Plutarch hashes are fully pinned (PHAS `1fc59ff5…`, pexcludes `03adaadf…`, both matching the deployed Aiken-native scripts).                                                                                                                                                                                                                       | Public-testnet economic parameters still need explicit, reviewed values.                                                                                                                    |
...
- [ ] Pin and verify the exact Aiken compiler version in CI, local docs, and build scripts.
  - Evidence: primary CI workflows now use the patched Aiken fork `v1.1.23+2a78108` (`Anastasia-Labs/aiken`, tag `midgard-2a78108c`), matching the `v1.1.23` release `onchain/aiken/aiken.toml` declares; local install docs and reproducible artifact verification still need enforcement.
...
- [x] Align Aiken compiler versions for the primary contract project, CI workflows, and real checked-in blueprint.
  - Evidence: `onchain/aiken/aiken.toml`, `.github/workflows/aiken-ci.yml`, `.github/workflows/midgard-node-ci.yml`, and the generated `onchain/aiken/plutus.json` reflect the patched Aiken fork `v1.1.23+2a78108` (`Anastasia-Labs/aiken`, tag `midgard-2a78108c`), whose `v1.1.23` release matches the `aiken.toml` declaration.
