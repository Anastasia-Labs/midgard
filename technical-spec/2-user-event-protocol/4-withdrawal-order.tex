  ...
  \item[Authenticate Withdrawal.] Properly record a new withdrawal order event from L2 to L1.
    Conditions:
      \begin{enumerate}
        ...
        \item The redeemer used for registering the witness script must be \code{MintOrBurn} carrying the \code{withdrawal} policy ID.
      ...
      \end{enumerate}
    \item[Burn Withdrawal NFT.] Require the event NFT burn and witness deregistration when the order either initializes payout or follows the invalid-order refund path.
    Conditions:
      \begin{enumerate}
        ...
        \item The redeemer used for unregistrering the witness script must be correct. Namely, it must be \code{MintOrBurn} carrying the \code{withdrawal} policy ID.
      \end{enumerate}
\end{description}
...
\begin{description}
  \item[Initialize Payout.] Transaction for reproducing the order at the payout contract to be filled up from the Midgard reserve.
    ...
    \begin{enumerate}
      ...
      \item After removing the payout NFT, the initial accumulator must not exceed the target quantity of any asset or introduce an extra asset.
  ...
  \item[Refund.] 
    ...
    \begin{enumerate}
      ...
      \item The \code{BurnEventNFT} endpoint of the withdrawal minting script must be invoked with the corresponding \code{l1\_id} of the withdrawal utxo.
      \item No reference script must be attached to the refund output.
    \end{enumerate}
\end{description}
