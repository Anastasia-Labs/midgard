...
\begin{enumerate}
    ...
    \item Total count of tokens in the deposit must not exceed \code{max\_tokens\_allowed\_in\_deposits}, a Midgard protocol parameter.
  ...
  \item[Authenticate Deposit.] Properly record a new deposit event from L1 to L2.
    Conditions:
      \begin{enumerate}
        ...
        \item The redeemer used for registering the witness script must be \code{MintOrBurn} carrying the \code{deposit} policy ID.
