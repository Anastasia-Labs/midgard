...
\paragraph{Launch consensus profile.}
The transaction structures below describe the codec's representational
superset. The immutable launch profile is narrower: protocol version (0),
native transaction version (1), public-key payment credentials only, and no
mint or burn, reference inputs, scripts, redeemers, observers, reference
scripts, or protected outputs. Unknown versions and every representable field
outside that profile fail closed. The canonical full transaction is at most
(8{,}192) bytes and each encoded output value is at most (1{,}024) bytes;
the reference-script limit is zero. These limits are consensus admission and
DA-attestation rules, not operator-tunable policy.

The codec retains future transaction fields, but the launch profile does not
admit script-controlled spends or token minting. Launch transactions spend
public-key outputs using address witnesses and preserve value according to the
no-mint equation in \cref{h:custom-midgard-ledger-rules}.
...
\begin{description}
    ...
    \item[No pre-Conway features.] Midgard does not need to maintain backwards compatibility with pre-Conway eras.
      ...
      The codec can represent public key, native-script, and Plutus credentials,
      but the launch profile admits public-key payment credentials only.
