...
A transaction set in Midgard is a finite map from transaction ID to accepted L2 transaction request.
The ID is the Blake2b-256 hash of the transaction body; equivalently, the map key must match the transaction's \code{body} field.
The witness-set hash and validity tag are authenticated by the block's \code{transactions\_root} through the map value, but they are not part of the transaction ID:
\begin{align*}
    \T{TxSet} &\coloneq \T{Map(TxId, MidgardTxCompact)} \\
      &\coloneq \Bigl\{
        (k_i: \T{TxId}, v_i: \T{MidgardTxCompact}) \mid
        k_i \equiv v_i.\T{body} ,\;
        \forall i \neq j.\; k_i \neq k_j
    \Bigr\}
\end{align*}
