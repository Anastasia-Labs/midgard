...
\T{Cbor(source)})\), matching the production V1 encoder.  The committed source
value does not carry a copy of it.  Its only consumer, the validation claim, binds
the initial machine state's commitment to the value it derives from
\T{NativeTxProofSourceV1} at the point of use, so the committed leaf holds no copy
to keep in agreement with its own derivation.  This is a statement about the
committed leaf alone: the L1 transaction-order payload and its field receipts still
carry the commitment explicitly, because there it names the material a receipt is
filed under while that material is still being published piecewise, and the order
event checks the carried value against the same derivation once the source is
complete.
