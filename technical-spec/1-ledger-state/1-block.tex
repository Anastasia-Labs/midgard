...
\begin{equation*}
    ...
    \begin{array}{ll}
        ...
        \T{forced\_transactions} : & \T{ForcedTransactionSet} \\
...
The source roots, \code{transition\_trace\_root}, and \code{event\_to\_step\_root} are counted, domain-separated commitments: fraud proofs can open membership, non-membership, and the committed member count for the relevant root.

\Cref{fig:block-tx-mpt} shows an example MPT representation of a block's transactions.
Each leaf stores a pair $(\T{TxId_i}, v_i)$ where $v_i : \T{MidgardTxCompact}$.
...
The \code{forced\_transactions\_root}, \code{transition\_trace\_root}, and \code{event\_to\_step\_root} use the same authenticated-map commitment pattern with their own key and value types.
...
\begin{itemize}
    ...
    \item The source roots are \code{withdrawals\_root}, \code{forced\_transactions\_root}, \code{transactions\_root}, and \code{deposits\_root}.
    ...
    \item The count fields commit to the cardinality of each source phase and the total trace length. The corresponding source-root counts must equal their header count fields, \code{event\_to\_step\_root.count} must equal \code{total\_event\_count}, and \code{transition\_trace\_root.count} must equal \code{transition\_step\_count}. They must satisfy:
    \begin{align*}
...
For the launch profile, \code{protocol\_version} must equal (0), and every
...
The launch count bounds are (10{,}000) members for each of withdrawals,
...
is additionally bounded to (32\,MiB) of canonical normal-transaction
bytes, (40{,}000) ledger operations, and (64\,MiB) total canonical
payload bytes. Header count bounds are checked by the L1 state-queue validator;
