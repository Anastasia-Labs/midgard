...
validation-trace descriptors and retained step witnesses, and count fields repeated from the header.
...
The figure is a schematic binary hash tree illustrating authentication, not the Midgard trie wire encoding.
The implementation uses the pinned Merkle Patricia Forestry trie with compressed paths and its prescribed branch hashing.
The header commits to the trie root together with the domain and count through \code{transition\_trace.commit\_counted\_root}; the canonical empty root uses the shared empty-tree sentinel.
...
% Root node
  \node[merkleroot] {Schematic root: $\mathcal{H}(N_{12} \mid\mid N_{34})$}
...
\end{tikzpicture}
\caption[Transaction authentication schematic]{Schematic transaction hash tree; the actual compressed-trie and counted-root encodings are defined by the implementation.}
...
A block header has a fixed field count and bounded scalar and hash fields.
Its canonical serialized length can vary with the integer values.
...
\begin{itemize}
    ...
    \item The \code{block\_slot} field supplies the transaction-validation slot (the node derives it from the block end time), and \code{expected\_network\_id}, \code{min\_fee\_a}, and \code{min\_fee\_b} bind the validation context used for the block.
