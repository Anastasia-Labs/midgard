...
\begin{align*}
    ...
    \T{WithdrawalValidity} \coloneq\;& \T{WithdrawalIsValid} \\
	                               ...
	                               \mid\;& \T{SpentWithdrawalUtxo}\{\T{l2\_tx\_id}\} \\
	                               ...
	                               \mid\;& \T{UnpayableWithdrawalValue}
\end{align*}
\endgroup

The \code{WithdrawalId} of a withdrawal event is the output reference of one of the inputs spent by the user in the L1 transaction that created the withdrawal request.
The authentication NFT's asset name is derived from this reference; the event ID itself is not a hash.
...
Only a withdrawal event tagged \code{WithdrawalIsValid} removes the output at \code{l2\_outref} from the block's UTxO set.
An invalid withdrawal is still included when due, but its transition is an exact no-op and settlement follows the refund path.

A withdrawal event must not be tagged as \code{WithdrawalIsValid} unless \code{l2\_value} can be paid exactly to \code{l1\_address} with \code{l1\_datum} as a valid Cardano L1 transaction output.
In particular, the exact \code{l2\_value} must satisfy the L1 output's minimum-ADA and token-bundle constraints without adding reserve funds or any other top-up.
If exact payment is not possible, the withdrawal must be tagged as \code{UnpayableWithdrawalValue} and follow the invalid-withdrawal refund path.
