...
In that case, its effect is to add a new L2 utxo to the block's utxo set containing the value from the L1 deposit utxo, except the deposit authentication NFT, at the address (\code{l2\_address}) on the committed native Midgard network (\code{l2\_network\_id}) and with the inline datum (\code{l2\_datum}) specified by the user.
...
Deposits are applied after withdrawals, forced transactions, and normal L2 transactions in the block transition trace.
Therefore an L2 transaction in the same block cannot spend the output created by a deposit in that block.
Such a spend is valid only in a later block after the deposit output is part of the previous committed UTxO root, unless the protocol deliberately changes the phase order.
