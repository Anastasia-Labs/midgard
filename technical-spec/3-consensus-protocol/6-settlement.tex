...
The settlement contract tracks deposit events, withdrawal orders, and forced L2 transaction orders until they are processed.
...
The settlement contains the block's deposits root hash, withdrawals root hash, the forced-transactions root hash for orders, and the normal-transactions root hash.
Users must reference a settlement utxo to process its confirmed deposits, withdrawals, or transaction orders.
For deposits, processed means the deposit utxo has been spent into the reserve.
For valid withdrawals, processed means the withdrawal order utxo has been spent into an authenticated payout accumulator; the later reserve-funding and payout-completion lifecycle is tracked by the payout NFT and payout validator.
For invalid withdrawals, processed means the withdrawal order utxo has been refunded according to its refund address and datum.
For transaction orders, processed means the transaction order utxo has been spent through its confirmed settlement path.

The current operator can optimistically resolve any settlement utxo, claiming that its confirmed user events have all been processed.
      ...
      \begin{enumerate}
        ...
        \item \code{merged\_block} and \code{new\_settlement} must match on all of these fields:
          \begin{itemize}
            ...
            \item \code{forced\_transactions\_root}
    ...
    Conditions:
      \begin{enumerate}
        ...
        \item A valid membership proof must be provided, proving that \code{unprocessed\_event} is a member of the corresponding tree in the settlement.
          Note that the withdrawal validity or typed transaction-order verdict must be provided via the redeemer.
