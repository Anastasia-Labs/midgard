...
Whenever a shift ends, or the current shift's operator fails to submit a block commitment within an acceptable inactivity period, the next operator in key-descending order from the \code{active\_operators} list can be picked to take over the next shift.
...
\subsection{State discrepency}
\label{h:scheduler-state-discrepency}

The scheduler is essentially a cursor for the active operators set, which means it has to have a local minimal representation of the set's state.

On the other hand, block commitments require referrals to the scheduler utxo in order to show they are submitted by the currently active operator.
This demands a high level of availability for this utxo, and therefore must be spent as few times as possible.

Considering operators can be added/removed to/from the active operators set during shifts, there can be deviation of states between the scheduler and the set.

The relative states can be summarized as the following.
First states are for the scheduler, while the second states are for the active operators set:
\begin{enumerate}
  \item \textbf{Empty---Empty}: No actions can be taken.
  \item \textbf{Empty---Non-empty}: There is at least one operator available to appoint.
  \item \textbf{Appointed---Empty}: While retirement of active operators is not allowed, this can happen if the appointed operator is slashed during their shift.
  \item \textbf{Appointed---One operator}: It is possible that they point to different operators.
  \item \textbf{Appointed---Two or more operators}: The scheduler can advance to the next operator in queue.
\end{enumerate}
