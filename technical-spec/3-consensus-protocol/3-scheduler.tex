...
That is, it should only be spent for actions that change the appointed operator.
Namely:
\begin{enumerate}
  \item \textbf{Advancing.} Picking the next operator in set, or updating the shift start time of the current one if no other operators are available.
  \item \textbf{Retirement.} If the retiring operator is currently active, the scheduler needs to be updated as well.
  \item \textbf{Skipping.} For operators who fail to submit block commitments in determined intervals.
  \item \textbf{Slashing.} Forfeiting bond of the appointed operator would remove them from the set, and therefore would require advancing the scheduler.
\end{enumerate}
