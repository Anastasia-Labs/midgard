...
\paragraph{Implementation conformance.} The end-of-shift conditions below retain the original contiguous-shift and early-consent design. In the current \code{scheduler.ak} validator, \code{validate\_end\_of\_shift\_and\_get\_operators} instead requires a short bounded validity interval whose inclusive lower bound is at least the previous shift end, and sets the next shift start to that lower bound. This route is permissionless and does not use the successor signature. The original conditions are not evidence that this stronger scheduling rule is enforced.
