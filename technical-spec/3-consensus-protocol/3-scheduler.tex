...
\begin{enumerate}
  ...
  \item \textbf{Skipping.} Operators who can be deemed inactive can be skipped to ensure network's liveness.
...
\begin{enumerate}
  ...
  \item \code{user\_events\_negligence\_timeout} determines how long operators can ignore user events slated for inclusion within their shifts.
    By referring to a user event utxo that's been waiting for inclusion in a block commitment long enough, an operator can be proven inactive.
  \item \code{new\_shift\_inactivity\_grace\_period} protects newly appointed operators who take over from inactive operators.
\end{enumerate}

Inactivity strikes are accumulated in active operators set datum, and are cleared whenever an operator retires.
