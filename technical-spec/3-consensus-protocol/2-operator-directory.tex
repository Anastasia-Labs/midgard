...
After enough strikes, violating operator's bond can be partially slashed and transferred to the penalty box set.

Operators in penalty box can either recover the remainder of their bond, or deposit the slashed portion in order to get back in the registered operators set.
...
\begin{itemize}
    ...
    \item \code{penalty\_box} is a set that tracks penalized operators for violating liveness of Midgard by taking too long to submit block commitments multiple times.
      This set is also implemented as a key-ascending linked list with operator PKHs as node keys.
\end{itemize}
...
It also keeps track of any holds on the ADA bonds of active, retired and penalized operators, which prevent those operators from recovering their bonds until their latest committed blocks and latest settlement resolution claims mature.
\begin{align*}
    ...
    \T{PenalizedOperator}  &\coloneq \Bigl( \T{bond\_unlock\_time}:
        \T{Option}(\T{PosixTime}) \Bigr)
\end{align*}
