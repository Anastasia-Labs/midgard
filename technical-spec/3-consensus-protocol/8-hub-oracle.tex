...
\paragraph{Implementation conformance.} The initialization conditions below are the intended atomic deployment contract. The current hub mint policy itself checks an exact pair of asset names (the hub NFT and correction-lock token), equal quantities, and consumption of its nonce for minting; it also has a paired burn branch. It does not itself inspect the hub output datum/address or require every list and scheduler initialization. Those stronger conditions require the deployment transaction and participating policies to establish them and must not be attributed to the hub policy alone.
...
The \code{reserve\_observer} field is reserved for a future observer-script design.
The reserve and payout validators defined in \cref{h:reserve-and-payout} do not rely on it for authorization unless this specification is extended with a concrete observer protocol.
