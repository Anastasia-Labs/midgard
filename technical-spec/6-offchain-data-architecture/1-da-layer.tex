...
The current implementation uses a DA committee that retains \code{DaPayloadV2} records keyed by \code{header\_hash}.
The retained payload contains the embedded header, final UTxO members, withdrawal members, forced transaction members, normal L2 transaction members, deposit members, transition trace members, event-to-step members, and counts.
...
\begin{equation*}
    \T{DaPayloadV2} \coloneq \left\{
    \begin{array}{ll}
        \T{version} : & \T{Int} \\
        \T{block\_body} : & \T{DaPayloadBodyV2}
    \end{array} \right\}
\end{equation*}

\begin{equation*}
    \T{DaPayloadBodyV2} \coloneq \left\{
    \begin{array}{ll}
        ...
        \T{transactions} : & \T{[(TxId, ByteArray)]} \\
