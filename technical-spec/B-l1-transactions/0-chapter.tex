...
This chapter describes the principal L1 transaction interactions with Midgard's consensus protocol smart contracts.
The diagrams are schematic: they omit deployment-specific reference scripts, governed DA references, withdrawal witnesses, and some current datum fields. They are not complete transaction templates or evidence that a deployment passes acceptance. Builders and the corresponding validator branches determine the complete transaction shape.
...
\begin{enumerate}
  ...
  \item Produces the scheduler, fraud proof catalogue, hub oracle, correction lock, and DA parameter governor singleton utxos. The current initialization builder does not create an escape-hatch singleton; that mechanism remains a target design.
  \item Registers the rewarding credentials required by the deployed withdrawal-zero helper scripts.
  \item Attaches the signatures necessary to spend the nonce utxo.
\end{enumerate}

Correctness is jointly enforced by the initialization policies, including the hub oracle minting policy's initialization branch (\cref{h:hub-oracle-minting-policy}).
...
\begin{enumerate}
  ...
  \item References an active operator node that proves that the newly assigned operator follows the previously assigned operator in key-ascending order of the active operators list.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item References the active operators' root and last nodes to prove that the previously assigned operator was at the end and the newly assigned operator is at the beginning of the key-ascending order of the active operators list.
\end{enumerate}
...
Merge the mature head block header, whose DA state must be \code{Attested} or \code{Published}, to the confirmed state via a transaction that (\cref{fig:tx-merge-to-confirmed-state}):
\begin{enumerate}
  ...
  \item If the header carries L2 material, mint a settlement NFT and create its settlement utxo, binding the header identity and the required roots and counts. The settlement datum commits to transactions, events, and traces; it does not contain their full payloads.
\end{enumerate}
...
Create a new withdrawal order for Midgard via a transaction that (\cref{fig:tx-withdrawal-order}):
\begin{enumerate}
...
Correctness is jointly enforced by these script redeemers:
\begin{itemize}
  \item Authenticate Withdrawal Order of the withdrawal order minting policy (\cref{h:withdrawal-order-minting-policy}).
  \item Register Staking Credential of the witness staking script (\cref{h:witness-staking-script}).
\end{itemize}
...
\begin{enumerate}
  ...
  \item Produces an output at the reserve address containing the complete deposit input value minus its NFT, with no datum or reference script.
...
\begin{enumerate}
  ...
  \item Mints its payout NFT and produces a payout accumulator with \code{l2\_value}, \code{l1\_address}, and \code{l1\_datum} datum fields matching those of the withdrawal order.
  \item The payout accumulator's initial value, excluding the payout NFT, must not exceed \code{l2\_value} for any asset.
...
Correctness is jointly enforced by these script redeemers:
\begin{itemize}
  \item Mint Payout of the payout accumulator minting policy (\cref{h:payout-minting-policy}).
...
\begin{enumerate}
  ...
  \item Produces a payout accumulator output at \code{payout\_addr} with the same payout NFT and exact same payout datum.
  \item The payout output combines the payout input value, excluding the payout NFT, with the portion of the reserve input needed to approach the payout datum's \code{l2\_value}, without exceeding it for any asset.
  \item Produces a datumless reserve utxo with no reference script if needed to hold the exact remainder of the reserve input.
  \item Pays transaction fees from non-reserve, non-payout inputs.
\end{enumerate}
...
\begin{enumerate}
  \item Spends a payout accumulator with funds exactly matching the amount defined in its \code{l2\_value} datum field, excluding the payout NFT.
  ...
  \item The produced output's value exactly matches the payout accumulator's \code{l2\_value}.
  \item Burns the payout NFT and produces no continuing payout accumulator for the same payout.
  \item Pays transaction fees from non-reserve, non-payout inputs.
\end{enumerate}
...
Correctness is enforced by the BurnForCancellation redeemer of the computation thread minting policy (\cref{h:fraud-proof-computation-threads-minting-policy}).
...
\begin{enumerate}
  ...
  \item If required by the verification step, includes a zero-valued withdrawal under the designated helper credential to execute the required proof helper.
\end{enumerate}

Correctness is jointly enforced by these script redeemers:
\begin{itemize}
  \item Continue of the fraud proof's spending validator (\cref{h:fraud-proof-computation-threads-spending-validators}).
  \item (If needed) The inclusion, exclusion, or other proof helper selected by the step and authenticated against the deployed helper script hash.
\end{itemize}
...
Perform the final verification step of a fraud proof via a transaction that (\cref{fig:tx-fraud-proof-conclude}):
\begin{enumerate}
  \item Spends a single fraud proof computation thread utxo, performing the verification step defined in the utxo's spending validator.
