...
The protocol requires coverage for every possible violation of Midgard's ledger rules (see \cref{h:ledger-rules-and-fraud-proofs}), using an onchain procedure (see \cref{h:fraud-proof-computation-threads}) to verify whether a given fraud proof demonstrates the existence of that violation in a given block.
...
\footnote{Using 4-byte integer indices means that the catalogue can identify up to $2^{32}$ procedures, which is more than sufficient for Midgard's ledger rules.}
