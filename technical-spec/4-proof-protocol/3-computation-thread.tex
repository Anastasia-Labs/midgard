...
In other words, the computation thread is a state machine (see \cref{h:single-threaded-state-machine}) whose execution follows authenticated transitions. Bounded walks can repeat a step and yield helpers can discharge part of a step, so the validator graph need not be a simple linear chain.
...
\begin{description}
    \item[Init.] Conditions:
        \begin{enumerate}
            ...
            \item Other than ADA and its one computation-thread NFT, \code{output\_state} must \emph{not} hold any other tokens.
...
In that case, the step's spending validator requires the computation thread token to be burned via the BurnForCancellation mint redeemer.
...
\begin{description}
            ...
            \item If this is the last step of the computation:
                \begin{itemize}
                    \item Mint the fraud token, which will implicitly burn the computation thread token with the Success redeemer.
                      Let \code{output\_state} be that transaction output.
                    \item The \code{output\_state} datum must be \code{FraudProofDatum\{fraud\_prover\}}; it has no computation-state field.
    ...
    \item[Cancel.] Cancel the computation.
      Conditions:
        \begin{enumerate}
            \item Validate the \code{BurnForCancellation} redeemer from the computation thread policy is invoked (this in turn validates the burning of the specified computation thread token).
