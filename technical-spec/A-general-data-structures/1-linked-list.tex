...
\begin{itemize}
    \item The \textbf{key-unordered} linked list is a cheaper/simpler variant that allows nodes to be appended at the end or prepended at the head and does not enforce the order of keys in the list.
...
In a non-empty list, the last node is the only node with a \code{link} field value of \code{None} because forward traversal through the list must end after this node (backward traversal must start at this node).
...
\begin{itemize}
    ...
    \item Reference-script restrictions are application obligations. The authentication helper returns the optional reference script; authentication alone does not reject one. Use the advanced API when transitions must inspect or preserve reference scripts.
\end{itemize}

Throughout this document we refer to the above conditions with the keyword \textbf{Authentic} for short.
The caller must select every supplied output from the actual transaction outputs, enforce the application datum and destination, and dedicate the list payment credential to list elements. Root and node asset-name namespaces must be fixed and disjoint at deployment. The helper does not independently establish these application preconditions.
...
\begin{itemize}
  ...
  \item \code{input\_anchor} and \code{output\_anchor} must match on address, asset names of their NFTs, and the underlying \code{data} field. The \code{link} changes as required by the operation; fold operations may also update the underlying data.
...
\begin{description}
    ...
    \item[Append unordered.] Add a node to the end of the list.
      Conditions follow Insert without its key-order comparisons; the application must ensure key uniqueness. Additionally:
      \begin{enumerate}
...
\begin{itemize}
  ...
  \item \textbf{Reference scripts.} The default structural API does not expose reference scripts to its callbacks. Applications requiring their preservation or inspection must use the advanced API and enforce the intended rule. Authentication by itself does not prove absence of a reference script.
\end{itemize}
