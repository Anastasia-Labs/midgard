...
\begin{itemize}
    ...
    \item No reference script must be attached to the utxo.\footnote{This is an opinionated design decision that is not strictly necessary for the linked list itself.
      There are two primary reasons for this decision: first, it prevents users from bloating utxos and therefore increasing the min required ADA for them, and second, it keeps the library API from getting too overwhelming by providing users with a potentially attached script.}
\end{itemize}
...
To allow these to be stated only once, all minting and inspection helpers return functions that require provision of these reserved values.
