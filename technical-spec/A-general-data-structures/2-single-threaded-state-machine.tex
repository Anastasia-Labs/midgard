...
\begin{description}
    ...
    The token name binds the initial information. Deterministic transition functions alone do not make it a commitment to the complete execution path: later redeemer inputs can select different transitions or provide additional arguments. Any required path or claim binding must be authenticated by the state and transition rules.
...
This reference input is crucial for the integrity of the state machine's state graph, so the validator must authenticate the reference input against the governed capability, and governance must control its creation and modification.
      ...
      Transitions:
        \begin{description}
            ...
            \item[Accept.] The drafter accepts the reviewer's change requests.
              Next state: Draft.
        \end{description}
    \item[Approved (final state).] The branch has been approved for merging into the main branch.
      Transitions:
        \begin{description}
...
\begin{itemize}
    ...
    \item Defines Approved as the final state.
