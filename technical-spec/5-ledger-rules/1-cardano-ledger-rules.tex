...
The rate is not a constant of this specification: $coins\_per\_utxo\_byte$ is
read from the pinned target-parameter snapshot (C70), so a snapshot change
re-derives every floor and invalidates the pinned vectors and proof-fit
evidence that depend on it. The floor is exactly the target-network limit: it
is linear in the serialized size with slope $coins\_per\_utxo\_byte$ and
carries no additional Midgard margin at any size, so an output the target
network funds adequately is never rejected by this rule. There is no arbitrary
sub-cap, and no substitution of an off-chain wallet check or a constant for the
parameterized floor.
