...
\paragraph{Scope and conformance.} These formulas state ledger obligations and schematic evidence relations; they are not the current validator ABI or a completeness claim for the catalogue. Current proofs authenticate compact sources, counted roots, output commitments, and bounded field carriage as specified in \cref{h:midgard-native-transaction-wire-format}. Accepted normal transactions and payable withdrawals change state; rejected forced transactions and non-payable withdrawals are no-ops. Resolve inputs against the state at the transaction's ordered transition, including preceding forced transactions and deposits, rather than treating the normal-transaction set alone as the whole ledger.
...
\begin{enumerate}
  ...
  \item A DA layer proof must be presented that certifies that $length(outputs(t_1)) \leq index(i)$
\end{enumerate}
...
\label{violation:DOUBLE-WITHDRAW}\paragraph{Conformance qualification.} The current family requires both distinct withdrawal leaves to carry \code{WithdrawalIsValid}. The unqualified equation below is the earlier rule; rejecting a repeated request as non-payable is valid behavior under the current transition semantics.
...
\label{rule:transaction-validity-range}\paragraph{Conformance gap.} The formula below requires interval containment, whereas the accompanying original violation prose used non-overlap. Current validation tests the authenticated \code{block\_slot} against transaction validity scalars. These are distinct predicates; the POSIX/slot boundary noted in \cref{h:midgard-native-transaction-wire-format} must be reconciled rather than assuming this formula is enforced.
...
Current static validation computes the minimum as the validation context's \code{min\_fee\_a} times the canonical native transaction size, plus \code{min\_fee\_b}. This is a size-linear native-transaction rule; it is not a claim to reproduce every component of Cardano L1 fee calculation.
...
\label{rule:required-signatures-are-correct}\paragraph{Conformance gap.} The equality and NON-REQ-SIGNER rule below require the explicit signer list to equal spending key credentials. Current validation checks explicit required-signer witness membership and separately authorizes spent key credentials; an additional explicitly required signer is not rejected solely because it does not own an input. This older equality must not be used as the current acceptance rule.
...
\begin{enumerate}
  ...
  \item Verify that the payment credential of $u$ is a public key hash and that it is absent from the required signers of the transaction, as stated by this subsection's original formula.
\end{enumerate}
...
All native scripts required by spent inputs are provided.
Formal specification:
\begin{empheq}[box=\ledgerRuleBox]{equation*}
\begin{split}
  &\forall t \in Ledger,\; \forall (r, u) \in utxos:\\
    &\quad
      \bigl(
      r \in spend\_inputs(t) \;\land\;
      addr(u) \in Addr^{native}_{v2}
      \bigr) \implies \bigl(
...
This ledger rule is violated if any of the following violations occur:
\begin{itemize-multi}
  \item \hyperref[violation:MISSING-NATIVE-SCRIPT-TX]{MISSING-NATIVE-SCRIPT-TX}
  \item \hyperref[violation:MISSING-NATIVE-SCRIPT-UTXO]{MISSING-NATIVE-SCRIPT-UTXO}
\end{itemize-multi}

\subsubsection{MISSING-NATIVE-SCRIPT-TX violation}
\label{violation:MISSING-NATIVE-SCRIPT-TX}
A transaction $t_1$ spends a utxo $u$, produced in the block by transaction $t_2$ at a native script address, without providing the required script as a witness.
Formal specification:
\begin{equation*}
\begin{split}
  &\exists t_1, t_2 \in txs,\; \exists i \in spend\_inputs(t_1),\; \exists u \in outputs(t_2):\\
    &\quad tx\_hash(i) = tx\_hash(t_2) \;\land\\
    &\quad u = elem\_at(outputs(t_2), i.index) \;\land\\
    &\quad addr(u) \in Addr^{native}_{v2} \;\land\\
    &\quad \bigl(
        \nexists (h, s) \in script\_tx\_wits(t_1),\; script\_hash(addr(u)) = h
      \bigr)
\end{split}
\end{equation*}
...
\begin{enumerate}
  ...
  \item Verify that $i$ matches $t_2$ on transaction hash.
  \item Let $u$ be the $t_2$ output indexed by $i$.
  \item Verify that the payment credential of $u$ is a native script hash.
  \item A DA layer proof must be shown that certifies that there is no $(h, s)$ corresponding to \break
        $script\_hash(addr(u)) = h$ in $script\_tx\_wits(t_1)$.
\end{enumerate}

\subsubsection{MISSING-NATIVE-SCRIPT-UTXO violation}
\label{violation:MISSING-NATIVE-SCRIPT-UTXO}
A transaction $t_1$ spends a utxo $u$ from the previous block's utxo set at a native script address without providing the required script as a witness.
Formal specification:
\begin{equation*}
\begin{split}
  &\exists t_1 \in txs,\; \exists i \in spend\_inputs(t_1),\; \exists (j, u) \in utxos_{pre}:\\
...
\begin{enumerate}
  ...
  \item Verify that $i \in spend\_inputs(t_1)$.
  \item Let $u$ be a utxo and $j$ be an output reference.
  \item A membership proof must be generated that proves that $(j, u)$ is in $utxos_{pre} = prev\_utxos(b)$ for the same block.
  \item Verify that $i = j$.
  \item Verify that the payment credential of $u$ is a native script hash.
  \item A DA layer proof must be shown that certifies that there is no $(h, s)$ corresponding to \break
        $script\_hash(addr(u)) = h$ in $script\_tx\_wits(t_1)$.
\end{enumerate}
...
The Ada component of the mint value must be zero. Canonical V1 supports
minting and burning non-Ada assets subject to value preservation and the
corresponding policy execution.
Formal specification:
\begin{empheq}[box=\ledgerRuleBox]{equation*}
\begin{split}
  &\forall t \in Ledger:\\
    &\quad lovelaces\bigl(mint(t)\bigr) = 0
\end{split}
\end{empheq}
...
The Ada component of the mint value in transaction $t$ is non-zero.
Formal specification:
\begin{equation*}
\begin{split}
  &\exists t \in txs:\\
    &\quad lovelaces\bigl(mint(t)\bigr) \neq 0
\end{split}
\end{equation*}
...
\begin{enumerate}
  \item Let $t$ be the transaction alleged to violate the ledger rule.
  \item A membership proof must be provided that shows that the transaction $t$ is included in the ledger.
  \item The Ada component of the canonical mint value is not zero.
\end{enumerate}

\subsection{Rule: Maximum transaction size}
\label{rule:maximum-transaction-size}

Canonical V1 has no independent 8{,}192-byte aggregate transaction ceiling. Each dynamic canonical field preimage has a 32{,}768-byte maximum. Its commitment is a hash of the entire canonical field; authentication uses the inline, raw-publication, or certified-publication carriage routes described in \cref{h:midgard-native-transaction-wire-format}. The 4{,}095-byte machine chunk reservation is not a universal cap on whole-field carriage or a replacement for each instruction's envelope budget. The effective transaction maximum derives from bounded fields and framing. Each enabled proof instruction still requires serialized transaction and execution-budget evidence; the field bound alone does not establish that every whole-field path fits L1.

\subsection{Rule: Maximum reference script size}
\label{rule:maximum-reference-script-size}

Reference scripts are supported in canonical V1. A reference script contained
in a ledger output shares that output preimage's concrete membership-proof
envelope. Its executable envelope and content-addressed program material are
authenticated and revealed through independently bounded proof steps; the
script itself is not disabled or replaced by an empty placeholder.

\subsection{Rule: Maximum value size}
\label{rule:maximum-value-size}

Canonical V1 caps the canonical CBOR encoding of an output value at
(5{,}000) bytes. This exact value-only bound leaves room for the containing
ledger-output disclosure, membership proof, and transaction overhead inside
the supported L1 fault-proof transaction envelope. The containing output and
transaction fields remain subject to their independently compiled preimage
and chunk bounds.
...
An output of transaction $t$ contains a negative asset quantity.
Formal specification:
\begin{equation*}
\begin{split}
...
The $min\_ada\_value$ function is the Cardano minimum-Ada rule verbatim. The
target ledger prices an output at its serialized size plus the fixed
$160$-byte UTxO entry overhead it adds to every output, at the target
network's $coins\_per\_utxo\_byte$ protocol parameter:
\[
    min\_ada\_value(o) =
      coins\_per\_utxo\_byte \cdot
      \bigl(160 + |\T{serialised\_output}(o)|\bigr),
\]
where $|\T{serialised\_output}(o)|$ is the length in bytes of the canonical
output encoding of $o$ used throughout this specification.

The floor is parameterized in the rate, and the rate is materialized as a
compiled deployment constant: $coins\_per\_utxo\_byte$ is taken from the pinned
target-parameter snapshot (C70) and compiled into the validators, pinned
identically across deployment environments and mirrored by the off-chain
consensus profile. It is deliberately neither a block-header field nor an
element of the hashed validation context: the rate that convicts a block is not
the block producer's to declare. A rate change is therefore a redeploy event,
not a per-block declaration --- it re-derives every floor and invalidates the
pinned vectors and proof-fit evidence that depend on it. What is forbidden is
replacing the size-linear floor with a constant; storing the rate as a constant
is not that substitution, since both implementations still take the rate as a
parameter and the cross-check below exercises them at several distinct rates.

The floor is exactly the target-network limit: it is linear in the serialized
size with slope $coins\_per\_utxo\_byte$ and carries no additional Midgard
margin at any size, so an output the target network funds adequately is never
rejected by this rule. There is no arbitrary sub-cap, and no substitution of an
off-chain wallet check or a constant for the parameterized floor.

Both violations below evaluate this same $min\_ada\_value$: MIN-ADA-TX over an
output of a transaction in the block, and MIN-ADA-UTXO over a UTxO of the
post-block set. An under-funded output is therefore convictable whether it is
produced by a transaction under proof or is only observable in the resulting
UTxO set, and cannot evade conviction by reaching state through a route a
transaction-shaped proof cannot address.

The rule has two hand-written implementations --- the on-chain validation
machine's $\T{min\_ada\_lovelace\_v1}$ / $\T{output\_meets\_min\_ada\_v1}$ and
their off-chain $\T{minAdaLovelaceV1}$ / $\T{outputMeetsMinAdaV1}$ twin --- and
neither derives from the other. They are bound together by a cross-language
cross-check that pins absolute floors, in lovelace, over shared serialized
output vectors, including the exactly-at-the-floor accept and
one-lovelace-below reject legs. Changing the slope, the intercept, or the
comparison in either copy alone fails that cross-check.
...
A transaction $t$ attempted to reference the UTxO $i$ that does not exist or was spent in a previous block.
Formal specification:
\begin{equation*}
\begin{split}
...
A transaction $t$ attempted to reference an input $i$ that was not produced by the transaction matching the tx hash of $i$.
Formal specification:
\begin{equation*}
\begin{split}
  &\exists t \in txs,\; \exists i \in reference\_inputs(t): \\
    &\quad\bigl( \exists t_1 \in txs,\;
      tx\_hash(t_1) = tx\_hash(i) \;\land\;
      length(outputs(t_1)) \leq index(i) \bigr)
\end{split}
\end{equation*}
...
\subsubsection{WITHDRAWN-REFERENCE-INPUT violation}
\label{violation:WITHDRAWN-REFERENCE-INPUT}
A transaction $t$ attempted to reference the UTxO $i$, which was spent in a withdraw transaction.
Formal specification:
\begin{equation*}
\begin{split}
  &\exists t \in txs,\; \exists i \in reference\_inputs(t),\; \exists w \in wtxs:\\
    &\quad i = l2\_outref(w)
\end{split}
\end{equation*}
...
\begin{enumerate}
  \item Let $t$ be the transaction alleged to violate the ledger rule. 
  \item A membership proof must be provided for $t$ that shows that the transaction is included in the block ($t \in txs$)
  \item A membership proof must be generated for input $i$ such that $i \in reference\_inputs(t)$
  \item A membership proof must be created to show that $w$ is in $wtxs$, which also spends input $i$
  \item A non-membership proof must also be generated that shows that no transactions in the block match the input's tx hash.
\end{enumerate}
