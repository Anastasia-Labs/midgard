...
\subsection{Rule: No mint or burn}
\label{rule:no-ada-minted}
The canonical mint preimage must be empty; no asset, including Ada, may be
minted or burned by a launch-profile L2 transaction.
Formal specification:
\begin{empheq}[box=\ledgerRuleBox]{equation*}
\begin{split}
  &\forall t \in Ledger:\\
    &\quad mint(t) = \varnothing
\end{split}
\end{empheq}
...
\subsubsection{MINT-OR-BURN violation}
\label{violation:ADA-MINTED}
The mint preimage of transaction $t$ is non-empty.
Formal specification:
\begin{equation*}
\begin{split}
  &\exists t \in txs:\\
...
\begin{enumerate}
  \item Let $t$ be the transaction alleged to violate the ledger rule.
  \item A membership proof must be provided that shows that the transaction $t$ is included in the ledger.
  \item The canonical mint preimage is not the empty list.
\end{enumerate}
...
the compact transaction and has a compiled maximum derived from the supported
(16{,}384)-byte L1 fault-proof transaction floor and the concrete overhead of
the one-step instruction that reveals it. For every instruction,
\[
    |\T{field\_preimage}| + |\T{reserved\_proof\_overhead}| <
    16{,}384.
\]
...
Canonical V1 assigns no arbitrary independent cap to a canonical value. The value
shares the compiled preimage envelope of the output or transaction field that
contains it. A narrower value-only limit is permitted only in a distinct
instruction whose concrete L1 proof reveals that value as one independent
preimage and whose measured proof overhead requires it.
