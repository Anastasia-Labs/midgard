...
Canonical V1 has no independent (8{,}192)-byte aggregate transaction
ceiling. Each dynamic canonical field preimage is independently committed by
the compact transaction and has a (32{,}768)-byte aggregate maximum. Proof
instructions consume each field through authenticated fragments or chunks of
at most (4{,}095) bytes, derived from the supported (16{,}384)-byte L1
fault-proof transaction floor and the concrete overhead of the one-step
instruction that reveals it. For every instruction,
\[
    |\T{revealed\_fragment\_or\_chunk}| +
    |\T{reserved\_proof\_overhead}| <
    16{,}384.
\]
The effective maximum canonical transaction size is the sum of every bounded
dynamic field after its canonical byte-string wrapping, plus all fixed-size
fields and CBOR framing. It is not an independently selected protocol cap.
Each enabled proof instruction must additionally pass its execution-unit and
serialized worst-case transaction test.
