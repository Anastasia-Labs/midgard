# Cardano capability P2 nested Data checkpoint — 2026-07-26
...
one list node, not padding. The balanced tree keeps construction depth
logarithmic; no library-specific recursion ceiling is treated as a Cardano
capability rule.
...
## Cross-language terminals

The TypeScript terminal controls, final frames, successor controls, and
summaries are pinned. Aiken applies the exact final frame transition and
rejects a substituted sequence root:
...
- `aiken check -m maximum_cardano_nested` (`2/2` tests).
