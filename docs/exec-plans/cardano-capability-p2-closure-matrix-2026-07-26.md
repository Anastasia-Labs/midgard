...
| Datum and redeemer Plutus `Data` maximum shapes | **PASS** — exact maximum nested datum and schema-parallel redeemer transactions survive both classifications; the genuine collateralized redeemer remains fail closed | **PASS** — datum/redeemer Data lengths and 10,776/10,650 semantic-node shapes are exact | **PASS** — the datum is chunk-bound inside the output proof; the redeemer field is four bounded chunks; every semantic Data source span is at most 14 bytes | **PASS** — exact maximum constructor/list/map/scalar traversal terminals reconstruct in TypeScript | **PASS** — exact maximum datum and redeemer terminal transitions agree in Aiken and substituted roots reject | **PASS** — datum `5,387` leaves / `16,382` signed bytes and redeemer `5,324` leaves / `16,382` signed bytes pass; their one-leaf adjacent shapes are `16,385` bytes and reject Cardano `maxTxSize` | **PASS** |
...
1. close script-envelope and program-material nodes;
2. close maximum incremental CBOR scans;
3. backfill any remaining maximum-specific ordered-field Aiken terminal
