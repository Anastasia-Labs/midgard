...
| Nested output `Value` policy/asset content | **PASS** — the exact 5,000-byte maximum-Value transaction and canonical output survive both classifications | **PASS** — seven policies and the maximum 1,592 distinct policy/asset entries are bound by the output descriptor and asset frontier | **PASS** — two output chunks and 1,592 authenticated reverse-membership steps; the largest asset witness payload is 358 bytes | **PASS** — 3,198 typed output-proof steps reach the exact terminal Value summary and descriptor | **PASS** — exact maximum terminal and cross-policy membership transitions match TypeScript; substituted quantity rejects | **PASS** — at protocol major `11`, exact canonical Value sizes `5,000`/`5,001` satisfy/violate the official `validateOutputTooBigUTxO` snapshot rule against `maxValueSize = 5,000`, while both signed transactions remain below `maxTxSize`; independent Midgard parity rejects the adjacent case with `E_VALUE_SIZE` | **PASS** |
| Datum and redeemer Plutus `Data` maximum shapes | **PASS** — exact maximum nested datum and schema-parallel redeemer transactions survive both classifications; the genuine collateralized redeemer remains fail closed | **PASS** — datum/redeemer Data lengths and 10,776/10,650 semantic-node shapes are exact | **PARTIAL** — the datum is chunk-bound inside the output proof and every Data span is at most 14 bytes; the redeemer field is four bounded chunks and its semantic traversal is exact, but the active challenged-transition redeemer scanner still requests the whole raw Data value and retains its obsolete 9,215-byte cap | **PASS** — exact maximum constructor/list/map/scalar traversal terminals reconstruct in TypeScript | **PASS** — exact maximum datum and redeemer terminal transitions agree in Aiken and substituted roots reject | **PASS** — datum `5,387` leaves / `16,382` signed bytes and redeemer `5,324` leaves / `16,382` signed bytes pass; their one-leaf adjacent shapes are `16,385` bytes and reject Cardano `maxTxSize` | **PARTIAL** |
...
1. replace the active whole-raw redeemer scan and its obsolete 9,215-byte cap
   with the retained bounded-chunk Data traversal;
