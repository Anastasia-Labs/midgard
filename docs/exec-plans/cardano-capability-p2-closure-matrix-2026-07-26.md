...
- the bounded observer test exhaustively folds `16` hashes in reverse for both
  Cardano withdrawals-map and Midgard list encodings, rejects adjacent `17`,
  duplicate, descending, and non-28-byte inputs, and agrees with the exact
  `24`-field CEK context-control ABI;
...
- the semantic maximum-`16` observer proof and terminal fold agree with the
