...
- **Integrity:** this file is tracked in Git under `docs/exec-plans/` (§2.4);
  Git already content-addresses it, so amendment history and drift detection
  come from `git log -p -- docs/exec-plans/GOAL_SPEC.md`. Do not duplicate its identity in a
...
- **Owner amendment 2026-08-08 (flat field-hash reversion, scheme
  altitude):** the nine compact-tx per-field commitments are flat
  blake2b-256 hashes over canonical enveloped field-preimage bytes, defined
...
3. `docs/spec/midgard-tx.md` — the implementation-normative format authority
   for the MidgardTx type, canonical encoding, the nine flat field
   commitments, the enveloped preimage grammar, and the field-preimage
   carriage convention. Per `docs/spec/README.md`, a `docs/spec/` component
   specification wins over the technical specification on concrete detail.
4. `docs/consensus-profile-v1.md` and the exact compiled canonical V1 profile.
5. `docs/fault-proofs/execution-plan.md`,
   `docs/fault-proofs/coverage-matrix.md`, and
   `docs/fault-proofs/catalogue-status.md`.
6. `docs/midgard/decisions/watcher-verification-boundaries.md` and
   `docs-site/content/docs/watchers/concept.mdx`.
7. The normative technical specification: `technical-spec/`, entry point
   `technical-spec/midgard.tex`, built into `technical-spec/midgard.pdf` by
   the root `make spec` target.
8. Current source, generated validators, and tests as implementation
   evidence.

Precedence by claim type:

- **Protocol semantics and on-chain rules:** the normative technical
  specification and the accepted decision/plan/component-spec documents
  (items 2–8), with current deployed source as implementation evidence, in
  that order. On concrete format detail covered by a `docs/spec/` component
  specification (item 3), that specification wins over the technical
  specification; divergences are technical-spec errata.
...
Complete the capability, control-plane, proof, release, and bounded-deployment
requirements in §8:
...
- Escape-hatch implementation and broader operator-liveness work unless an
  ...
  removal of an expired unattested commitment and its dependent suffix (Q61) is inside
...
- updated capability, fault-coverage, catalogue, and public-readiness status
...
14. **Necessary work is finished, not discarded.** No task may delete,
    ...
    This rule protects required implementation, not obsolete documentation.
    Remove delivered plans and superseded evidence without a current executable
    consumer; preserve lasting decisions in concise ADRs. A historical citation
    or old instruction to retain a record is not a present need.
...
2. **Large Values and Data are complete-item-first.** Exact signed quantities,
   ...
   Value bookmark, the Canonical-Data Acceptor) are defined in the component
   specification's §§10–11. Its §7 invariants, including positions rather
   than bytes in resumable state, remain binding on every consumer. No
...
representation that fits, in this order (the three carriage tiers are
defined normatively in `docs/spec/midgard-tx.md`):

1. **Tier 1 — redeemer carriage:** carry the complete item or field preimage
   directly in the proof transaction, verified in place against its flat
   commitment;
2. **Tier 2 — single raw UTxO:** publish the complete preimage once as a
   nothing-but-bytes inline datum in a single dedicated output, then
   consume or reference that output from the proof transaction, verifying
   the whole preimage against its flat commitment;
3. **Tier 3 — chunked + certified digest-manifest:** when one publication
   transaction cannot carry the complete preimage, accept the same complete
   logical item at the public API and transparently publish it as
   deterministic fixed-size chunks plus one certified digest-manifest bound
   to the disputed transaction's committed identity, for later
   reference-input use; and
...
2. **Execution fit:** execution memory and CPU are at or below the
   ...
   (`docs/consensus-profile-v1.md` §10). _Basis note (owner amendment
   2026-08-08):_ every flat-reversion measurement is judged against one
   ...
   14,000,000 cap. _Provenance of the 16,500,000 cap, stated exactly:_ the
   ...
   parameter binding, which supersedes both.
...
2. Inspect `git rev-parse HEAD`, branch, `git status --short`, tool versions,
   ...
   document.
3. Reconcile current source against §8.2 capability requirements and the live
   fault-proof and watcher references before assigning implementation work.
...
The delivered progress diary has been removed; its original text remains in
Git history. Progress notes are not an authoritative database, do not define
task state, and must not be parsed by scripts, CI, closure manifests, or release
gates. See [the verification decision](../midgard/decisions/semantic-verification-over-bookkeeping.md). Current truth comes from the
source, focused tests, domain documentation, open issues/decisions, and the
final human-readable completion report. Remove obsolete historical notes once
current requirements and lasting decisions have a maintained home; they do not
create implementation blockers by themselves.
...
- Generated `onchain/aiken/plutus.json` belongs to the parent integration
  ...
  pinned by `.github/workflows/aiken-ci.yml` (the base version alone in
  `onchain/aiken/aiken.toml` does not identify the patched compiler).
...
acceptance run under IG5 are required before final closure. `goal:accept:testnet`
is the required C79 orchestrator surface; it is not a currently registered
workspace package command and must not be presented as runnable yet.
...
| F10 | P0/P1/P2 evidence reconciliation                  | F01–F02    | Re-run or invalidate every claimed P0/P1/P2 pass against current source; reconcile the §8.2 capability requirements without weakening them.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
count/length, complete-item tier-1 direct or tier-2 single raw-UTxO
reference carriage (§3.2) wherever that path fits, exhaustive TypeScript
terminal, Aiken terminal agreement, accepted Cardano maximum, and
immediately adjacent rejection where Cardano defines a boundary. Tier-3
chunked or incremental evidence is required only for a family with a §3.2
necessity artifact, and must coexist with the complete-item path for
ordinary fitting items.
...
| --- | ------------------------------------------- | ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| C21 | Complete proof-item carriage                | F10          | Public tooling accepts the complete canonical item; the tier-1 direct and tier-2 single raw-UTxO reference paths (§3.2, per `docs/spec/midgard-tx.md`) are constructed and measured first. Any tier-3 chunked or incremental fallback has a §3.2 necessity artifact, preserves the ordinary complete-item path, and rejects omission, duplication, reorder, substitution, and trailing data. |
...
| CG2 | Proof-item capability gate                  | C20-0–C33    | Every applicable capability requirement in §8.2 is proven; complete-item carriage through the simplest fitting §3.2 tier remains available wherever it fits; every tier-3 or incremental fallback has a measured §3.2 necessity artifact and exact semantic-equivalence tests; production searches and ABI tests reject unjustified bounded-only paths.                                      |
...
3. Correct flat field-hash/preimage-grammar binding per
...
| Q61 | Attestation-timeout removal (D-L1 i)              | F04, Q00–Q01, Q52 | A non-faulty committed block that never receives the required DA attestation becomes removable after the F04 `da_attestation_timeout` without slashing the operator bond (the D-L1 recommendation). Removal is permitted at any pending queue position, prunes descendants under the correction lock, and preserves the preceding prefix without waiting for its maturity. Premature removal, attested-block removal, and post-timeout attestation races reject. Off-chain actuation has a named owner: a resumable permissionless command exists, the operator-node scheduler is the default production driver, and an alert fires when a block nears the timeout unattested; the watcher observes but does not own this liveness action. The wider escape hatch stays out of scope per §2.3. |
| Q62 | Current-committee DA apply binding (D-DA4)        | F20, Q00          | State-queue apply references the current DA-params state and requires the frozen `committee_signers_hash`/`da_threshold` to still match the governed values; `get_da_params` re-derives the committee hash. A quorum gathered under a rotated-out committee cannot apply.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| W38 | API, metrics, and alerts            | W03–W37                   | Read-only status/diagnostics; queued age, verification/DA latency, deadlines, proof steps, events, L1-source freshness; alerts required by the watcher verification boundaries; bounded/paginated and secret-safe.                                                                                     |
...
- rebuild with the Aiken fork/revision pinned by `.github/workflows/aiken-ci.yml`;
...
(Owner amendment 2026-08-08: this is the first amendment to edit §12 itself.
Every previous amendment left §12 byte-identical and said so; this one
cannot, because AC-Q12, AC-C20, and AC-C21 named the retired counted/typed
commitment scheme and the pre-reversion carriage vocabulary. Criteria
stability yields to scheme truth: the edits below rebind those criteria to
the flat field commitments and §3.2 carriage tiers per
`docs/spec/midgard-tx.md` without weakening any acceptance obligation.)
...
- **AC-C20:** Every P2 family and cell passes CG2: authenticated normal/forced
  source, exact typed count/length, complete-item proof-fit through the
  simplest fitting §3.2 carriage tier (tier-1 direct or tier-2 single
  raw-UTxO reference), exhaustive TS terminal, Aiken terminal, and
  applicable maximum/adjacent Cardano boundary. Any tier-3 chunked or
...
- **AC-C21:** Production searches, ABI tests, and applied proof-fit evidence
  ...
  every fitting item uses a tier-1 direct or tier-2 single raw-UTxO
  reference path without mandatory prover-managed chunking. Every tier-3 or
  incremental fallback is limited to a concretely justified family/shape,
  preserves the complete-item path for fitting cases, and is semantically
  identical.
...
- **AC-Q12:** All proof decoders bind the canonical native V1 flat field
  commitments and enveloped preimage grammar per `docs/spec/midgard-tx.md`;
...
# The normative technical specification (§1 item 7) must build from the
# repository root. Skip it when technical-spec/ is unchanged in the diff.
make spec

# Aiken uses the fork/revision in .github/workflows/aiken-ci.yml.
# aiken.toml supplies only the base version. Run in the contract project.
(
  cd onchain/aiken
  aiken --version
  aiken fmt --check
...
# Required exact large Aiken vectors use the guarded runner from onchain/aiken.
# Replace placeholders with real source module/test names; assert nonzero count.
(
  cd onchain/aiken
  node scripts/run-focused-check.mjs <module> <exact-test-name> [<exact-test-name> ...]
)

# Explicitly serialize workspace build/typecheck; the root shortcuts use four workers.
...
- complete-item tier-1 direct and tier-2 single raw-UTxO
  publication/reference transaction identities, plus every §3.2 necessity
  artifact and any justified tier-3 chunked or incremental path;
...
Evidence is committed on an appropriate domain evidence path and referenced by
...
that _was_ implemented produced recurring red gates and one mutually
