# Goal artifact templates (F05)

Worked examples for the artifact genres `GOAL_SPEC.md` requires. These are
templates — placeholder values, excluded from evidence aggregation; copy the
shape, never the values.

- `task-manifest-row.json` — a fully detailed (`detailStatus: READY`) row for
  `docs/exec-plans/evidence/canonical-v1-goal-task-manifest-v1.json`.
- `necessity-artifact-template.md` — the §3.2 bounded-fallback necessity
  artifact.
- `structural-na-template.md` — an executable structural `N/A` claim (§9.1,
  Q49/F21).
- `evidence-binding-template.json` — a §13.4 evidence binding (repository and
  external forms).
- `subagent-assignment-brief.md` — the §5.1 assignment brief quoting a
  manifest row.
