...
onchain/aiken/
  validators/
    ...
    fraud-proofs/<type>/step-NN.ak  # 13 top-level compiled proof families
  lib/midgard/
...
| --------------------------- | -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `zero-input`                | 2                    | native counted-root membership in step 01; `bad_tx_spend_inputs_hash` must equal the bounded-collection-v1 field-0 empty commitment (`eb25ed4ae02426602eee44b29d93e9dcd0be514b2087eda02f398b16fbb0ec76`; `zero-input/step-02.ak:20-25,78-102`). This deliberately does not use the PlutusData `env.empty_list_hash`. |
...
| `validation-trace`          | interactive dispute  | opener/source/game/boundary/timeout/award controls plus prepare, semantic, and direct resolvers; catalogue category `00000006`. The compiled family and TypeScript submission tests do not by themselves establish full emulator/preprod acceptance.                                                                 |
...
| `validators/fraud-proofs/zero-input/step-02.ak:95-239`                           | 5                  | native empty-list encoding/literal, non-empty inequality, and direct full-handler accept/reject fixtures; exact-tree blueprint comparison confirms the test declarations do not change deployed script hashes |
...
invalid-range step-01 and zero-input step-02 — at the Aiken level these are exercised
only indirectly via the TypeScript emulator suite
(`demo/midgard-fault-proofs/tests/submit-init-emulator.test.ts`),
