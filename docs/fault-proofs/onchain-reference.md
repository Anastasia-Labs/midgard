...
| `reference-input-no-idx`    | 4                    | Reference-input mirror of `input-no-idx`: `producing_tx_id == bad_reference_input_tx_id` (`step-03.ak`); `bad_reference_input_output_index >= list.length(outputs_preimage)` (`step-04.ak`). Step-01 commits `reference_inputs_hash`; steps 02-04 compile to the same UPLC as `input-no-idx`'s and are shared with it |
...
| `invalid-signature`         | 2                    | `verify_ed25519_signature(vkey, bad_tx_id, sig) == False` at the accused index (`step-02.ak`), after unwrapping the `NativeTxWitnessSetCompact` preimage against `witness_set_hash` and the address-witness list against `addr_tx_wits_hash`; native counted-root binding |
...
| File                                                                             | Tests | Coverage                                                                                                                                                                                                                |
| -------------------------------------------------------------------------------- | ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `lib/midgard/fraud-proofs/transition-trace/proof.test.ak`                        | 23    | accept/reject pair per family; gaps: `CountFault` 1/5 sub-variants, `OmittedDueL1Event`/`OutOfWindowSourceEvent` deposit-only, `SourcePhaseMismatch` zero, `ValidForcedTransactionUnsupported` reject-only (`:869-900`) |
