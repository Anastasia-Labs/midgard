...
| Lucid Emulator end-to-end                   | ✅ Q13 plus existing family evidence      | the `submit-init-emulator*.test.ts` family includes Q13 `input-no-idx` init → all steps → permanent token → faulty-block removal, alongside existing double-spend, invalid-range, non-existent-input, transition-trace, zero-input, and validation-dispute evidence. Q13's emulator evidence is local only; Q14–Q20 and system-wide preprod acceptance remain open.                                                                                                                                                |
