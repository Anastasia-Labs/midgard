...
| Preprod / real testnet                      | 🟠 zero-input reported; system unconfirmed | PR #461's author supplied a preprod zero-input transaction sequence through removal after the counted-root/native-MPF changes, but it was not independently rerun and the repository lacks a reproducible automated acceptance artifact. The older operator-local 2026-05-08 canonical-root report predates counted roots and the MPF rewrite. System-wide readiness remains unconfirmed; `public_testnet_readiness.md` still lists fault proofs "Partial, not public-testnet ready". |
| Autonomous end-to-end (detect→prove→remove) | ❌                                         | Watcher L1-source, consistency/finality, rollback, durable-state, and protocol-indexing code exists, but no acceptance artifact proves a continuous committed-block classify→prove→remove loop.                                                                                                                                                                                                                                                                                       |
