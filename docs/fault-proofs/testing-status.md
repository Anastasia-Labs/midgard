...
| Lucid Emulator end-to-end                   | ✅ for 8 families                         | `submit-init-emulator.test.ts`: full chains for double-spend, invalid-range, non-existent-input, transition-trace, zero-input, no-reference-input, input-no-idx, and reference-input-no-idx from `submitInit` through fault-proof-token mint to `submitRemoveFaultyBlock`, incl. tail + non-tail removal topologies and lease edge cases; `spend-input-witness.test.ts` (180-input witness)                                                                                                                                                 |
...
| `prepare-double-spend.test.ts` / `prepare-invalid-range.test.ts` / `prepare-non-existent-input.test.ts` / `prepare-zero-input.test.ts` / `prepare-no-reference-input.test.ts` / `prepare-input-no-idx.test.ts` / `prepare-reference-input-no-idx.test.ts` | unit (mocked fetch/file)                 | violation detection + MPF proof generation per family; zero-input pins and requires the authoritative counted-root; no-reference-input and input-no-idx commit the reconstructed root to the counted root and fail closed when an expected root is supplied; input-no-idx also detects the out-of-range spend input and binds both bad + producing tx; reference-input-no-idx mirrors it over the reference-input list |
