...
| Lucid Emulator end-to-end                   | ✅ for 7 families                         | `submit-init-emulator.test.ts`: full chains for double-spend, invalid-range, non-existent-input, transition-trace, zero-input, no-reference-input, and input-no-idx from `submitInit` through fault-proof-token mint to `submitRemoveFaultyBlock`, incl. tail + non-tail removal topologies and lease edge cases; `spend-input-witness.test.ts` (180-input witness)                                                                                                                                                 |
