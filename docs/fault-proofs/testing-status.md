...
| TypeScript unit tests                       | ✅ broad                                      | prepare-\* logic, MPF proofs, lease protocol, contract inspection, transition-trace detection/reconstruction, retained-DA binding, blueprint decoding, and validation-dispute submission (all 23 files in `demo/midgard-fault-proofs/tests/`)                                                                                                                                                                                                                                                                                                                                                                                                                              |
| Lucid Emulator end-to-end                   | ✅ for 5 legacy families + validation dispute | the `submit-init-emulator*.test.ts` family (`submit-init-emulator`, `-ledger-rules`, `-removal-lease`, `-transition-trace`, `-validation-dispute`, `-soundness`): full chains for double-spend, invalid-range, non-existent-input, transition-trace, and zero-input through faulty-block removal, plus canonical V1 validation-dispute lifecycle coverage. The family is one suite split across files only because `@lucid-evolution/uplc` never reclaims wasm linear memory and vitest isolates per file (see `tests/support/uplc-heap-guard.ts`); `pnpm --dir demo run test:tx-prep:emulator` runs all of it. `spend-input-witness.test.ts` covers the 180-input witness |
...
| -------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `submit-init-emulator*.test.ts` (6 files)                                                                                              | Emulator + real `plutus.json`            | the strongest e2e evidence in the repo (see above)                                                                            |
