...
| Lucid Emulator end-to-end                   | ✅ broad local lifecycle evidence         | the `submit-init-emulator*.test.ts` family includes Q13 `input-no-idx` and Q20 standalone min-fee init → all steps → permanent token → faulty-block removal, alongside double-spend, invalid-range, non-existent-input, transition-trace, zero-input, and validation-dispute evidence. Q20 additionally pins exact-boundary honest refusal, cancel/resume and pre-registration reference-script execution. System-wide preprod acceptance remains open.                                                            |
