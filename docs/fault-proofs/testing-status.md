...
| Lucid Emulator end-to-end                   | ✅ for 6 families                         | `submit-init-emulator.test.ts`: full chains for double-spend, invalid-range, non-existent-input, transition-trace, zero-input, and no-reference-input (`:4282`) from `submitInit` through fault-proof-token mint to `submitRemoveFaultyBlock`, incl. tail + non-tail removal topologies and lease edge cases; `spend-input-witness.test.ts` (180-input witness)                                                                                                                                                     |
...
| `prepare-double-spend.test.ts` / `prepare-invalid-range.test.ts` / `prepare-non-existent-input.test.ts` / `prepare-zero-input.test.ts` / `prepare-no-reference-input.test.ts` | unit (mocked fetch/file)                 | violation detection + MPF proof generation per family; zero-input pins and requires the authoritative counted-root; no-reference-input commits the reconstructed root to the counted root and fails closed when an expected root is supplied |
