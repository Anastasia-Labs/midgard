...
| Emulator today                | no resolver-7 fixture; `resolver-proof-fit-sweep-generate.test.ts` records resolver 7 in `unfit[]`                                                                                                                                                                                                                                                                                                                        |
...
and every work-witness byte stay identical; `midgard-core/src/ledger-output-proof.ts`
...
- `refuses a forged successor at the same frontier` (valid-block negative): the honest operator successor is presented as the challenger's claim at the same evidence frontier; expect on-chain refusal (`expect-onchain-refusal.ts`) — the yield rejects the forged `next_control_cbor`.
...
- Remove `oversized: true` for resolver 7 from `dispute-scenario.ts` / `tests/support/emulator/reference-scripts.ts`; the C53 fit sweep (`resolver-proof-fit-sweep-generate.test.ts`) moves resolver 7 out of `unfit[]`.
...
pnpm test -- tests/semantic-resolver-arity-gate.test.ts tests/compiled-script-arity-gate.test.ts tests/validation-dispute-submit.test.ts tests/inspect-contracts.test.ts
...
cd ../midgard-node && pnpm test -- tests/deployment-manifest.test.ts
```
