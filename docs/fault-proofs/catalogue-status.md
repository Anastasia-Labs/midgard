...
(`demo/midgard-sdk/src/fraud-proof/catalogue.ts:23-29` — only 5 categories). Unregistered
...
| --- | --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- | ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- | -------------------------------------------------------------- |
| 1   | `double-spend` (4 steps + input-witness)            | Two in-block txs spend the same UTxO. Decisive: `tx1_id != tx2_id` + equal spent outref (`step-04.ak:82`).                                                                                                                                                                                                                                            | REAL                              | ✅ `doubleSpend`             | ✅ `prepare-double-spend` + `submit-step-01..04` (5 manual CLI steps)                                                                  | ✅ through removal (`tests/submit-init-emulator.test.ts`) | ✅ Complete & verified (emulator only; not preprod)            |
...
| 6   | `zero-input` (2 steps)                              | Tx spends zero inputs; `spend_inputs_hash == env.empty_list_hash` (`step-02.ak:64`).                                                                                                                                                                                                                                                                  | REAL logic; **legacy binding** ⚠️ | ❌ not registered            | ❌ none                                                                                                                                | ❌                                                        | 🔶 Implemented, not fully verified — **binding blocker (§1a)** |
...
### 1a. Binding blocker — eight compiled proofs cannot bind to current counted-root native-v1 blocks

Only `double-spend`, `no-input`, and `invalid-range` use the native binding path
...
The other eight step-1 validators (types 5–12 above) still call the legacy
...
this membership check cannot succeed against a real block. **Consequence:** these eight
...
| `SourceMembershipMismatch` (`:52`, impl `:600-643`)       | Sub-variants `MappedEventMissingFromSource`, `SourceEventMissingTrace`, `SourcePhaseMismatch` (`:108-127`)                                                                                                                                                                                             | REAL                        | 🟠 `SourcePhaseMismatch` untested                                 |
| `InvalidOneStepTransition` (`:53`, dispatch `:1159-1241`) | 6 sub-variants: `ValidWithdrawalTransition` (`:981`), `InvalidWithdrawalNoOpTransition` (`:1010`), `InvalidForcedTransactionNoOpTransition` (`:1034`), **`ValidForcedTransactionUnsupported` → hard-wired `False` (`:1201`)**, `ValidDepositTransition` (`:1058`), `L2TransactionTransition` (`:1117`) | REAL except disabled branch | 🟠 disabled branch has reject-only test (`proof.test.ak:869-900`) |
...
**Disabled branch**: there is currently no way to prove a fault where an operator should
have executed a _valid_ forced transaction's effect but produced a wrong or no-op delta.
This matches the spec's fail-closed launch gate — the tx-order validator "rejects the
unsupported valid-forced redeemer path"
(`technical-spec/1-ledger-state/5-transaction-order-event.tex:45-47`) — so within the
launch configuration the branch is a documented deferral, not an exploitable hole; it
becomes one the moment valid forced transactions are enabled.
...
| SDK catalogue deployment (`demo/midgard-sdk/src/fraud-proof/catalogue.ts`)                    | 🟠     | 5 of 12 categories; single first-step hash per category; TODO for multi-step design (`common.ts:160-161`)                                                                                                               |
...
- **Delivered & functional (emulator-proven)**: generic machinery; double-spend, no-input,
  invalid-range full chains; transition-trace engine + library tooling.
- **Delivered, functional on-chain, but unreachable in deployment**: zero-input,
...
| `valid-forced-transition`     | enable the `ValidForcedTransactionUnsupported` branch (extends `transition-trace`, not a new type)                                                                                                                                                                                                                                    | W-C8      | D-S9                        | censorship                                            |
...
- **W-C13** — port the eight legacy-binding proofs to the native counted-root path
