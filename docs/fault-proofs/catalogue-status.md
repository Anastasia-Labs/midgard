...
| `da-hash-preimage`            | DA payload hash/preimage mismatch (`6-transaction.tex:175`). **Implemented and registered (Q44)** as catalogue category `daHashPreimage` (`00000007`): a committed `transactions_root` leaf whose key is not the canonical native-V1 transaction id of its own value. `LOCAL_PASS`; emulator/testnet evidence pending.                 | complete; evidence pending | D-DA1 (committee bond) open | provability                                           |
