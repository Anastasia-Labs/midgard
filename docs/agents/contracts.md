...
lives in `demo/midgard-sdk/src/fraud-proof/contracts/blueprint.ts`.
...
the deployed, parameterized validator behaves as intended.
...
polarities. A dedicated arity check does not replace behavior tests.

Completion requires passing positive and negative scenarios using the updated
deployment path and reporting exactly which checks ran.
