...
| SDK/provider                        | Partial                           | Provider submit, protocol-info parsing, DA transport V1 envelope codecs, and `DaPayload` payload codecs exist. Packaging, abort/timeout behavior, and public docs need hardening.                                                                                                                                        |
...
- [x] Persist and serve canonical `DaPayload` block-body payloads for locally finalized blocks.
...
- [x] Locally finalized blocks persist canonical DA payload records keyed by header hash.
  - Evidence: local finalization builds `DaPayload`, recomputes committed roots/counts, and upserts `da_payloads` before marking the pending finalization complete.
...
- [x] Define and expose the current node-produced DA payload shape.
  - Evidence: `DaPayload` includes header hash, header CBOR semantics, sorted UTxO, withdrawal, forced-transaction, L2 transaction, deposit, transition-trace, event-to-step entries, and member counts; the node exposes no HTTP payload route; `demo/midgard-node/src/da/libp2p-producer.ts` serves `payload-by-header` and `payload-chunk` requests over libp2p, and DA committee nodes validate and exchange the payload over the same protocols.
...
- [ ] Publish proof-bundle retrieval APIs beyond `/block`, `/tx`, and raw `DaPayload`.
