...
nonMembershipTests =
    testGroup
        ...
        [ testCase "mpf_proof_v1_verifies_empty_trie_non_membership" $
            holds $
                pdoesNotHave # Env.pemptyMerkleTreeRoot # pconstant key # emptyProof
...
rootTransitionTests =
    testGroup
        ...
        [ testCase "mpf_proof_v1_total_root_mutations_do_not_call_partial_paths_first" $
            holds $
                pall'
                    [ (pinsertRoot # Env.pemptyMerkleTreeRoot # pconstant key # pconstant value # emptyProof)
                    ...
                    , pisNothing (pupdateRoot # Env.pemptyMerkleTreeRoot # pconstant key # pconstant value # pconstant updatedValue # emptyProof)
                    , (pdeleteRoot # pconstant singletonRoot # pconstant key # pconstant value # emptyProof)
                        #== pjust pnull_hash
                    , pisNothing (pdeleteRoot # Env.pemptyMerkleTreeRoot # pconstant key # pconstant value # emptyProof)
                    ]
        , -- The partial library functions abort on a bad proof; these must not.
          testCase "a malformed proof yields Nothing rather than aborting" $
            holds $
                pall'
                    [ pisNothing (pinsertRoot # Env.pemptyMerkleTreeRoot # pconstant key # pconstant value # malformedFork)
                    , pisNothing (pupdateRoot # Env.pemptyMerkleTreeRoot # pconstant key # pconstant value # pconstant updatedValue # malformedFork)
                    , pisNothing (pdeleteRoot # Env.pemptyMerkleTreeRoot # pconstant key # pconstant value # malformedFork)
                    ]
        ]
