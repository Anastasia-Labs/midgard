...
import Midgard.CekData (PDataSummaryV1 (..), pemptyDataPairSummaryV1, psemanticDataSummaryV1)
...
tests =
  testGroup
    ...
    , testCase "proves lexicographic order across canonical frontier indices" $ passertEvalNoTrace (permutedValue 0)
    , testCase "refuses missing within-policy head opening" $ passertEvalNoTrace (permutedValue 1)
    , testCase "refuses substituted previous-head quantity" $ passertEvalNoTrace (permutedValue 2)
    , testCase "refuses a substituted authenticated asset index" $ passertEvalNoTrace (permutedValue 3)
    , testCase "refuses a repeated authenticated asset" $ passertEvalNoTrace (permutedValue 4)
    , testCase "refuses ascending lexicographic execution" $ passertEvalNoTrace (permutedValue 5)
...
foldsExactValue =
  plet assetPeaks $ \peaks ->
    plet (pinitialControlV1 # 3) $ \c0 ->
      plet (expectJust $ pstepV1 # c0 # 3 # peaks # 8_000_000 # assetWitness 2 34 (bytes "bbcc") pmaxUint64 pnil (pcon PDNothing)) $ \c1 ->
        plet (expectJust $ pstepV1 # c1 # 3 # peaks # 8_000_000 # assetWitness 1 17 (bytes "aa") 42 (plist [pdata firstLeaf]) (pcon PDNothing)) $ \c2 ->
          plet (expectJust $ pstepV1 # c2 # 3 # peaks # 8_000_000 # assetWitness 0 17 (pconstant "") 1 (plist [pdata secondLeaf]) (headOpening (bytes "aa") 42)) $ \c3 ->
            plet (expectJust $ pstepV1 # c3 # 3 # peaks # 8_000_000 # pcon PLedgerOutputValueNoWitness) $ \c4 ->
              plet (expectJust $ pstepV1 # c4 # 3 # peaks # 8_000_000 # pcon PLedgerOutputValueNoWitness) $ \terminal ->
                pfinalizeV1
                  # terminal
                  #== pcon (PJust $ psemanticDataSummaryV1 # pconstant expectedValueData)
                  #&& pencodeControlV1
                  # terminal
                  #== terminalControl
...
terminalControlDecodes = plet (pdecodeControlV1 # terminalControl) $ \control -> pmatch control $ \c ->
  pfromData (pvalueControl'stage c)
    #== pstageTerminal
    #&& pencodeControlV1
    # control
    #== terminalControl
        ...
        plet (expectJust $ pfinalizeV1 # terminal) $ \result ->
          pmatch pre $ \preFields -> pmatch result $ \summary ->
            pfrontierIsWellFormed
              # 1_592
              # maximumPeaks
              #&& pfromData (pvalueControl'stage preFields)
              #== pstageFinalize
              #&& pfromData (pvalueControl'assetRemaining preFields)
              #== 0
              #&& pfromData (pvalueControl'currentPolicy preFields)
              #== policy 17
              #&& terminal
              #== decoded
              #&& pencodeControlV1
              # terminal
              #== maximumTerminalControl
              #&& pfromData (psummary'root summary)
              #== maximumRoot
              #&& pfromData (psummary'cborLength summary)
              #== 5_002
              #&& pfromData (psummary'memory summary)
              #== 16_198
              #&& pfinalizeV1
              # pre
              #== pcon PNothing
              #&& pnot
              # (pstepV1 # pre # 1_592 # maximumPeaks # 30_000_001 # pcon PLedgerOutputValueNoWitness #== pcon (PJust decoded))
...
substitutedFactsFail =
  pstepV1
    ...
    # assetPeaks
    # 8_000_000
    # assetWitness 2 34 (bytes "bbcc") (pmaxUint64 - 1) pnil (pcon PDNothing)
    #== pcon PNothing
    ...
    plet (expectJust $ pstepV1 # pre # 1_592 # maximumPeaks # 30_000_000 # pcon PLedgerOutputValueNoWitness) $ \post ->
      pmatch pre $ \preFields -> pmatch post $ \postFields ->
        pfromData (pvalueControl'stage preFields)
          ...
          #&& pfromData (pvalueControl'assetRemaining preFields)
          #== 0
          #&& pfromData (pvalueControl'stage postFields)
          #== pstageTerminal
          #&& pencodeControlV1
          # post
          #== maximumTerminalControl
          #&& pnot
          # (pfinalizeV1 # post #== pcon PNothing)
  ...
  plet (pdecodeControlV1 # policyTransitionPreControl) $ \pre ->
    plet (expectJust $ pstepV1 # pre # 1_592 # maximumPeaks # 30_000_000 # maximumPolicyWitness 1) $ \post ->
      pmatch pre $ \preFields -> pmatch post $ \postFields ->
        pfromData (pvalueControl'assetRemaining preFields)
          #== 228
          #&& pfromData (pvalueControl'assetRemaining postFields)
          #== 227
          #&& pfromData (pvalueControl'currentPolicy postFields)
          #== policy 17
          #&& pencodeControlV1
          # post
          #== policyTransitionPostControl
          #&& pstepV1
          # pre
          # 1_592
          # maximumPeaks
          # 30_000_000
          # maximumPolicyWitness 2
          #== pcon PNothing

assetWitness :: forall s. Term s PInteger -> Term s PInteger -> Term s PByteString -> Term s PInteger -> Term s (PBuiltinList (PAsData PByteString)) -> Term s (PMaybeData PLedgerOutputValueHeadV1) -> Term s PLedgerOutputValueWitnessV1
assetWitness index policyByte assetName quantity siblings previous =
  pcon $
    PLedgerOutputValueAsset
      (pdata index)
      (pdata $ policy policyByte)
      (pdata assetName)
      (pdata quantity)
      (pdata siblings)
      previous

headOpening :: forall s. Term s PByteString -> Term s PInteger -> Term s (PMaybeData PLedgerOutputValueHeadV1)
headOpening name quantity = pcon $ PDJust $ pdata $ pcon $ PLedgerOutputValueHeadV1 (pdata name) (pdata quantity) (pdata pemptyDataPairSummaryV1)

maximumPolicyWitness :: forall s. Term s PInteger -> Term s PLedgerOutputValueWitnessV1
maximumPolicyWitness quantity =
  assetWitness
    227
    17
    (bytes "e2")
...
maximumPeaks =
 ...
 where
  peak height hashValue = pdata $ pcon $ PFrontierPeak (pdata height) (pdata hashValue)
...
-- Native CBOR orders the one-byte ff before the two-byte 0000. The script
-- context sorts lexicographically, so its descending fold opens indices 0, 1.
permutedValue :: forall s. Integer -> Term s PBool
permutedValue mutation =
  plet (passetLeafHash # policy 17 # bytes "ff" # 1) $ \first ->
    plet (passetLeafHash # policy 17 # bytes "0000" # 2) $ \second ->
      plet (pappendLeaf # 1 # (pappendLeaf # 0 # pnil # first) # second) $ \peaks ->
        let start = pinitialControlV1 # 2
            high = assetWitness 0 17 (bytes "ff") 1 (plist [pdata second]) (pcon PDNothing)
            low = assetWitness 1 17 (bytes "0000") 2 (plist [pdata first]) (pcon PDNothing)
         in plet (expectJust $ pstepV1 # start # 2 # peaks # 0 # (if mutation == 5 then low else high)) $ \c1 ->
              let witness = case mutation of
                    1 -> low
                    2 -> assetWitness 1 17 (bytes "0000") 2 (plist [pdata first]) (headOpening (bytes "ff") 9)
                    3 -> assetWitness 0 17 (bytes "0000") 2 (plist [pdata first]) (headOpening (bytes "ff") 1)
                    4 -> assetWitness 0 17 (bytes "ff") 1 (plist [pdata second]) (headOpening (bytes "ff") 1)
                    5 -> assetWitness 0 17 (bytes "ff") 1 (plist [pdata second]) (headOpening (bytes "0000") 2)
                    _ -> assetWitness 1 17 (bytes "0000") 2 (plist [pdata first]) (headOpening (bytes "ff") 1)
                  result = pstepV1 # c1 # 2 # peaks # 0 # witness
               in if mutation /= 0
                    then result #== pcon PNothing
                    else plet (expectJust result) $ \c2 ->
                      plet (expectJust $ pstepV1 # c2 # 2 # peaks # 0 # pcon PLedgerOutputValueNoWitness) $ \c3 ->
                        plet (expectJust $ pstepV1 # c3 # 2 # peaks # 0 # pcon PLedgerOutputValueNoWitness) $ \c4 ->
                          pfinalizeV1
                            # c4
                            #== pcon
                              ( PJust $
                                  psemanticDataSummaryV1
                                    # pconstant
                                      (Map [(B (BS.replicate 28 17), Map [(B (Base16.decodeLenient "0000"), I 2), (B (Base16.decodeLenient "ff"), I 1)])])
                              )
