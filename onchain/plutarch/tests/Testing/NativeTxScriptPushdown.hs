...
tests =
  testGroup
    "Midgard.NativeTxScriptPushdown"
    [ testGroup
        "authenticated signer predicate"
        [ testCase "matches materialised signers" $ passertEvalNoTrace authenticatedSignerPredicateMatches
        , testCase "preserves an unsatisfied verdict" $ passertEvalNoTrace authenticatedSignerPredicateUnsatisfied
        ]
    , testGroup
        "traversal"
...
verdictCase name script expected =
  testCase name $
    passertEvalNoTrace $
...
pauthenticatedRun :: forall s. BS.ByteString -> (Term s PByteString -> Term s PBool) -> Term s PNativeScriptWalkV1
pauthenticatedRun script signerIsPresent =
  pnativeScriptRunWithSignerVerdict
    ...
    # 300
    # 16
    # plam signerIsPresent

authenticatedSignerPredicateMatches :: forall s. Term s PBool
authenticatedSignerPredicateMatches =
  let script = anyOf [signature key1, signature key3]
   in pmatch pcontext $ \ctx ->
        pmatch (pnativeScriptVerdict # prun script 16) $ \case
          PNothing -> pconstant False
          PJust materialised ->
            pmatch
              ( pnativeScriptVerdict
                  # pauthenticatedRun script (\keyHash -> pelem # pdata keyHash # pfromData (pnativeContext'signers ctx))
              )
              $ \case
                PNothing -> pconstant False
                PJust authenticated -> authenticated #== materialised

authenticatedSignerPredicateUnsatisfied :: forall s. Term s PBool
authenticatedSignerPredicateUnsatisfied =
  let script = allOf [signature key2, signature key3]
   in pmatch (pnativeScriptVerdict # pauthenticatedRun script (\keyHash -> keyHash #== pconstant key1)) $ \case
        PNothing -> pconstant False
...
exactStepCount =
  pnativeScriptWalkIsComplete
    # prun nestedScript 15
    #&& pnot
    # (pnativeScriptWalkIsComplete # prun nestedScript 14)
   ...
   in plet (pstopped script 2) $ \stopped ->
        pmatch (pnativeScriptWalkFrames # stopped) $ \case
          ...
          PCons topData _ -> pmatch (pfromData topData) $ \top ->
            pfromData (pnativeFrame'required top)
              #== 33
              #&& plet
                (presume stopped script)
                ( \resumed ->
        ...
        pmatch (pnativeScriptVerdict # finished) $ \case
          ...
          PJust verdict ->
            verdict
              ...
              #== 8
              #&& pnot
              # (pnativeScriptWalkIsComplete # starved)
  ...
  pfix
    ( \self -> plam $ \budget ->
        pif (budget #> 12) (pconstant True)
          ...
          $ \stopped ->
            pif
              (pnativeScriptWalkIsComplete # stopped)
...
cursorCborShape = plet (pencodeNativeScriptCursor # pstopped nestedScript 5) $ \cursor ->
  plengthBS
    ...
    # 3
    # cursor
    #== pconstant (hexBytes [0x87, 0x58, 0x20])
    #&& psliceBS
    # 35
    # 2
    # cursor
    #== pconstant (hexBytes [0x58, 0x20])
    #&& psliceBS
    # 69
    # 1
    # cursor
    #== pconstant (hexBytes [0x43])
    #&& psliceBS
    # 73
    # 1
    # cursor
...
depthBoundaryFixtures =
  pinspectedShape (nestedAlls 14 $ signature key1) 15 15
    ...
    #&& pnot
    # pisChecked (nestedAlls 16 $ signature key1)
...
pinspectedShape script expectedDepth expectedNodes = pmatch
  (pinspectNativeScriptV1 # pconstant script # 200 # 300 # (pcons # pconstant key1 # pnil))
  $ \case
    PNothing -> pconstant False
    PJust proof -> pmatch proof $ \checked ->
      pfromData (pnativeProof'depth checked)
        #== pconstant expectedDepth
        #&& pfromData (pnativeProof'nodeCount checked)
        #== pconstant expectedNodes
...
atLeast required children =
  hexBytes [0x83, 0x03]
    <> uintBytes required
    <> BS.singleton (0x80 + fromIntegral (length children))
    <> BS.concat children
