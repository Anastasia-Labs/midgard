...
inlineFieldCarriage items =
  pcon $ NativeField.PInline (pdata $ NativeField.pencodeFieldPreimage # items)
...
tests =
  testGroup
    ...
    , testCase "minimum Ada absolute vectors match the TypeScript twin" $
        passertEvalNoTrace minAdaAbsoluteVectors
    ...
    , testGroup
        "validation-machine-v1 maximum field-terminal TypeScript vectors"
        [ maximumFieldTerminalCase "maximum_spend_input_field_terminal_matches_typescript" "maximum-spend-inputs"
        , maximumFieldTerminalCase "maximum_reference_input_field_terminal_matches_typescript" "maximum-reference-inputs"
        , maximumFieldTerminalCase "maximum_observer_field_terminal_matches_typescript" "maximum-observers"
        , maximumFieldTerminalCase "maximum_output_field_terminal_matches_typescript" "maximum-outputs"
        , maximumFieldTerminalCase "maximum_required_signer_field_terminal_matches_typescript" "maximum-required-signers"
        , maximumFieldTerminalCase "maximum_mint_field_terminal_matches_typescript" "maximum-mint"
        ]
    ...
    , testCase "CEK aggregate and split resolvers authenticate a native-only execution selection" $
        passertEvalNoTraceWithoutHoistChecks cekNativeOnlyExecutionSelectionTransition
    , testGroup
        "validation-machine-v1 CEK context ABI and selection parity"
    ...
    , testCase "Value-and-Mint stage 3 admits the exact minimum-Ada floor" $
        passertEvalNoTrace (valueAndMintMinAdaBoundary False)
    , testCase "Value-and-Mint stage 3 rejects one lovelace below minimum Ada" $
        passertEvalNoTrace (valueAndMintMinAdaBoundary True)
    ...
    , testCase "ScriptSourcesStageFour rejects a forged output total count" $
        passertEvalNoTrace (scriptSourcesStageFourForgedOutputTransition False)
    , testCase "ScriptSourcesStageFour rejects a carried output total count the field denies" $
        passertEvalNoTrace scriptSourcesStageFourDeniedOutputTotalCount
    ...
    , testCase "ScriptSourcesStageSix mint begin authenticates the flat field preimage" $
        passertEvalNoTrace scriptSourcesStageSixBeginTransition
    ...
    , testCase "ScriptSourcesOutputProofFinalize rejects foreign raw network nibble 2" $
        passertEvalNoTrace (scriptSourcesOutputProofFinalizeWrongNetwork 0x62)
    , testCase "ScriptSourcesOutputProofFinalize rejects protected foreign raw network nibble 15" $
        passertEvalNoTrace (scriptSourcesOutputProofFinalizeWrongNetwork 0x6f)
    ...
    , testGroup
        "validation-machine-v1 CEK maximum mint and one-step context fixtures"
                  ...
                  plet
                    ( pcon $
                        PValueInputAssetWitness
                          ...
                          (pdata $ cekHash 0x41)
                          (pdata $ phexByteStr "80")
                          (pdata 0)
                          (pdata $ preplicateBS # 28 # (pintegerToByte # 0x42))
                          (pdata $ phexByteStr "abcd")
                          (pdata 5)
                          (pdata pnil)
                          (pdata pnil)
                          (pdata mutation)
                                      ...
                                      plet
                                        ( pcon $
                                            PLedgerDeltaProofFrameWitness
                                              (pdata proofFrame)
                                              (pdata $ pcons # pdata (cekHash 0x47) # pnil)
                                        )
                                        $ \frame ->
                                          plet
                                            ( pcon $
                                                ...
                                                $ \descriptor ->
                                                  plet
                                                    ...
                                                    $ \mintFold ->
                                                      plet
                                                        ( pcons
                                                            # pforgetData (pdata inputAsset)
                                                            #$ pcons
                                                            # pforgetData (pdata outputAsset)
                                                            #$ pcons
                                                            # pforgetData (pdata mintAsset)
                                                            #$ pcons
                                                            # pforgetData (pdata replay)
                                                            #$ pcons
                                                            # pforgetData (pdata output)
                                                            #$ pcons
                                                            # pforgetData (pdata frame)
                                                            #$ pcons
                                                            # pforgetData (pdata operation)
                                                            #$ pcons
                                                            # pforgetData (pdata descriptor)
                                                            #$ pcons
                                                            # pforgetData (pdata mintFold)
                                                            # pnil
                                                        )
                                                        $ \items ->
                                                          pand'List
                                                            ...
                                                            , pblake2b_256
                                                                # (pserialiseData # (plistData # items))
      ...
      $ \control ->
        plet (pencodeValueAndMintControlV1 # control) $ \controlCbor ->
          pand'List
            ...
            , valueAccumulatorUpdateConstructorIs
                0
                1
...
pvalidationAuxiliaryCorpusIsExact ::
  forall s.
...
resolvedInputAccumulatorVector =
  plet pinitialResolutionAccumulator $ \initial ->
    initial
      #== phexByteStr "07eb401e2f7e5de17444414ec48a5d9dca455dea72f4675cc2b08bf5b4e39979"
...
minAdaAbsoluteVectors :: forall s. Term s PBool
minAdaAbsoluteVectors =
  pand'List
    [ pminAdaLovelaceV1 # 4_310 # 0 #== 689_600
    , pminAdaLovelaceV1 # 4_310 # 41 #== 866_310
    , pminAdaLovelaceV1 # 4_310 # 196 #== 1_534_360
    , pminAdaLovelaceV1 # 1 # 0 #== 160
    , pminAdaLovelaceV1 # 1 # 41 #== 201
    , pminAdaLovelaceV1 # 5_000 # 196 #== 1_780_000
    , pminAdaLovelaceV1 # 4_310 # 5_000 #== 22_239_600
    ]
...
earlyPhaseWitnessEncodingVectors =
  pand'List
          ...
          plet (phexByteStr "5820" <> (preplicateBS # 32 # (pintegerToByte # 0xdd))) $ \suffix ->
            pand'List
              ...
              , pencodeResolveInputsWitness
                  ...
                  # (preplicateBS # 32 # (pintegerToByte # 0xdd))
                  #== prefix
                  <> (Codec.pencodeDefiniteBytes # activeCbor)
                  <> suffix
    ...
    PResolveInputOutputProofV1
      ...
      ( pdata $
          LedgerOutputProof.pinitialControlV1
            # 0
            # 1
            # (preplicateBS # 32 # (pintegerToByte # 0x11))
      )
...
malformedResolveInputsWitness =
  pencodeResolveInputsWitness
    ...
    # phexByteStr "04"
    # 0
    # (preplicateBS # 31 # (pintegerToByte # 0x41))
    # (preplicateBS # 32 # (pintegerToByte # 0x42))
    # 0
    # (preplicateBS # 32 # (pintegerToByte # 0x43))
    # pcon PDNothing
...
emptyLedgerDeltaPendingMutation =
  pcon $
    PLedgerDeltaPendingMutationV1
      (pdata 0)
      (pdata 1)
...
emptyLedgerDeltaControl pendingCbor =
  pcon $
    PLedgerDeltaControlV1
      (pdata 0)
      (pdata $ preplicateBS # 32 # (pintegerToByte # 0xaa))
        ...
        plet (preplicateBS # 32 # (pintegerToByte # 0xbb)) $ \secondHash ->
          pand'List
            ...
            , pencodeScriptIntegrityWitnessV1
                # control
                #== pconstant "\x82"
                <> (Codec.pencodeDefiniteBytes # controlCbor)
                <> phexByteStr "00"
...
nativeScriptsControlWithRedeemerCount ::
  forall s.
...
nativeScriptsControlWithCompactCbor ::
  forall s.
...
nativeScriptsControlWithResolvedItem ::
  forall s.
  Term s PByteString -> Term s PNativeScriptsControlV1
nativeScriptsControlWithResolvedItem leaf =
  pcon $
    PNativeScriptsControlV1
      (pdata $ phexByteStr "01")
      (pdata $ phexByteStr "02")
...
nativeScriptsControlWithMint ::
  forall s.
  Term s PInteger ->
  Term s (PBuiltinList (PAsData Merkle.PFrontierPeak)) ->
  Term s PNativeScriptsControlV1
nativeScriptsControlWithMint mintCount mintPeaks =
  pcon $
    PNativeScriptsControlV1
      (pdata $ phexByteStr "01")
      (pdata $ phexByteStr "02")
...
nativeScriptsControlForRedeemerSelection ::
  forall s.
  Term s PByteString -> Term s PByteString -> Term s PNativeScriptsControlV1
nativeScriptsControlForRedeemerSelection redeemerLeaf purposeLeaf =
  pcon $
    PNativeScriptsControlV1
      (pdata $ phexByteStr "01")
      (pdata $ phexByteStr "02")
...
nativeScriptsControlFixture ::
  forall s.
  Term s PByteString -> Term s PInteger -> Term s PInteger -> Term s PNativeScriptsControlV1
nativeScriptsControlFixture compactCbor languageBitmap redeemerCount =
  pcon $
    PNativeScriptsControlV1
      (pdata compactCbor)
      (pdata $ phexByteStr "02")
...
cekNativeOnlyExecutionSelectionTransition :: forall s. Term s PBool
cekNativeOnlyExecutionSelectionTransition =
  plet (phexByteStr "8200581c" <> preplicateBS # 28 # (pintegerToByte # 0xaa)) $ \scriptBytes ->
    plet
      ( pcon $
          PMidgardVersionedScript
      ...
      $ \script ->
        plet (pencodeMidgardVersionedScript # script) $ \scriptCbor ->
          pmatch (inputSetsSingletonProof 6 scriptCbor) $ \(PPair _ firstChunkProof) ->
                            ...
                            plet (cekSinglePeak purposeLeaf) $ \purposePeaks ->
                              plet (cekSinglePeak executionLeaf) $ \executionPeaks ->
                                pmatch (cekObserverProofSource NativeField.pemptyFieldCommitment) $ \(PPair base transactionId) ->
                                  plet
                                    (nativeScriptsExecutionControl base sourcePeaks purposePeaks executionPeaks 1 0)
                                    $ \nativeControl ->
                                                ...
                                                plet (phashValidationContext # pfromData (pnativeControl'contextCbor baseFields)) $ \contextHash ->
                                                  plet
                                                    ( cekBoundStateFixture
                                                        ...
                                                        (pcon PCek)
                                                        40
                                                        (phashWorkWitness # pcon PCek # 40 # workCbor)
                                                        0
                                                        0
                                                    )
                                                    $ \pre ->
                                                        ...
                                                        $ \post ->
                                                          plet (pcon $ PValidationOneStepWitnessV1 (pdata workCbor) (pdata post)) $ \witness ->
                                                            plet
                                                              ...
                                                              $ \auxiliary ->
                                                                plet
                                                                  (pcon $ PValidationOneStepEvidenceV1 (pdata witness) (pdata auxiliary))
                                                                  $ \evidence ->
                                                                    pand'List
                                                                      [ pverifyCekOneStepV1 # pre # evidence # inlineFieldDoor
                                                                      , CekSemantics.pverifyCekExecutionSelectionSemanticsV1
                                                                          # pre
                                                                          # witness
                                                                          # auxiliary
                                                                      , pnot
                                                                          # ( CekSemantics.pverifyCekFinishSemanticsV1
                                                                                # pre
                                                                                # witness
                                                                            )
                                                                      ]
  ...
  plet
    ...
    $ \oversized ->
      plengthBS
        ...
        #== 51
        #&& pmatch
          (CekProof.pinspectProgramEnvelopeV1 # oversized)
          ( \case
                                              ...
                                              Bounded.PChunkProofV1
                                                ...
                                                ( pdata $
                                                    pcons
                                                      # pdata hash1
                                                      #$ pcons
                                                      # pdata (Merkle.phashBranch # hash2 # hash3)
                                                      # pnil
                                                )
                                          )
                                          $ \firstChunkProof ->
                                                                                                      ...
                                                                                                      plet (pcon $ PValidationOneStepEvidenceV1 (pdata transition) (pdata auxiliary)) $ \evidence ->
                                                                                                        pand'List
                                                                                                          ...
                                                                                                          , pverifyCekExecutionSelection
                                                                                                              ...
                                                                                                              # (plengthBS # nativeScriptCbor)
                                                                                                              # nativeCommitment
                                                                                                              # (pcons # pdata effectfulSource # pnil)
                                                                                                              # pconstant ""
                                                                                                              # (pcons # pdata effectfulExecution # pnil)
                                                                                                              # firstChunkProof
                                                                                                          ]
...
nativeScriptsControlForExecutionSelection ::
  forall s.
  Term s PByteString ->
  Term s PByteString ->
  Term s PByteString ->
      ...
      $ \seedAuxiliary ->
        pand'List
          [ pcekContextAuxiliaryMatchesStage
              # (cekContextStageFixture 6 3 True)
              # noAuxiliary
          , pnot
              #$ pcekContextAuxiliaryMatchesStage
              # (cekContextStageFixture 6 3 True)
          ...
          , pnot
              #$ pcekContextAuxiliaryMatchesStage
              # (cekContextStageFixture 7 3 True)
              # noAuxiliary
          , pcekContextAuxiliaryMatchesStage
                ...
                plet (cekStateFixture (pcon PValueAndMint) 30 (cekHash 0xa1) 7 8) $ \preZero ->
                  plet
                    ( phashWorkWitness
                        # pcon PValueAndMint
                        # 31
                        # (pencodeValueAndMintControlV1 # stageOne)
                    )
                    $ \stageOneRoot ->
                      plet (cekStateFixture (pcon PValueAndMint) 31 stageOneRoot 7 8) $ \postZeroAndPreOne ->
                        plet
                          ( phashWorkWitness
                              # pcon PValueAndMint
                              # 32
                              # (pencodeValueAndMintControlV1 # stageTwo)
                          )
                          $ \stageTwoRoot ->
                                                                                ...
                                                                                plet
                                                                                  ...
                                                                                  $ \assetAuxiliary ->
                                                                                    pvalueAndMintStageTwo
                                                                                      ...
                                                                                      # beforeReplay
                                                                                      #&& pvalueAndMintStageTwo
                                                                                      # postReplayAndPreAsset
                                                                                      # assetWitness
                                                                                      # assetAuxiliary
                                                                                      # beforeAsset
                                      _ -> pconstant False

valueDescriptorForAsset ::
  forall s.
  Term s PByteString ->
  Term s PByteString ->
  Term s PInteger ->
              ...
              pmatch (ScriptContext.pspendDatumSummaryV1 # outputCbor) $ \case
                ...
                PJust spend ->
                  OutputCommitment.pencodeLedgerOutputCommitment
                    # ( pcon $
                          OutputCommitment.PLedgerOutputCommitmentV1
                                      ...
                                      plet
                                        ...
                                        $ \finishWitness ->
                                          pvalueAndMintStageThreeDescriptor
                                            ...
                                            # postOutputAndPreFinish
                                            # finishWitness
                                            # pcon PNoAuxiliaryWitness
                                            # afterOutput

valueAndMintMinAdaBoundary :: forall s. Bool -> Term s PBool
valueAndMintMinAdaBoundary belowFloor =
  plet (plengthBS # valueOutputCbor) $ \outputLength ->
    plet (pminAdaLovelaceV1 # pcoinsPerUtxoByte # outputLength) $ \floor ->
      plet (pif (pconstant belowFloor) (floor - 1) floor) $ \lovelace ->
        plet (cekDescriptorForIndexAndLovelace 0 lovelace) $ \descriptorCbor ->
          plet (ScriptProof.poutputDescriptorLeafHash # 0 # descriptorCbor) $ \descriptorLeaf ->
            plet (cekSinglePeak descriptorLeaf) $ \descriptorPeaks ->
              plet (valueOutputNativeControl descriptorPeaks) $ \nativeControl ->
                plet (valueOutputControl nativeControl 3 0 0 (cekHash 0) pinitialValueAccumulator) $ \beforeOutput ->
                  plet (cekStateFixture (pcon PValueAndMint) 53 (cekHash 0xa1) 7 8) $ \preOutput ->
                    plet
                      ( pif
                          (pconstant belowFloor)
                          (inputSetsExactRejection preOutput $ pconstant "E_MIN_ADA")
                          ( plet
                              (pcon $ PValueAccumulatorV1 (pdata $ 0 - lovelace) (pdata pnull_hash) (pdata 0) (pdata 0))
                              $ \nextAccumulator ->
                                plet (valueOutputControl nativeControl 3 1 0 (cekHash 0) nextAccumulator) $ \afterOutput ->
                                  cekStateFixture
                                    (pcon PValueAndMint)
                                    54
                                    (phashWorkWitness # pcon PValueAndMint # 54 # (pencodeValueAndMintControlV1 # afterOutput))
                                    7
                                    8
                          )
                      )
                      $ \claimedSuccessor ->
                        plet
                          (pcon $ PValidationOneStepWitnessV1 (pdata $ pconstant "") (pdata claimedSuccessor))
                          $ \witness ->
                            plet
                              ( pvalueAndMintStageThreeDescriptor
                                  # preOutput
                              ...
                              $ \result ->
                                pif
                                  (pconstant belowFloor)
                                  ( plet (inputSetsExactRejection preOutput $ pconstant "E_MIN_FEE") $ \wrongSuccessor ->
                                      plet
                                        (pcon $ PValidationOneStepWitnessV1 (pdata $ pconstant "") (pdata wrongSuccessor))
                                        $ \wrongWitness ->
                                          result
                                            #&& pnot
                                            # ( pvalueAndMintStageThreeDescriptor
...
staticRulesClearedDeltaCannotReject =
  staticRulesTransition
    ...
    True
    staticRulesNonemptyClaimedDeltaRoot
    staticRulesEmptyClaimedDeltaRoot
    False
...
inputSetsSingletonCommitment ::
  forall s.
  Term s PInteger -> Term s PByteString -> Term s PByteString
inputSetsSingletonCommitment _fieldIndex key =
  NativeField.pfieldCommitmentFromItems # (pcons # key # pnil)
...
inputSetsItemEvidence witness fieldIndex itemIndex carriage =
  pcon $
    PValidationOneStepEvidenceV1
      (pdata witness)
      ( pdata $
                    ...
                    $ \wrongPhase ->
                      plet (pcon $ PValidationOneStepWitnessV1 (pdata $ inputSetsWitnessCbor control) (pdata wrongPhase)) $ \wrongWitness ->
                        pand'List
                          ...
                          , pverifyInputSetsOneStepV1 # pre # evidence # inlineFieldDoor
                      ...
                      plet (inputSetsStateFixture transactionId firstControl 2) $ \firstPre ->
                        plet (inputSetsStateFixture transactionId secondControl 3) $ \secondPre ->
                          plet (pcon $ PValidationOneStepWitnessV1 (pdata $ inputSetsWitnessCbor firstControl) (pdata secondPre)) $ \firstWitness ->
                            plet (inlineFieldCarriage $ pcons # referenceKey # pnil) $ \referenceCarriage ->
                              plet (inputSetsSignaturesPost secondPre secondControl fullSchedule) $ \signaturesPost ->
                                plet (pcon $ PValidationOneStepWitnessV1 (pdata $ inputSetsWitnessCbor secondControl) (pdata signaturesPost)) $ \secondWitness ->
                                  plet (inlineFieldCarriage $ pcons # spendKey # pnil) $ \spendCarriage ->
                                    pand'List
                                      ...
                                      , pverifyInputSetsOneStepV1
                                          # secondPre
                                          # inputSetsItemEvidence secondWitness 0 0 spendCarriage
                                          # inlineFieldDoor
              ...
              pmatch
                ...
                $ \(PPair secondControl _) ->
                  plet (inputSetsStateFixture transactionId firstControl 2) $ \firstPre ->
                    plet (inputSetsStateFixture transactionId secondControl 3) $ \secondPre ->
                      plet (pcon $ PValidationOneStepWitnessV1 (pdata $ inputSetsWitnessCbor firstControl) (pdata secondPre)) $ \firstWitness ->
                        plet (inlineFieldCarriage $ pcons # key # pnil) $ \referenceCarriage ->
                          plet (inputSetsExactRejection secondPre $ pconstant "E_DUPLICATE_INPUT_IN_TX") $ \rejection ->
                            plet (pcon $ PValidationOneStepWitnessV1 (pdata $ inputSetsWitnessCbor secondControl) (pdata rejection)) $ \secondWitness ->
                              plet (inlineFieldCarriage $ pcons # key # pnil) $ \spendCarriage ->
                                pverifyInputSetsItemSemanticsV1
                                  # firstPre
                                  # firstWitness
                                  # inlineFieldDoor
        ...
        $ \(PPair control transactionId) ->
          plet (inputSetsStateFixture transactionId control 2) $ \pre ->
            plet (inputSetsExactRejection pre $ pconstant "E_INVALID_VALIDITY_INTERVAL_FORMAT") $ \rejection ->
              plet (pcon $ PValidationOneStepWitnessV1 (pdata $ inputSetsWitnessCbor control) (pdata rejection)) $ \witness ->
                plet (inlineFieldCarriage $ pcons # key # pnil) $ \carriage ->
                  pverifyInputSetsItemSemanticsV1
                    # pre
                    # witness
                    # inlineFieldDoor
                                    ...
                                    plet
                                      ...
                                      $ \handoffEvidence ->
                                        pand'List
                                          ...
                                          , pverifySignaturesOneStepV1 # handoffPre # handoffEvidence # inlineFieldDoor
                  ...
                  plet (pcon $ PValidationOneStepWitnessV1 (pdata $ signaturesWitnessCbor addressControl) (pdata requiredPre)) $ \addressWitness ->
                    plet (inputSetsExactRejection requiredPre $ pconstant "E_MISSING_REQUIRED_WITNESS") $ \rejection ->
                      plet (pcon $ PValidationOneStepWitnessV1 (pdata $ signaturesWitnessCbor requiredControl) (pdata rejection)) $ \requiredWitness ->
                        plet (inlineFieldCarriage $ pcons # signerHash # pnil) $ \carriage ->
                  ...
                  pmatch
                    ...
                    $ \(PPair requiredControl _) ->
                      plet (signaturesStateFixture transactionId addressControl 4) $ \addressPre ->
                        plet (signaturesStateFixture transactionId requiredControl 5) $ \requiredPre ->
                          plet (pcon $ PValidationOneStepWitnessV1 (pdata $ signaturesWitnessCbor addressControl) (pdata requiredPre)) $ \addressWitness ->
                            plet (inlineFieldCarriage $ pcons # addressWitnessCbor # pnil) $ \carriage ->
...
phaseANativeControlFixtureWithOutputsAndObservers
  ...
  nodeCount
  result =
    phaseANativeControlFixtureWithMint
      scriptCommitment
      redeemerCommitment
      outputsCommitment
      observerCommitment
                          ...
                          plet (phaseANativeStateFixture transactionId itemControl 8) $ \itemPre ->
                            plet (phaseANativeStateFixture transactionId tokenControl 9) $ \tokenPre ->
                              plet (pcon $ PValidationOneStepWitnessV1 (pdata $ phaseANativeWitnessCbor itemControl) (pdata tokenPre)) $ \itemWitness ->
                                plet (inlineFieldCarriage $ pcons # nativeItemCbor # pnil) $ \carriage ->
                                  ...
                                  plet
                                    ...
                                    $ \tokenWitness ->
                                      plet
                                        ...
                                        $ \finalizeWitness ->
                                          pmatch (inputSetsSingletonProof 6 itemCbor) $ \(PPair _collectionProof chunkProof) ->
                                            plet (inlineFieldCarriage $ pcons # itemCbor # pnil) $ \carriage ->
                                              pand'List
                                                [ pverifyPhaseANativeItemSemanticsV1
                                                    # itemPre
...
phaseANativeContainerPayloadTransitions =
  pand'List
          ...
          $ \(PPair headControl transactionId) ->
            pmatch
              ...
              $ \(PPair payloadControl _) ->
                plet
                  ( pcon $
                      NativeScriptScan.PNativeScriptTokenV1
                        (pdata NativeScriptScan.pallNode)
                        (pdata 6)
                        (pdata 1)
                        (pdata 0)
                        (pdata $ pconstant "")
              ...
              plet (phaseANativeStateFixture transactionId resultControl 29) $ \pre ->
                plet (inputSetsExactRejection pre $ pconstant "E_NATIVE_SCRIPT_INVALID") $ \post ->
                  plet
                    ( pcon $
                        PValidationOneStepWitnessV1
                          (pdata $ phaseANativeWitnessCbor resultControl)
                          (pdata post)
                    )
                    $ \witness ->
                            ...
                            plet (phaseANativeStateFixture transactionId childResultControl 30) $ \pre ->
                              plet (phaseANativeStateFixture transactionId reducedControl 31) $ \post ->
                                plet
                                  ( pcon $
                                      PValidationOneStepWitnessV1
                                        (pdata $ phaseANativeWitnessCbor childResultControl)
                                        (pdata post)
                                  )
                                  $ \witness ->
...
phaseAScriptPreconditionsStateFixture ::
  forall s.
  Term s PByteString ->
...
phaseAScriptPreconditionsResolvePost ::
  forall s.
  Term s PValidationMachineStateV1 ->
  Term s PPhaseAScriptPreconditionsControlV1 ->
  Term s PValidationMachineStateV1
                                              ...
                                              $ \invalidEvidence ->
                                                pand'List
                                                  ...
                                                  , pphaseAScriptPreconditionsFinalize
                                                      ...
                                                      # pconstant False
                                                      # 0
                                                  , pverifyPhaseAScriptPreconditionsOneStepV1 # emptyPre # emptyEvidence # inlineFieldDoor
                                                  , pverifyPhaseAScriptPreconditionsSemanticsV1 # invalidPre # invalidEvidence # inlineFieldDoor
                                                  ]
                                    ...
                                    plet
                                      ...
                                      $ \itemEvidence ->
                                        plet
                                          ( pcon $
                                              PValidationOneStepEvidenceV1
                                                (pdata finalizeWitness)
                                                (pdata $ pcon PNoAuxiliaryWitness)
                                          )
                                          $ \finalizeEvidence ->
                                            pand'List
                                              [ pverifyPhaseAScriptPreconditionsOneStepV1 # initialPre # itemEvidence # inlineFieldDoor
                                              , pverifyPhaseAScriptPreconditionsSemanticsV1 # completePre # finalizeEvidence # inlineFieldDoor
                                              ]
                  ...
                  plet (phaseAScriptPreconditionsStateFixture transactionId secondControl 39) $ \secondPre ->
                    plet (inputSetsExactRejection secondPre $ pconstant "E_INVALID_FIELD_TYPE") $ \rejection ->
                      plet
                        ( pcon $
                            PValidationOneStepWitnessV1
                              (pdata $ phaseAScriptPreconditionsWitnessCbor initialControl)
                              (pdata secondPre)
                        )
                        $ \firstWitness ->
                          plet
                            ( pcon $
                                PValidationOneStepWitnessV1
                                  (pdata $ phaseAScriptPreconditionsWitnessCbor secondControl)
                                  (pdata rejection)
                            )
                            $ \secondWitness ->
                              plet
                                (inlineFieldCarriage $ pcons # observerHash #$ pcons # observerHash # pnil)
                                $ \carriage ->
                                  plet
                                    ( pcon $
                                        PValidationOneStepEvidenceV1
...
phaseAScriptPreconditionsDuplicateObserverCommitment ::
  forall s.
  Term s PByteString -> Term s PByteString
phaseAScriptPreconditionsDuplicateObserverCommitment observerHash =
  NativeField.pfieldCommitmentFromItems
    # (pcons # observerHash #$ pcons # observerHash # pnil)
...
resolveInputsInitialControlFixture ::
  forall s.
...
resolveInputsScriptSourcesCbor ::
  forall s.
...
resolveInputsScriptSourcesCborAtStage ::
  forall s.
...
scriptSourcesStageZeroCborFromPhase ::
  forall s.
...
scriptSourcesCborFromPhaseAtStageWithRedeemers
  ...
  redeemerCount
  redeemerPeaks
  redeemerTotalCount = pmatch control $ \c ->
    pencodeScriptSourcesWitness
      # pfromData (pphaseANative'compactCbor c)
      # pfromData (pphaseANative'witnessSetCompactCbor c)
      # pfromData (pphaseANative'fieldPreimageLengthsCbor c)
...
scriptSourcesReplayInitializationCbor
  ...
  resolutionScheduleHash = pmatch control $ \c ->
    pencodeScriptSourcesWitness
      # pfromData (pphaseANative'compactCbor c)
      # pfromData (pphaseANative'witnessSetCompactCbor c)
      # pfromData (pphaseANative'fieldPreimageLengthsCbor c)
...
scriptSourcesReplayDescriptor
  ...
  referenceScriptItemCommitment =
    scriptSourcesReplayDescriptorForOutput
      2
      (phexByteStr "a200581d68aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa01821a0012d687a1581c11111111111111111111111111111111111111111111111111111111a142223307")
      address
      1_234_567
                ...
                OutputCommitment.pencodeLedgerOutputCommitment
                  # ( pcon $
                        OutputCommitment.PLedgerOutputCommitmentV1
                          ...
                          (pdata outputIndex)
                          (pdata $ plengthBS # outputCbor)
                          (pdata $ Bounded.pfromBytes # 2 # outputIndex # outputCbor)
                          (pdata address)
                          (pdata lovelace)
                          (pdata 0)
                          (pdata $ Merkle.pfrontierCommitment # 0 # pnil)
                                ...
                                plet (scriptSourcesStateFromPhase transactionId phaseControl 66 workCbor) $ \pre ->
                                  plet (scriptSourcesStateFromPhase transactionId phaseControl 67 nextWorkCbor) $ \post ->
                                    plet
                                      ...
                                      $ \witness ->
                                        plet (inlineFieldCarriage $ pcons # redeemerCbor # pnil) $ \carriage ->
                                          pstructuralTransitionIsValid
                                            # pre
                                            # witness
                                ...
                                plet
                                  ...
                                  $ \workCbor ->
                                    plet
                                      ( pencodeScriptSourcesRedeemerItemWitness
                                          # baseControl
                                          # (RedeemerItemProof.phashControlV1 # nextItemControl)
                                      )
                                      $ \nextWorkCbor ->
                                ...
                                plet
                                  ...
                                  $ \wrongWitness ->
                                    pstructuralTransitionIsValid
                                      ...
                                      # witness
                                      #&& pnot
                                      # (pverifyScriptSourcesStageThreeFinishSemanticsV1 # pre # wrongWitness)

scriptSourcesStageThreeReferenceReplayTransition :: forall s. Term s PBool
scriptSourcesStageThreeReferenceReplayTransition =
  plet (preplicateBS # 28 # (pintegerToByte # 0x41)) $ \scriptCredential ->
    plet (CekProof.pencodeProgramEnvelopeV1 # 1 # 1 # 0 # cekHash 0xaa # 3 # 144) $ \programEnvelope ->
      plet
        ( pcon $
            PMidgardVersionedScript
                    ...
                    ( pdata $
                        pcon $
                          PMidgardAddress
                            (pdata $ pconstant False)
                            (pdata 0)
                            (pdata $ pcon $ PMidgardScriptCredential $ pdata scriptCredential)
                            (pdata $ pcon PDNothing)
                    )
                    (pdata $ pcon $ PMidgardValue (pdata 0) (pdata $ pcon $ PAssocMap pnil))
                    (pdata $ pcon PDNothing)
                    (pdata $ pcon $ PDJust $ pdata script)
              )
              $ \output ->
                plet (pencodeMidgardTxOutput # output) $ \outputCbor ->
                  plet (preplicateBS # 1 # (pintegerToByte # 0x70) <> scriptCredential) $ \address ->
                    plet (ScriptProof.pversionedScriptHash # script) $ \scriptHash ->
                      plet (Bounded.pfromBytes # 2 # 0 # scriptCbor) $ \scriptCommitment ->
                        plet
                          ( scriptSourcesReplayDescriptorForOutput
                              0
                              outputCbor
                                        ...
                                        plet
                                          ...
                                          $ \resolvedLeaf ->
                                            plet (cekSinglePeak resolvedLeaf) $ \resolvedPeaks ->
                                              plet
                                                (ScriptProof.preferenceSourceLeafHash # key # 3 # scriptHash # (plengthBS # scriptCbor) # scriptCommitment)
                                                $ \sourceLeaf ->
                                                  plet (cekSinglePeak sourceLeaf) $ \sourcePeaks ->
                                                    pmatch
                                                      ( phaseANativeControlFixture
                                ...
                                plet
                                  ...
                                  $ \appendWitness ->
                                    plet
                                      ...
                                      $ \finishWitness ->
                                        plet (inlineFieldCarriage $ pcons # outputCbor # pnil) $ \carriage ->
                                          plet
                                            (pcon $ PTransactionRedeemerItemBeginWitness $ pdata carriage)
                                            $ \appendAuxiliary ->
                                              pstructuralTransitionIsValid
                                                # initialPre
                                                # appendWitness
...
scriptSourcesStageFourForgedOutputTransition forgeCommitment =
  plet (phexByteStr "01") $ \outputCbor ->
    plet (inputSetsSingletonCommitment 2 outputCbor) $ \outputsCommitment ->
      plet (Bounded.pfromBytes # 2 # 0 # outputCbor) $ \itemCommitment ->
        plet
          ( ScriptProof.poutputItemLeafHash
              # 0
              # (if forgeCommitment then cekHash 0xff else itemCommitment)
          )
          $ \claimedOutputLeaf ->
            plet (cekSinglePeak claimedOutputLeaf) $ \claimedOutputPeaks ->
              plet (if forgeCommitment then 1 else 2) $ \claimedOutputTotalCount ->
                pmatch
                  ( phaseANativeControlFixtureWithOutputs
                      NativeField.pemptyFieldCommitment
                      NativeField.pemptyFieldCommitment
                  ...
                  $ \(PPair phaseControl transactionId) ->
                    plet (scriptSourcesStageFourCbor phaseControl 4 0 pnil 0) $ \workCbor ->
                      plet
                        (scriptSourcesStageFourCbor phaseControl 4 1 claimedOutputPeaks claimedOutputTotalCount)
                        $ \nextWorkCbor ->
                          plet (scriptSourcesStateFromPhase transactionId phaseControl 83 workCbor) $ \pre ->
                            plet (scriptSourcesStateFromPhase transactionId phaseControl 84 nextWorkCbor) $ \post ->
                              plet (pcon $ PValidationOneStepWitnessV1 (pdata workCbor) (pdata post)) $ \witness ->
                                plet (inlineFieldCarriage $ pcons # outputCbor # pnil) $ \carriage ->
                                  plet (pcon $ PTransactionRedeemerItemBeginWitness $ pdata carriage) $ \auxiliary ->
                                    pnot # (pverifyScriptSourcesStageFourSemanticsV1 # pre # witness # auxiliary # inlineFieldDoor)

scriptSourcesStageFourDeniedOutputTotalCount :: forall s. Term s PBool
scriptSourcesStageFourDeniedOutputTotalCount =
  plet (phexByteStr "01") $ \outputCbor ->
    plet (inputSetsSingletonCommitment 2 outputCbor) $ \outputsCommitment ->
      plet (Bounded.pfromBytes # 2 # 0 # outputCbor) $ \itemCommitment ->
        plet (ScriptProof.poutputItemLeafHash # 0 # itemCommitment) $ \outputLeaf ->
          plet (cekSinglePeak outputLeaf) $ \outputPeaks ->
            pmatch
              ( phaseANativeControlFixtureWithOutputs
                  NativeField.pemptyFieldCommitment
              ...
              $ \(PPair phaseControl transactionId) ->
                plet (scriptSourcesStageFourCbor phaseControl 4 0 pnil 2) $ \workCbor ->
                  plet (scriptSourcesStageFourCbor phaseControl 4 1 outputPeaks 2) $ \nextWorkCbor ->
                    ...
                    plet
                      ...
                      $ \wrongWitness ->
                        pstructuralTransitionIsValid
                          ...
                          # witness
                          #&& pnot
                          # (pverifyScriptSourcesStageFiveFinishSemanticsV1 # pre # wrongWitness)
            ...
            plet
              ( pcon $
                  PLedgerOutputProofFinalizeWitness
                    (pdata resolveInputsNoReferenceDescriptor)
                    (pdata $ pcon PNoSignerSetProof)
              )
              $ \finalizeWitness ->
                plet
                    ...
                    plet (pdata resolveInputsNoReferenceTerminal) $ \terminalProof ->
                      pand'List
                        ...
                        , pscriptSourcesStageFiveBranchV1
                            # 0
                            # 1
                            # pcon (PDJust activeProof)
                            # stepWitness
                            #== 3
                        , pscriptSourcesStageFiveBranchV1
                            # 0
                            # 1
                            # pcon (PDJust terminalProof)
                            # finalizeWitness
                            #== 4
                        ]

scriptSourcesStageSixCbor ::
  forall s.
  Term s PPhaseANativeScriptsControlV1 ->
  Term s PInteger ->
  Term s PMintFoldControlV1 ->
  Term s PByteString
scriptSourcesStageSixCbor control stage mintFold =
  scriptSourcesStageSixMintCbor control stage 0 pnil mintFold

scriptSourcesStageSixMintCbor ::
  forall s.
  Term s PPhaseANativeScriptsControlV1 ->
  Term s PInteger ->
  Term s PInteger ->
...
scriptSourcesStageSixBeginTransition :: forall s. Term s PBool
scriptSourcesStageSixBeginTransition =
  plet
    ...
    $ \policyItemCbor ->
      plet (phexByteStr "11111111111111111111111111111111111111111111111111111111") $ \policyId ->
        plet (inputSetsSingletonCommitment 5 policyItemCbor) $ \mintCommitment ->
          plet
              ...
              plet
                ( pcon $
                    PMintFoldControlV1
                      ...
                      (pdata $ pconstant "")
                      (pdata policyId)
                      (pdata $ plengthBS # policyItemCbor)
                      (pdata $ Bounded.pfromBytes # 5 # 0 # policyItemCbor)
                      (pdata 32)
                      (pdata 1)
                      (pdata 0)
                      (pdata $ pconstant "")
                ...
                $ \activeFold ->
                  pmatch
                    ...
                    $ \(PPair phaseControl transactionId) ->
                      plet (scriptSourcesStageSixCbor phaseControl 6 pemptyMintFoldControl) $ \workCbor ->
                        plet (scriptSourcesStageSixMintCbor phaseControl 6 1 purposePeaks activeFold) $ \nextWorkCbor ->
                          plet (scriptSourcesStateFromPhase transactionId phaseControl 95 workCbor) $ \pre ->
                            plet (scriptSourcesStateFromPhase transactionId phaseControl 96 nextWorkCbor) $ \post ->
                              plet (pcon $ PValidationOneStepWitnessV1 (pdata workCbor) (pdata post)) $ \witness ->
                                plet (inlineFieldCarriage $ pcons # policyItemCbor # pnil) $ \carriage ->
                                  pstructuralTransitionIsValid
                                    # pre
                                    # witness
...
scriptSourcesDiscoveryStateCbor ::
  ...
  Term s PInteger ->
  Term s (PBuiltinList (PAsData Merkle.PFrontierPeak)) ->
  Term s PScriptDiscoveryControlV1 ->
  Term s PByteString
...
scriptSourcesSerialiseDataReencoding ::
  forall s.
  Term s PByteString -> Term s PByteString
                      ...
                      plet (scriptSourcesStateFromPhase transactionId phaseControl 20 nextWorkCbor) $ \post ->
                        plet (pcon $ PValidationOneStepWitnessV1 (pdata workCbor) (pdata post)) $ \witness ->
                          pverifyScriptSourcesStageEightFinishSemanticsV1
                            ...
                            # witness
                            #&& pnot
                            # (pverifyScriptSources # pre # witness # pcon PNoAuxiliaryWitness # inlineFieldDoor)
    ...
    plet (Bounded.phashChunk # 8 # 0 # 0 # redeemer) $ \chunkLeaf ->
      pcon $
        Bounded.PChunkProofV1
          ...
          (pdata 8)
          (pdata 0)
          (pdata $ plengthBS # redeemer)
          (pdata 0)
          (pdata redeemer)
          (pdata $ cekSinglePeak chunkLeaf)
          (pdata pnil)

scriptSourcesStageTenItemWitness ::
  forall s.
                                            ...
                                            \wrongWitness ->
                                              if accepting
                                                then
                                                  ...
                                                  pnot
                                                    # (pverifyScriptSourcesStageTwelveFinishSemanticsV1 # pre # wrongWitness)
                                                    #&& pnot
                                                    # ( pverifyScriptSourcesStageTwelveSemanticsV1
                                                          # pre
                                                          # wrongWitness
            ...
            plet (phashValidationContext # pfromData (pnativeControl'contextCbor c)) $ \contextHash ->
              plet
                ( cekBoundStateFixture
                    ...
                    (pcon PNativeScripts)
                    134
                    (phashWorkWitness # pcon PNativeScripts # 134 # workCbor)
                    0
                    0
                )
                $ \pre ->
              ...
              plet (phashValidationContext # pfromData (pnativeControl'contextCbor c)) $ \contextHash ->
                plet
                  ( cekBoundStateFixture
                      ...
                      (pcon PScriptIntegrity)
                      135
                      (phashWorkWitness # pcon PScriptIntegrity # 135 # workCbor)
                      0
                      0
                  )
                  $ \pre ->
                    plet
                      ( cekBoundStateFixture
                          ...
                          (pcon PScriptIntegrity)
                          136
                          (phashWorkWitness # pcon PScriptIntegrity # 136 # nextCbor)
                          0
                          0
                      )
                      $ \post ->
...
data MaximumFieldTerminalVector
  = MaximumFieldTerminalVectorV1
  { maximumLabel :: String
...
instance FromJSON MaximumFieldTerminalVector where
  parseJSON = withObject "MaximumFieldTerminalVector" $ \object ->
    MaximumFieldTerminalVectorV1
      <$> object .: "maximumLabel"
      <*> object .: "maximumTransactionId"
      <*> object .: "maximumTransactionCommitment"
      <*> object .: "maximumCompactCbor"
      <*> object .: "maximumWitnessSetCbor"
      <*> object .: "maximumLengthsCbor"
      <*> object .: "maximumFieldPreimage"
      <*> object .: "maximumContextCbor"
      <*> object .: "maximumFieldIndex"
      <*> object .: "maximumItemCount"
      <*> object .: "maximumItemIndex"
      <*> object .: "maximumTerminalChunkIndex"
      <*> object .: "maximumEncodedLength"
      <*> object .: "maximumPreWorkRoot"
      <*> object .: "maximumPostWorkRoot"

maximumFieldTerminalCase :: String -> String -> TestTree
maximumFieldTerminalCase name label = testCase name $ do
  decoded <-
    eitherDecodeFileStrict' "tests/fixtures/validation-machine-field-terminal-v1.json" ::
      IO (Either String [MaximumFieldTerminalVector])
  case decoded of
    Left err -> assertFailure err
    Right vectors -> case find ((== label) . maximumLabel) vectors of
      Nothing -> assertFailure $ "missing maximum field-terminal vector: " <> label
      Just vector -> passertEvalNoTraceWithoutHoistChecks (verifyMaximumFieldTerminal vector)
...
verifyMaximumFieldTerminal vector =
  let transactionId = maximumHex (maximumTransactionId vector)
      ...
      fieldPreimage = maximumHex (maximumFieldPreimage vector)
      contextCbor = maximumHex (maximumContextCbor vector)
      ...
      terminalChunkIndex = pconstant (maximumTerminalChunkIndex vector)
      encodedLength = pconstant (maximumEncodedLength vector)
      carriage = pcon $ NativeField.PInline (pdata fieldPreimage)
   in pmatch
        ( NativeCompact.pverifyNativeTxProofSourceV1
            # transactionId
            # compactCbor
                      ...
                      plet
                        ...
                        $ \pre ->
                          plet
                            ...
                            $ \post ->
                              plet (pcon $ PValidationOneStepWitnessV1 (pdata workCbor) (pdata post)) $ \transition ->
                                plet (maximumFieldTerminalAlternateLengthsCbor # lengthsCbor # fieldIndex) $ \alternateLengthsCbor ->
                                  plet
                                    ( pencodeTransactionFieldScanWitness
                                        # compactCbor
                                        # witnessSetCbor
                                        # alternateLengthsCbor
                                        # contextCbor
                                        # fieldIndex
                                        # itemIndex
                                        # terminalChunkIndex
                                    ...
                                    $ \alternateWorkCbor ->
                                      plet
                                        ( maximumFieldTerminalStateWithPriorRoot
                                            transactionId
                                            (NativeCompact.pnativeTxProofCommitmentV1 # compactCbor # witnessSetCbor # alternateLengthsCbor)
                                            transactionCommitment
                                            contextCbor
                                            40
                                            (phashWorkWitness # pcon PCanonicalDecode # 40 # alternateWorkCbor)
                                        )
                                        $ \alternatePre ->
                                          plet
                                            ( pcon $
                                                PValidationOneStepWitnessV1
                                                  (pdata alternateWorkCbor)
                                                  (pdata $ inputSetsExactRejection alternatePre $ pconstant "E_FIELD_PREIMAGE_SIZE")
                                            )
                                            $ \alternateTransition ->
                                              pand'List
                                                [ NativeCompact.pnativeTxProofCommitmentV1
                                                    ...
                                                    # lengthsCbor
                                                    #== transactionCommitment
                                                , NativeField.pfieldCommitment
                                                    # fieldPreimage
                                                    #== maximumFieldTerminalSlotCommitment
                                                    # compactCbor
                                                    # witnessSetCbor
                                                    # fieldIndex
                                                ...
                                                , FieldDoor.pmachineFieldItemCount # terminalItem #== itemCount
                                                , terminalChunkIndex
                                                    + 1
                                                      #== FieldDoor.pmachineFieldItemChunkCount
                                                      # terminalItem
...
maximumFieldTerminalSlotCommitment = phoistAcyclic $
  plam $ \compactCbor witnessSetCbor fieldIndex ->
    pmatch (NativeCompact.pdecodeNativeTxCompactV1 # compactCbor) $ \compact ->
      pmatch (pcompact'body compact) $ \body ->
        pmatch (NativeCompact.pdecodeNativeTxWitnessSetCompact # witnessSetCbor) $ \witnessSet ->
                ...
                pif (fieldIndex #== 3) (pbodyCompact'requiredObserversHash body) $
                  pif (fieldIndex #== 4) (pbodyCompact'requiredSignersHash body) $
                    pif (fieldIndex #== 5) (pbodyCompact'mintHash body) $
                      pif (fieldIndex #== 6) (pfromData $ pwitnessSetCompact'scriptTxWitsHash witnessSet) $
                        pif (fieldIndex #== 7) (pfromData $ pwitnessSetCompact'addrTxWitsHash witnessSet) $
                          pif (fieldIndex #== 8) (pfromData $ pwitnessSetCompact'redeemerTxWitsHash witnessSet) perror

maximumFieldTerminalAlternateLengthsCbor ::
  forall s.
  Term s (PByteString :--> PInteger :--> PByteString)
maximumFieldTerminalAlternateLengthsCbor = phoistAcyclic $
  plam $ \lengthsCbor fieldIndex ->
    pmatch (NativeCompact.pdecodeNativeTxFieldPreimageLengthsV1 # lengthsCbor) $ \lengths ->
      NativeCompact.pencodeNativeTxFieldPreimageLengthsV1
        # pcon
          ( PNativeTxFieldPreimageLengthsV1
              { plengths'spendInputs =
                  pif (fieldIndex #== 0) (plengths'spendInputs lengths + 1) (plengths'spendInputs lengths)
              , plengths'referenceInputs =
                  pif (fieldIndex #== 1) (plengths'referenceInputs lengths + 1) (plengths'referenceInputs lengths)
              , plengths'outputs =
                  pif (fieldIndex #== 2) (plengths'outputs lengths + 1) (plengths'outputs lengths)
              , plengths'requiredObservers =
                  pif (fieldIndex #== 3) (plengths'requiredObservers lengths + 1) (plengths'requiredObservers lengths)
              , plengths'requiredSigners =
                  pif (fieldIndex #== 4) (plengths'requiredSigners lengths + 1) (plengths'requiredSigners lengths)
              , plengths'mint =
                  pif (fieldIndex #< 5) (plengths'mint lengths) (plengths'mint lengths + 1)
              , plengths'addressWitnesses = plengths'addressWitnesses lengths
              , plengths'scriptWitnesses = plengths'scriptWitnesses lengths
              , plengths'redeemers = plengths'redeemers lengths
              }
          )
...
maximumFieldTerminalStateWithPriorRoot ::
  ...
  Term s PByteString ->
  Term s PValidationMachineStateV1
maximumFieldTerminalStateWithPriorRoot transactionId transactionCommitment priorLedgerRoot contextCbor counter workRoot =
  pcon $
    PValidationMachineStateV1
...
{- Retired per-item proof vectors. The current vectors are loaded from the
generated whole-field fixture above.
        ...
        $ \workCbor ->
          plet
            ( cekBoundStateFixture
                ...
                0
                (phashWorkWitness # pcon PCanonicalDecode # 0 # workCbor)
                0
                0
            )
            $ \pre ->
...
canonicalDecodeChunkTransition =
  plet (pencodeMidgardTxInput # pcon (PMidgardTxInput (pdata $ cekHash 0x41) (pdata 0))) $ \itemCbor ->
    plet (inlineFieldCarriage $ pcons # itemCbor # pnil) $ \carriage ->
                ...
                $ \body ->
                  plet (pcon $ PNativeTxCompact body (pblake2b_256 # witnessSetCbor) 0) $ \compact ->
                    plet (NativeCompact.pencodeNativeTxBodyCompact # body) $ \bodyCbor ->
                      plet (NativeCompact.pnativeTxIdForVersion # 1 # bodyCbor) $ \transactionId ->
                        plet (NativeCompact.pencodeNativeTxCompactV1 # compact) $ \compactCbor ->
                          plet (pcon $ PNativeTxFieldPreimageLengthsV1 41 1 1 1 1 1 1 1 1) $ \lengths ->
                                      ...
                                      $ \workCbor ->
                                        plet
                                          ( cekBoundStateFixture
                                              ...
                                              (pcon PCanonicalDecode)
                                              7
                                              (phashWorkWitness # pcon PCanonicalDecode # 7 # workCbor)
                                              0
                                              0
                                          )
                                          $ \pre ->
                                              ...
                                              $ \post ->
                                                plet (inputSetsExactRejection pre $ pconstant "E_FIELD_PREIMAGE_SIZE") $ \wrongPost ->
                                                  plet (pcon $ PValidationOneStepWitnessV1 (pdata workCbor) (pdata post)) $ \witness ->
                                                    plet (pcon $ PValidationOneStepWitnessV1 (pdata workCbor) (pdata wrongPost)) $ \wrongWitness ->
                                                      plet
                                                        (pcon $ PTransactionFieldChunkWitness (pdata 0) (pdata 0) (pdata carriage))
                                                        $ \auxiliary ->
                                                          plet (pcon $ PValidationOneStepEvidenceV1 (pdata witness) (pdata auxiliary)) $ \evidence ->
                                                            plet (pcanonicalDecodeControlFromWitness # workCbor) $ \control ->
                                                              pand'List
                                                                [ pcanonicalArgumentHeaderSize # 1 #== 1
                                                                , pmatch (ptransactionFieldItemEncodedLength # 0 # 38) $ \case
                                                                    PJust encodedLength -> encodedLength #== 40
                                                                    PNothing -> pconstant False
                                                                , pstructuralTransitionIsValid # pre # witness
                                                                ...
                                                                , pverifyCanonicalDecodeChunkSemanticsV1
                                                                    ...
                                                                    # 0
                                                                    # carriage
                                                                , pverifyCanonicalDecode # pre # witness # auxiliary # inlineFieldDoor
                                                                , pverifyCanonicalDecodeOneStepV1 # pre # evidence # inlineFieldDoor
                                                                , pverifyCanonicalDecodeSemanticsV1 # pre # evidence # inlineFieldDoor
                                                                , pnot
                                                                    # ( pverifyCanonicalDecodeChunkSemanticsV1
                                                                          # pre
                                                                          # wrongWitness
                                          ...
                                          plet
                                            ...
                                            $ \pre ->
                                              plet
                                                ...
                                                $ \post ->
                                                  plet (pcon $ PValidationOneStepWitnessV1 (pdata workCbor) (pdata post)) $ \transition ->
                                                    plet (inlineFieldCarriage $ pcons # itemCbor # pnil) $ \honestCarriage ->
                                                      plet
                                                        ( pcon $
                                                            PValidationOneStepEvidenceV1
                                                              (pdata transition)
                                                              ( pdata $
                                                                  pcon $
                                                                    PTransactionFieldChunkWitness (pdata 7) (pdata 0) (pdata honestCarriage)
                                                              )
                                                        )
                                                        $ \fieldSevenEvidence ->
                                                          pand'List
                                                            [ pnot
                                                                # ( NativeField.pfieldCommitment
                                                                      # (NativeField.pencodeFieldPreimage #$ pcons # itemCbor # pnil)
                                                                      #== NativeField.pemptyFieldCommitment
                                                                  )
                                                            , pnot
                                                                # ( pverifyCanonicalDecodeChunkSemanticsV1
                                      ...
                                      $ \workCbor ->
                                        plet
                                          ( cekBoundStateFixture
                                              ...
                                              (pcon PCanonicalDecode)
                                              9
                                              (phashWorkWitness # pcon PCanonicalDecode # 9 # workCbor)
                                              0
                                              0
                                          )
                                          $ \pre ->
                                            plet
                                              ...
                                              $ \post ->
                                                plet (pcon $ PValidationOneStepWitnessV1 (pdata workCbor) (pdata post)) $ \witness ->
                                                  plet
                                                    (pcon $ PTransactionFieldItemWitness (pdata carriage))
                                                    $ \auxiliary ->
                                                      plet (pcon $ PValidationOneStepEvidenceV1 (pdata witness) (pdata auxiliary)) $ \evidence ->
                                                        plet (pbindCanonicalDecodeItemSourceV1 # pre # witness) $ \source ->
                                                          plet (pobserveCanonicalDecodeItemV1 # pre # witness # inlineFieldDoor # carriage) $ \observation ->
                                                            plet (pverifyCanonicalDecodeItemObservationV1 # witness # source # observation) $ \proof ->
                                                              pmatch proof $ \proofFields ->
                                                                pand'List
                                                                  ...
                                                                  , pfromData (pcanonicalProof'nextEncodedLength proofFields) #== 41
                          ...
                          plet (phashValidationContext # pfromData (pnativeControl'contextCbor c)) $ \contextHash ->
                            plet
                              ( cekBoundStateFixture
                                  ...
                                  (pcon PScriptIntegrity)
                                  136
                                  (phashWorkWitness # pcon PScriptIntegrity # 136 # compactCbor)
                                  0
                                  0
                              )
                              $ \compactPre ->
                                plet
                                  ( cekBoundStateFixture
                                      ...
                                      (pcon PScriptIntegrity)
                                      137
                                      (phashWorkWitness # pcon PScriptIntegrity # 137 # witnessSetCbor)
                                      0
                                      0
                                  )
                                  $ \witnessSetPre ->
                                    plet
                                      ( cekBoundStateFixture
                                          ...
                                          (pcon PScriptIntegrity)
                                          138
                                          (phashWorkWitness # pcon PScriptIntegrity # 138 # finalizeCbor)
                                          0
                                          0
                                      )
                                      $ \finalizePre ->
                                                                        ...
                                                                        $ \invalidCommitment ->
                                                                          plet
                                                                            ( cekBoundStateFixture
                                                                                ...
                                                                                (pcon PScriptIntegrity)
                                                                                140
                                                                                (phashWorkWitness # pcon PScriptIntegrity # 140 # invalidCbor)
                                                                                0
                                                                                0
                                                                            )
                                                                            $ \invalidPre ->
  ...
  pmatch base $ \c ->
    pcon $
      PNativeScriptsControlV1
        ...
        (pdata 1)
        (pdata sourcePeaks)
        (pnativeControl'redeemerCount c)
        (pnativeControl'redeemerPeaks c)
        (pdata 1)
        (pdata purposePeaks)
        (pnativeControl'outputCount c)
        (pnativeControl'outputPeaks c)
        (pnativeControl'outputDescriptorPeaks c)
        (pnativeControl'mintCount c)
        (pnativeControl'mintPeaks c)
        (pdata 1)
        (pdata executionPeaks)
        (pdata cursor)
        (pdata bitmap)
                                              ...
                                              plet (phashValidationContext # pfromData (pnativeControl'contextCbor baseFields)) $ \contextHash ->
                                                plet
                                                  ( cekBoundStateFixture
                                                      ...
                                                      (pcon PNativeScripts)
                                                      136
                                                      (phashWorkWitness # pcon PNativeScripts # 136 # workCbor)
                                                      0
                                                      0
                                                  )
                                                  $ \pre ->
                                                    plet
                                                      ( cekBoundStateFixture
                                                          ...
                                                          (pcon PNativeScripts)
                                                          137
                                                          (phashWorkWitness # pcon PNativeScripts # 137 # nextCbor)
                                                          0
                                                          0
                                                      )
                                                      $ \post ->
                                        ...
                                        plet (pencodeNativeScriptsControlV1 # control) $ \workCbor ->
                                          plet
                                            ( pencodePhaseANativeScriptsScanWitness
                                                ...
                                                # 0
                                                # (-1)
                                                # pfromData (pnativeControl'signerCount baseFields)
                                                # pnil
                                                # workCbor
                                            )
                                            $ \phaseAWorkCbor ->
                                                  ...
                                                  plet (phashValidationContext # pfromData (pnativeControl'contextCbor baseFields)) $ \contextHash ->
                                                    plet
                                                      ( cekBoundStateFixture
                                                          ...
                                                          (pcon PNativeScripts)
                                                          138
                                                          (phashWorkWitness # pcon PNativeScripts # 138 # workCbor)
                                                          0
                                                          0
                                                      )
                                                      $ \pre ->
                                                        plet
                                                          ( cekBoundStateFixture
                                                              ...
                                                              (pcon PPhaseANativeScripts)
                                                              139
                                                              (phashWorkWitness # pcon PPhaseANativeScripts # 139 # phaseAWorkCbor)
                                                              0
                                                              0
                                                          )
                                                          $ \post ->
                                                              ...
                                                              plet
                                                                ( pcon $
                                                                    PNativeExecutionDescriptorWitness
                                                                      ...
                                                                      (pdata 0)
                                                                      (pdata sourceKey)
                                                                      (pdata scriptTotalLength)
                                                                      (pdata itemCommitment)
                                                                      (pdata pnil)
                                                                      (pdata $ pconstant "")
                                                                      (pdata pnil)
                                                                      (pdata $ pcon $ PDJust $ pdata firstChunkProof)
                                                                      (pdata pnil)
                                                                )
                                                                $ \auxiliary ->
...
scriptSourcesOutputProofFinalizeWrongNetwork :: forall s. Integer -> Term s PBool
scriptSourcesOutputProofFinalizeWrongNetwork rawHeader =
  plet
    ( preplicateBS # 1 # (pintegerToByte # pconstant rawHeader)
        <> preplicateBS # 28 # (pintegerToByte # 0x11)
    )
    $ \address ->
      plet (phexByteStr "a200581d" <> address <> phexByteStr "018200a0") $ \outputCbor ->
        plet (resolveInputsNoReferenceDescriptorFor outputCbor address) $ \descriptorCbor ->
                              ...
                              $ \baseWorkCbor ->
                                plet (pscriptSourcesStageZeroControlFromWitness # baseWorkCbor) $ \control ->
                                  plet
                                    ...
                                    $ \workCbor ->
                                      plet (scriptSourcesStateFromPhase transactionId phaseControl 91 workCbor) $ \pre ->
                                        plet (inputSetsExactRejection pre $ pconstant "E_NETWORK_ID_MISMATCH") $ \post ->
                                          plet (pcon $ PValidationOneStepWitnessV1 (pdata workCbor) (pdata post)) $ \witness ->
                                            pand'List
                                              [ LedgerOutputProof.pdescriptorIsExactV1
                                                  # outputProof
...
scriptSourcesInlinePending ::
  ...
  Term s NativeScriptScan.PVersionedScriptHeaderV1 ->
  Term s Blake2b224.PBlake2b224TraceControlV1 ->
  Term s PInlineSourceHashControlV1
                              ...
                              plet (scriptSourcesInlinePending collectionProof (pcon h) terminalHash) $ \pending ->
                                plet (scriptSourcesPendingWorkCbor phaseControl pending) $ \workCbor ->
                                  plet
                                    ( ScriptProof.pinlineSourceLeafHash
                                        # 0
                                        # pfromData (NativeScriptScan.pheader'languageTag h)
                                        # scriptHash
                                        # pfromData (BoundedCollection.pitemProof'itemLength item)
                                        # pfromData (BoundedCollection.pitemProof'itemCommitment item)
                                    )
...
resolveInputsWithPendingProof ::
  forall s.
  Term s PResolveInputsControlV1 ->
  Term s PResolveInputOutputProofV1 ->
  Term s PResolveInputsControlV1
...
resolveInputsStateFixtureAtRoot ::
  forall s.
  Term s PByteString ->
  Term s PResolveInputsControlV1 ->
  Term s PInteger ->
                          ...
                          $ \pending ->
                            plet (resolveInputsStateFixtureAtRoot transactionId control 46 singletonRoot) $ \memberPre ->
                              plet
                                ( resolveInputsStateFixtureAtRoot
                                    transactionId
                                    (resolveInputsWithPendingProof control pending)
                                    47
                                    singletonRoot
                                )
                                $ \memberPost ->
                                  plet
                                    ( pcon $
                                        PValidationOneStepWitnessV1
                                          (pdata $ resolveInputsControlCbor control)
                                          (pdata memberPost)
                                    )
                                    $ \memberWitness ->
...
resolveInputsOutputChunkProof =
  plet
    ( Bounded.phashChunk
        # OutputCommitment.poutputFieldIndex
        # 0
        # 0
        # resolveInputsOutputCbor
    )
                      ...
                      plet
                        ( pcon $
                            PResolveInputOutputProofV1
                              ...
                              (pdata pemptyResolutionScheduleHash)
                              (pdata descriptorCbor)
                              (pdata initialOutputProof)
                        )
                        $ \pending ->
                          plet
                            ( pcon $
                                LedgerOutputProof.PLedgerOutputProofChunks
                                  (pdata resolveInputsOutputChunkProof)
                                  (pdata $ pcon PDNothing)
                            )
                            $ \proofWitness ->
                                            ...
                                            plet (resolveInputsStateFixture transactionId activeControl 50) $ \pre ->
                                              plet (resolveInputsStateFixture transactionId nextControl 51) $ \post ->
                                                plet
                                                  ( pcon $
                                                      PValidationOneStepWitnessV1
                                                        (pdata $ resolveInputsControlCbor activeControl)
                                                        (pdata post)
                                                  )
                                                  $ \witness ->
                                                    pstructuralTransitionIsValid
  ...
  plet
    ( pcon $
        LedgerOutputScan.PLedgerOutputScanControlV1
          (pdata LedgerOutputScan.pversion)
          (pdata LedgerOutputScan.pstageTerminal)
          (pdata 37)
          (pdata 2)
          (pdata 0)
          (pdata address)
    ...
    $ \scan ->
      plet
        ( pcon $
            LedgerOutputValue.PLedgerOutputValueControlV1
              (pdata LedgerOutputValue.pversion)
              (pdata LedgerOutputValue.pstageTerminal)
              (pdata 0)
              (pdata $ pconstant "")
              (pdata CekData.pemptyDataPairSummaryV1)
              (pdata CekData.pemptyDataPairSummaryV1)
        ...
        $ \value ->
          pcon $
            LedgerOutputProof.PLedgerOutputProofControlV1
              (pdata LedgerOutputProof.pversion)
              (pdata LedgerOutputProof.pstageTerminal)
              (pdata 0)
              (pdata $ plengthBS # outputCbor)
              (pdata $ Bounded.pfromBytes # 2 # 0 # outputCbor)
              (pdata scan)
        ...
        $ \leaf ->
          plet (cekSinglePeak leaf) $ \peaks ->
            plet
              ( nativeScriptsControlForContext
                  1
                  (pconstant spendCount)
                  peaks
                  0
                  (Merkle.pfrontierCommitment # 0 # pnil)
                  0
                  pnil
              )
              $ \nativeControl ->
...
maximumObserverCekCase = do
  ...
  case decoded of
    Left err -> assertFailure err
    Right vectors -> case find ((== "maximum-observers") . maximumLabel) vectors of
      Nothing -> assertFailure "missing maximum observer field-terminal vector"
      Just vector ->
        passertEvalNoTraceWithoutHoistChecks (cekObserverCardanoMaximumFirstAndTerminal vector)

cekObserverCardanoMaximumFirstAndTerminal :: forall s. MaximumFieldTerminalVector -> Term s PBool
cekObserverCardanoMaximumFirstAndTerminal vector =
  let transactionId = maximumHex (maximumTransactionId vector)
      observerHash = phexByteStr "ffab1dd64f82b6991818c1ecc5047d52ce5d00f6fdbc5023e2980167"
      observersCarriage = pcon $ NativeField.PInline (pdata $ maximumHex $ maximumFieldPreimage vector)
   in plet (maximumObserverNativeControl vector) $ \nativeControl ->
...
maximumObserverNativeControl :: forall s. MaximumFieldTerminalVector -> Term s PNativeScriptsControlV1
maximumObserverNativeControl vector =
  pcon $
    PNativeScriptsControlV1
...
cekObserverRejectsMalformedProofsOrderAndSuccessors =
  plet (preplicateBS # 27 # (pintegerToByte # 0) <> phexByteStr "01") $ \observer0 ->
    plet (preplicateBS # 27 # (pintegerToByte # 0) <> phexByteStr "02") $ \observer1 ->
      plet (inlineFieldCarriage $ pcons # observer0 #$ pcons # observer1 # pnil) $ \carriage ->
            ...
            plet (BoundedCollection.phashBoundedCollectionItem # 3 # 0 # 28 # commitment0) $ \leaf0 ->
              plet (BoundedCollection.phashBoundedCollectionItem # 3 # 1 # 28 # commitment1) $ \leaf1 ->
                plet (Merkle.pbuildFrontier #$ pcons # pdata leaf0 #$ pcons # pdata leaf1 # pnil) $ \frontier ->
                  pmatch frontier $ \built ->
                    plet (NativeField.pfieldCommitmentFromItems #$ pcons # observer0 #$ pcons # observer1 # pnil) $ \observerCommitment ->
                      pmatch (cekObserverProofSource observerCommitment) $ \(PPair nativeControl transactionId) ->
                        plet (observerTwoItemCollectionProof 1 commitment1 (Merkle.pbuiltFrontier'peaks built) leaf0 3 2 28) $ \_proof1 ->
                          plet (observerTwoItemChunkProof 1 observer1 commitment1 3 28) $ \_chunk1 ->
                            plet (observerTwoItemCollectionProof 0 commitment0 (Merkle.pbuiltFrontier'peaks built) leaf1 3 2 28) $ \_proof0 ->
                              plet (observerTwoItemChunkProof 0 observer0 commitment0 3 28) $ \_chunk0 ->
                                plet (cekContextStageFixture 5 3 True) $ \contextControl ->
                                  plet (pprependCekObserverItemV1 # observer1 # pconstant False # CekData.pemptyDataPairSummaryV1) $ \firstItems ->
                                    plet (cekContextWithTestObservers 5 2 firstItems observer1 cekEmptySummary) $ \firstControl ->
                                      plet (cekContextWithTestObservers 5 2 firstItems observer0 cekEmptySummary) $ \malformedSuccessor ->
                                        plet (cekContextWithTestObservers 5 1 firstItems observer1 cekEmptySummary) $ \understatedCountSuccessor ->
                                                                    ...
                                                                    plet (cekObserverTransitionWitness nativeControl wrongSummaryControl 59) $ \summaryWitness ->
                                                                      pand'List
                                                                        ...
                                                                        , pnot
                                                                            # ( pverifyCekObserverContextItem
                                                                                  # pre
                                                                                  # (cekObserverTransitionWitness nativeControl understatedCountSuccessor 51)
                                                                                  # nativeControl
                                                                                  # contextControl
                                                                                  # inlineFieldDoor
                                                                                  # 3
...
observerTwoItemCollectionProof ::
  forall s.
  Integer ->
  Term s PByteString ->
  Term s (PBuiltinList (PAsData Merkle.PFrontierPeak)) ->
...
observerTwoItemCollectionProof itemIndex commitment peaks sibling fieldIndex itemCount itemLength =
  pcon $
    BoundedCollection.PItemProofV1
      ...
      (pdata $ pconstant itemLength)
      (pdata commitment)
      (pdata peaks)
      (pdata $ pcons # pdata sibling # pnil)

observerTwoItemChunkProof ::
  forall s.
          ...
          $ \malformedSummary ->
            plet (cekContextWithTestObservers 6 0 CekData.pemptyDataPairSummaryV1 (pconstant "") malformedSummary) $ \malformedControl ->
              plet (cekContextStageFixture 9 3 True) $ \successorControl ->
                pnot
                  # (pcekContextControlIsWellFormed # nativeControl # malformedControl)
...
cekObserverProofSourceWithIntegrity ::
  forall s.
  Term s PByteString ->
  Term s PByteString ->
  Term s PInteger ->
...
cekObserverBodyCborWithIntegrity ::
  forall s.
  Term s PByteString -> Term s PByteString -> Term s PByteString
...
valueOutputCbor :: forall s. Term s PByteString
valueOutputCbor =
  phexByteStr "a200581d68aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa01821a0012d687a1581c11111111111111111111111111111111111111111111111111111111a142223307"

cekDescriptorForIndex :: forall s. Integer -> Term s PByteString
cekDescriptorForIndex outputIndex = cekDescriptorForIndexAndLovelace outputIndex 1_234_567

cekDescriptorForIndexAndLovelace :: forall s. Integer -> Term s PInteger -> Term s PByteString
cekDescriptorForIndexAndLovelace outputIndex lovelace =
  plet valueOutputCbor $ \outputCbor ->
    pmatch (ScriptContext.ptxOutSummaryV1 # outputCbor # pconstant False) $ \case
      PNothing -> perror
      PJust cardano ->
        pmatch (ScriptContext.ptxOutSummaryV1 # outputCbor # pconstant True) $ \case
...
cekStateFixtureWithTransactionId ::
  forall s.
  Term s PByteString ->
  Term s PValidationPhase ->
  Term s PInteger ->
  ...
  Term s PInteger ->
  Term s PValidationMachineStateV1
cekStateFixtureWithTransactionId transactionId phase counter workRoot cpu memory =
  pcon $
    PValidationMachineStateV1
      (pdata pmachineVersion)
      (pdata $ cekHash 1)
