    ...
    testGroup
        ...
        , testGroup
            ...
            [ testCase "accepts a fresh empty root" $
                passertEval $
                    runInit (mkOut rootValue (rootDatum linkNone)) (mintOf rootName 1)
            ...
            , testCase "rejects a node datum" $
                pfails $
                    runInit (mkOut (nodeValue "aaa") (nodeDatum linkNone)) (mintOf (nodeName "aaa") 1)
        ...
        , testGroup
            ...
            [ testCase "accepts burning an empty root" $
                passertEval $
                    runDeinit [rootIn linkNone] (mintOf rootName (-1))
            , testCase "rejects deinitialising a non-empty list" $
                pfails $
                    runDeinit [rootIn linkToB] (mintOf rootName (-1))
        ...
        , testGroup
            ...
            , testCase "ignores a pure-Ada funding input before the root" $
                passertEval $
                    runInsert
                        [pureAdaIn, rootIn linkNone]
                        (mkOut rootValue (rootDatum (linkTo "bbb")))
                        (mkOut (nodeValue "bbb") (nodeDatum linkNone))
                        (mintOf (nodeName "bbb") 1)
            , testCase "ignores a pure-Ada funding input after the root" $
                passertEval $
                    runInsert
                        [rootIn linkNone, pureAdaIn]
                        (mkOut rootValue (rootDatum (linkTo "bbb")))
                        (mkOut (nodeValue "bbb") (nodeDatum linkNone))
                        (mintOf (nodeName "bbb") 1)
            ...
            [ testCase "accepts removing the node the root points at" $
                passertEval $
                    runRemove
                        ...
                        (mkOut rootValue (rootDatum linkNone))
                        (mintOf (nodeName "bbb") (-1))
            , testCase "ignores a pure-Ada input in the dual-input scan" $
                passertEval $
                    runRemove
                        [rootIn (linkTo "bbb"), pureAdaIn, nodeIn "bbb" linkNone]
                        (mkOut rootValue (rootDatum linkNone))
                        (mintOf (nodeName "bbb") (-1))
...
fundingAddress = scriptHashAddress (ScriptHash (unCurrencySymbol foreignPolicy))
...
-- | A normal wallet funding input has only the Ada policy in its value.
pureAdaIn :: TxInInfo
pureAdaIn =
    TxInInfo
        (outRefN 2)
        (TxOut fundingAddress (mkAdaValue 5_000_000) NoOutputDatum Nothing)
