...
{- |
Module      : Testing.Blake2b256Trace
Description : Behavioural tests for the Plutarch port of
              @lib/midgard/blake2b-256-trace-v1.ak@.

=== The oracle is the builtin

This module reimplements BLAKE2b-256 in modular arithmetic over byte strings so
that a hash can be replayed one round at a time. The reference it is checked
against is @blake2b_256@ itself — the ledger's own implementation, in C, reached
...
@blake2b_256("")@ is perfectly well defined. That is a deliberate narrowing, and
it is pinned here so that a later port cannot quietly widen it.
-}
module Testing.Blake2b256Trace (tests) where
...
import Testing.BoundedItem (definiteBytes)
...
aikenParityTests =
  testGroup
    ...
    [ testCase "complete_partial_block_trace_matches_the_builtin_256" $
        passertEval $
          traceDigest "\x6b" #== pblake2b_256 # pconstant "\x6b"
    , testCase "complete_full_block_trace_matches_the_builtin_256" $
        passertEval $
          traceDigest (BS.replicate 128 107)
            #== pblake2b_256 # pconstant (BS.replicate 128 107)
    , testCase "streamed_framing_matches_the_cek_blob_leaf_hash" $
        passertEval $
          plet (pconstant (BS.replicate 64 106)) $ \chunk ->
            traceDigest ("MidgardCekBlobChunkV1" <> definiteBytes (BS.replicate 64 106))
              #== phashBlobChunkV1 # chunk
    , testCase "decodes_the_typescript_multiblock_terminal_state_256" $
        passertEval typescriptTerminalStateIsExact
    ]

typescriptTerminalStateIsExact :: forall (s :: S). Term s PBool
typescriptTerminalStateIsExact =
  plet (pdecodeControlV1 # pconstant crossLanguageTerminalControlCbor) $ \control ->
    pmatch control $ \c ->
      pfromData (pctl'stage c) #== 3
        ...
        #&& pdigestV1 # control
          #== pcon
            ( PJust $
                pconstant $
                  hex "f9ddc8a9845baa65c817312211978ef15f2755c6187cfb471438370299d41f38"
            )
        #&& pencodeControlV1 # control #== pconstant crossLanguageTerminalControlCbor

crossLanguageTerminalControlCbor :: BS.ByteString
crossLanguageTerminalControlCbor =
  hex
  ...
  testGroup
    ...
    [ testGroup
        ...
        [ testCase (show n <> " bytes") $
            passertEval $
              traceDigest (message n) #== (pblake2b_256 # pconstant (message n))
        | n <- messageLengths
        ]
    , testCase "a message of all zeros, where a dropped counter would still pass" $
        passertEval $
          traceDigest (BS.replicate 200 0) #== (pblake2b_256 # pconstant (BS.replicate 200 0))
...
finished :: forall (s :: S). BS.ByteString -> Term s PBlake2b256TraceControlV1
finished msg =
...
steps ::
  forall (s :: S).
  Term s PBlake2b256TraceControlV1 -> [Maybe BS.ByteString] -> Term s PBool
...
controlT :: forall (s :: S). RefControl -> Term s PBlake2b256TraceControlV1
controlT c =
  pcon $
    PBlake2b256TraceControlV1
...
@0x01010020@ XORed into the low four bytes of the first — rather than taken from
...
initialChainingValue =
  ...
  where
    -- 0x01010020 little-endian: digest length 32, no key, fanout 1, depth 1.
    parameterBlockLe = [0x20, 0x00, 0x01, 0x01]
