...
pverifyCommittedEventMembershipV1 headerD committedEvent = P.do
  ...
  binding <- plet $ pmatch committedEvent $ \case
    PCommittedDuplicateDepositV1 membership ->
      pcon $ PEventMembershipBinding membership (pdata $ pcon PDepositsRootDomain)
        (pfromData pheader'depositsRoot) (pfromData pheader'depositCount)
    PCommittedDuplicateWithdrawalV1 membership ->
      pcon $ PEventMembershipBinding membership (pdata $ pcon PWithdrawalsRootDomain)
        (pfromData pheader'withdrawalsRoot) (pfromData pheader'withdrawalCount)
    PCommittedDuplicateForcedTransactionV1 membership ->
      pcon $ PEventMembershipBinding membership (pdata $ pcon PForcedTransactionsV1RootDomain)
        (pfromData pheader'forcedTransactionsRoot) (pfromData pheader'forcedTransactionCount)
  PEventMembershipBinding membership domain root count <- pmatch binding
  pverifyMembership membership domain root count
...
pverifyConfirmedDuplicateV1 state settlementRefInputIndex settledEvent referenceInputs = P.do
  ...
  pgetAuthenticInputDatumAndAssetNameWithPolicyAt
    referenceInputs
    pstep02State'settlementPolicyId
    settlementRefInputIndex $ \settledHeaderName settlementDatumData ->
                ...
                PDuplicateDepositV1 -> pmatch settledEvent $ \case
                  PCommittedDuplicateDepositV1 membership ->
                    pmatch (pfromData membership) $ \PRootMembershipProof {prootMembership'count} ->
                      pcon $ PEventMembershipBinding membership (pdata $ pcon PDepositsRootDomain)
                        (pfromData psettlement'depositsRoot) (pfromData prootMembership'count)
                  _ -> perror
                PDuplicateWithdrawalV1 -> pmatch settledEvent $ \case
                  PCommittedDuplicateWithdrawalV1 membership ->
                    pmatch (pfromData membership) $ \PRootMembershipProof {prootMembership'count} ->
                      pcon $ PEventMembershipBinding membership (pdata $ pcon PWithdrawalsRootDomain)
                        (pfromData psettlement'withdrawalsRoot) (pfromData prootMembership'count)
                  _ -> perror
                PDuplicateForcedTransactionV1 -> pmatch settledEvent $ \case
                  PCommittedDuplicateForcedTransactionV1 membership ->
                    pmatch (pfromData membership) $ \PRootMembershipProof {prootMembership'count} ->
                      pcon $ PEventMembershipBinding membership (pdata $ pcon PForcedTransactionsV1RootDomain)
                        (pfromData psettlement'forcedTransactionsRoot) (pfromData prootMembership'count)
                  _ -> perror
              PEventMembershipBinding membership domain root count <- pmatch binding
              PRootMembershipProof {prootMembership'key} <- pmatch $ pfromData membership
              pif
                ( prootMembership'key #== pforgetData pstep02State'eventKey
                    #&& pverifyMembership membership domain root count
                )
                settledHeaderHash
                perror
