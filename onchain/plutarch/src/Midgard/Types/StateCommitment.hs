...
module Midgard.Types.StateCommitment where
...
data PMidgardTxInfo (s :: S)
  ...
  , -- value minted by the transaction
    pmidguardTxInfo'mint :: Term s (PAsData PMintValue)
  , pmidguardTxInfo'redeemers :: Term s (PAsData (AssocMap.PSortedMap PMidgardScriptPurpose PRedeemer))
...
phasTransactionHash = phoistAcyclic $ plam $ \txroot txHash txInfoHash proof ->
  phasV1 # pto (pfromData txroot) # txHash # txInfoHash # proof
