{- |
Module      : Midgard.Blake2b224Trace
Description : Plutarch port of @lib/midgard/blake2b-224-trace-v1.ak@.

BLAKE2b-224 as a __replayable trace__: the hash is computed one step at a time —
...
-- | Aiken @version@.
pblake2b224TraceVersion :: forall (s :: S). Term s PInteger
pblake2b224TraceVersion = 1
...
@0x0101001c@ read as a little-endian word: one byte of digest length (28), one
of key length (0), fanout 1 and depth 1. It is XORed into the first word of the
IV, which is what makes this BLAKE2b-/224/ rather than BLAKE2b-512.
-}
pparameterBlockV1 :: forall (s :: S). Term s PInteger
pparameterBlockV1 = 16842780
...
{- | Aiken @Blake2b224TraceControlV1@.
...
-}
data PBlake2b224TraceControlV1 (s :: S) = PBlake2b224TraceControlV1
  ...
  deriving (PlutusType) via (DeriveAsDataStruct PBlake2b224TraceControlV1)
...
pexpectedActiveBlockLength ::
  forall (s :: S). Term s (PBlake2b224TraceControlV1 :--> PInteger)
...
pactiveStateIsEmpty :: forall (s :: S). Term s (PBlake2b224TraceControlV1 :--> PBool)
...
pactiveBlockPaddingIsZero ::
  forall (s :: S). Term s (PBlake2b224TraceControlV1 :--> PBool)
...
-}
pcontrolIsWellFormed :: forall (s :: S). Term s (PBlake2b224TraceControlV1 :--> PBool)
      ...
      plet (pfromData (pctl'stage c)) $ \stage ->
        plet (pfromData (pctl'cursor c)) $ \cursor ->
          plet (pfromData (pctl'totalLength c)) $ \totalLength ->
            pand'List
              [ pfromData (pctl'version c) #== pblake2b224TraceVersion
              , pstageReady #<= stage
...
pinitialControlV1 ::
  forall (s :: S). Term s (PInteger :--> PBlake2b224TraceControlV1)
pinitialControlV1 = phoistAcyclic $
  plam $ \totalLength ->
    plet
      ( pcon $
          PBlake2b224TraceControlV1
            { pctl'version = pdata pblake2b224TraceVersion
...
pcontrolFromDataV1 ::
  forall (s :: S). Term s (PData :--> PBlake2b224TraceControlV1)
...
-}
pdecodeControlV1 ::
  forall (s :: S). Term s (PByteString :--> PBlake2b224TraceControlV1)
...
pbeginBlockV1 ::
  ...
  Term
    ...
    ( PBlake2b224TraceControlV1
        :--> PByteString
        :--> PMaybe PBlake2b224TraceControlV1
    )
...
proundV1 ::
  forall (s :: S).
  Term s (PBlake2b224TraceControlV1 :--> PBlake2b224TraceControlV1)
...
The first 28 bytes of the chaining value, and only at @terminal@. A trace that
has not finished has no digest — not a partial one.
-}
pdigestV1 ::
  forall (s :: S). Term s (PBlake2b224TraceControlV1 :--> PMaybe PByteString)
