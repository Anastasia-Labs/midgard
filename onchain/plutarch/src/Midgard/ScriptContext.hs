...
module Midgard.ScriptContext (
  ...
  ptxInInfoDataV1,
  ptxInInfoSummaryV1,
  pprependAuthenticatedListItemV1,
  pprependResolvedTxInInfoV1,
  pprependResolvedDescriptorTxInInfoV1,
  pprependOutputV1,
  pprependOutputDescriptorV1,
  pprependSignerV1,
  pobserverCollectionSummaryV1,
  pvalidityIntervalSummaryV1,
  pfeeSummaryV1,
  ptransactionIdSummaryV1,
  pscriptPurposeSummaryV1,
  ptxInfoTailFieldsSummaryV1,
  ptxInfoFromTailSummaryV1,
  ptxInfoSummaryV1,
  pspendDatumSummaryV1,
  pledgerOutputSummariesV1,
  pledgerOutputSummariesWithValueV1,
  pledgerOutputSummariesOfAuthenticatedPartsV1,
  pcardanoScriptInfoSummaryV1,
  pcardanoSpendScriptInfoFromDescriptorV1,
...
pmaterialiseDatum = phoistAcyclic $ plam $ \datumCbor ->
  pif
    (datumCbor #== pconstant "\x40")
    (pcon $ PJust $ pforgetData $ pdata $ pconstant @PByteString "")
    ( pif
        (pisMaterialisablePlutusDataV1 # datumCbor)
        (pdeserialise # datumCbor)
        (pcon PNothing)
    )
          ...
          PJust datum ->
            pcon $
              PJust $
                pconstrData 0 $
                  pcons
                    # paddressData output midgardEncoding
                    #$ pcons
                    # pscriptValueData valueData
                    #$ pcons
                    # datum
                    #$ pcons
                    # preferenceScriptData output
                    # pnil
...
pledgerOutputSummariesV1 = phoistAcyclic $ plam $ \outputCbor ->
  pledgerOutputSummariesWithValueV1 # outputCbor # pcon PNothing

pledgerOutputSummariesWithValueV1 :: forall s. Term s (PByteString :--> PMaybe PDataSummaryV1 :--> PMaybe (PPair PDataSummaryV1 (PPair PDataSummaryV1 PDataSummaryV1)))
pledgerOutputSummariesWithValueV1 = phoistAcyclic $ plam $ \outputCbor valueSummary ->
  pmatch (pdecodeCanonicalOutput # outputCbor) $ \case
    PNothing -> pcon PNothing
    PJust output -> pmatch (pdeserialise # outputCbor) $ \case
      PNothing -> pcon PNothing
      PJust outputData -> pmatch (poutputValueData outputData) $ \case
        ...
        PJust valueData -> pmatch (pdatumData output) $ \case
          PNothing -> pcon PNothing
          PJust datumField -> plet
            ( pmatch valueSummary $ \case
                PJust summary -> summary
                PNothing -> psemanticDataSummaryV1 # pscriptValueData valueData
            )
            $ \value ->
              pcon $ PJust $ pledgerOutputSummariesOfAuthenticatedPartsV1 # output # value # datumField

-- The caller authenticates the output metadata, value summary and datum field.
pledgerOutputSummariesOfAuthenticatedPartsV1 :: forall s. Term s (PMidgardTxOutput :--> PDataSummaryV1 :--> PData :--> PPair PDataSummaryV1 (PPair PDataSummaryV1 PDataSummaryV1))
pledgerOutputSummariesOfAuthenticatedPartsV1 = phoistAcyclic $ plam $ \output value datumField ->
  plet (psemanticDataSummaryV1 # datumField) $ \datum ->
    plet (psemanticDataSummaryV1 # preferenceScriptData output) $ \referenceScript ->
      let summary midgard =
            psmallConstrDataSummaryV1
              # 0
              # ( pprependDataListSummaryV1
                    # (psemanticDataSummaryV1 # paddressData output midgard)
                    # ( pprependDataListSummaryV1
                          # value
                          # ( pprependDataListSummaryV1
                                # datum
                                # (pprependDataListSummaryV1 # referenceScript # pemptyDataListSummaryV1)
                            )
                      )
                )
       in pcon $ PPair (summary $ pconstant False) $ pcon $ PPair (summary $ pconstant True) (pspendDatumSummaryOfField datumField)

ptxOutRefData :: forall s. Term s PMidgardTxInput -> Term s PData
ptxOutRefData input = pmatch input $ \i ->
  pconstrData 0 $
...
pprependAuthenticatedListItemV1 = phoistAcyclic $
  plam $
      ...
      pmatch item $ \i -> pmatch tailSummary $ \tailFields ->
        pif
          ( itemCount
              ...
              #&& itemIndex
              #== itemCount
              - pfromData (pseq'length tailFields)
              - 1
                #&& pverifyMembership
                # itemCount
                # peaks
                # itemIndex
                # ( Proof.pcontextItemLeafHash
                      # collectionKind
                      # itemIndex
                      # pfromData (psummary'root i)
                      # pfromData (psummary'cborLength i)
                      # pfromData (psummary'memory i)
                  )
                # siblings
          )
          (pcon $ PJust $ pprependDataListSummaryV1 # item # tailSummary)
          (pcon PNothing)
...
pprependResolvedTxInInfoV1 = phoistAcyclic $
  plam $
            ...
            pif
              ( resolvedCount
                  ...
                  - 1
                    ...
                    # peaks
                    # itemIndex
                    # (Proof.presolvedContextItemLeafHash # sourceKind # itemIndex # inputCbor # outputCbor)
                    # siblings
              )
...
pprependResolvedDescriptorTxInInfoV1 = phoistAcyclic $
  plam $
            ...
            plet (pdecodeLedgerOutputCommitment # descriptorCbor) $ \descriptor ->
              plet
                ( pif
                    ...
                    (pmatch descriptor $ \d -> pfromData $ poutputCommitment'cardanoTxOut d)
                )
                $ \output ->
                  pmatch input $ \inputFields -> pmatch descriptor $ \descriptorFields -> pmatch tailSummary $ \tailFields ->
                    pif
                      ( resolvedCount
                          ...
                          # input
                          #== inputCbor
                          #&& pfromData (poutputCommitment'outputIndex descriptorFields)
                          #== pfromData (ptxInput'outputIndex inputFields)
                          #&& pfrontierIsWellFormed
                          # resolvedCount
                          # peaks
                          #&& itemIndex
                          ...
                          - 1
                            ...
                            # peaks
                            # itemIndex
                            # (Proof.presolvedContextItemLeafHash # sourceKind # itemIndex # inputCbor # descriptorCbor)
                            # siblings
                      )
    ...
    pif
      ( count
          ...
          - 1
            ...
            # peaks
            # index
            # (Proof.poutputLeafHash # index # outputCbor)
            # siblings
      )
...
pprependOutputDescriptorV1 = phoistAcyclic $ plam $ \count peaks index descriptorCbor siblings midgardEncoding tailSummary ->
  plet (pdecodeLedgerOutputCommitment # descriptorCbor) $ \descriptor -> pmatch descriptor $ \d ->
    plet
      ( pif
          midgardEncoding
          (pfromData $ poutputCommitment'midgardTxOut d)
          (pfromData $ poutputCommitment'cardanoTxOut d)
      )
      $ \output -> pmatch tailSummary $ \tailFields ->
        pif
          ( count
              #>= 0
              #&& pfromData (poutputCommitment'outputIndex d)
              #== index
              #&& pfrontierIsWellFormed
              # count
              # peaks
              ...
              - 1
                ...
                # peaks
                # index
                # (Proof.poutputDescriptorLeafHash # index # descriptorCbor)
                # siblings
          )
          (pcon $ PJust $ pprependDataListSummaryV1 # output # tailSummary)
          (pcon PNothing)
...
pinsertSortedUnique = phoistAcyclic $ pfix $ \self -> plam $ \value values -> pmatch values $ \case
  ...
  PCons headValue rest ->
    pif (value #== headValue) values $
...
pobserverCollectionSummaryV1 = phoistAcyclic $ plam $ \observers midgardEncoding ->
  plet (psortUnique # observers) $ \canonical ->
    pif
      (plength # canonical #<= pmaxTxSizeDerivedItemCount #&& pallHash28 # canonical)
      ( pcon $
          PJust $
            psemanticDataSummaryV1
              # pif
...
pvalidityIntervalSummaryV1 = phoistAcyclic $ plam $ \start end ->
  psemanticDataSummaryV1
    # pconstrData
      0
      ( pfields2
...
ptransactionIdSummaryV1 = phoistAcyclic $ plam $ \txId ->
  pif
    (plengthBS # txId #== 32)
    (psemanticDataSummaryV1 # pbytesData txId)
    perror
    ...
    $ plet
      ( pif
          ...
          ( plet (pdecodeMidgardTxInputCbor # subject) $ \input ->
              pif
                (pencodeMidgardTxInput # input #/= subject)
                (pcon PNothing)
                ( pcon $
                    PJust $
                      pconstrData 1 $
...
ptxInfoFromTailSummaryV1 = phoistAcyclic $
  plam $
            ...
            plet (pfeeSummaryV1 # fee) $ \feeSummary ->
              plet (pvalidityIntervalSummaryV1 # start # end) $ \validity ->
                plet
                  ...
                  $ \fields -> psmallConstrDataSummaryV1 # 0 # fields
 where
  ifSummary midgard yes no tailFields =
    pif
      midgard
...
ptxInfoSummaryV1 = phoistAcyclic $
  plam $
      ...
      pmatch input $ \i -> pmatch descriptor $ \d ->
        pif
          ( pencodeMidgardTxInput
              ...
              #== subject
              #&& pfromData (poutputCommitment'outputIndex d)
              #== pfromData (ptxInput'outputIndex i)
          )
          ( plet (psemanticDataSummaryV1 # ptxOutRefData input) $ \outRef ->
              pcon $
                PJust $
