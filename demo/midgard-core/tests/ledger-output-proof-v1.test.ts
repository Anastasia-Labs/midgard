...
describe("bounded ledger output proof V1", () => {
  it("authenticates and hashes a real multi-chunk reference script", () => {
    ...
    expect(
      digestMidgardLedgerOutputReferenceScriptV1(trace.terminal)!.toString(
        "hex",
      ),
    ).toBe(hashMidgardVersionedScript(script));
    expect(
      summarizeMidgardLedgerOutputCardanoSpendDatumV1(trace.terminal)!.root,
    ).toHaveLength(32);
    expect(trace.terminal.datum!.stage).toBe(7);
    expect(
      commitMidgardLedgerOutputReferenceScriptItemV1(trace.terminal),
    ).toStrictEqual(
      buildMidgardBoundedItemV1({
  ...
  it("fails closed for substituted chunks and malformed native syntax", () => {
    ...
    const datumCommitmentStep = validTrace.steps.find(
      ({ control, witness: stepWitness }) =>
        control.stage === MidgardLedgerOutputProofStagesV1.DatumTraversal &&
  ...
  it("encodes the long terminal control canonically for Aiken", () => {
    ...
    const terminalCbor = encodeMidgardLedgerOutputProofControlV1(
      trace.terminal,
    );
    ...
    expect(MidgardLedgerOutputProofResultKindsV1.Advanced).toBe("advanced");
  });
});
