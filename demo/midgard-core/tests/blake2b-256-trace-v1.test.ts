...
describe("bounded BLAKE2b-256 trace V1", () => {
  ...
  it("reproduces the CEK blob-leaf hash from streamed framing", () => {
    ...
    const terminal = buildMidgardBlake2b256TraceV1(message).at(-1)!.next;

    expect(digestMidgardBlake2b256TraceV1(terminal)).toStrictEqual(
      hashMidgardCekBlobChunkV1(chunk),
    );
  });
  ...
  it("encodes the multi-block terminal state canonically for Aiken", () => {
    ...
    const terminal = buildMidgardBlake2b256TraceV1(message).at(-1)!.next;
    expect(
      encodeMidgardBlake2b256TraceControlV1(terminal).toString("hex"),
    ).toBe(
  ...
  it("fails closed for wrong block boundaries and forged initial state", () => {
    const initial = initialMidgardBlake2b256TraceControlV1(129);
    expect(
      advanceMidgardBlake2b256TraceV1({
        control: initial,
