...
export const MIDGARD_BLAKE2B_256_TRACE_V1_VERSION = 1 as const;
export const MIDGARD_BLAKE2B_256_BLOCK_BYTES = 128;
export const MIDGARD_BLAKE2B_256_ROUNDS = 12;
export const MIDGARD_BLAKE2B_256_DIGEST_BYTES = 32;

export const MidgardBlake2b256TraceStagesV1 = Object.freeze({
  Ready: 0,
  Round: 1,
  Finish: 2,
  Terminal: 3,
...
export type MidgardBlake2b256TraceStageV1 =
  (typeof MidgardBlake2b256TraceStagesV1)[keyof typeof MidgardBlake2b256TraceStagesV1];

const WORD_MASK = 0xffff_ffff_ffff_ffffn;
const PARAMETER_BLOCK_V1 = 0x0101_0020n;
...
export type MidgardBlake2b256TraceControlV1 = {
  readonly version: typeof MIDGARD_BLAKE2B_256_TRACE_V1_VERSION;
  readonly stage: MidgardBlake2b256TraceStageV1;
...
export type MidgardBlake2b256TraceStepV1 = {
  readonly control: MidgardBlake2b256TraceControlV1;
  readonly block: Buffer | null;
  readonly next: MidgardBlake2b256TraceControlV1;
};

// Callers bind a non-empty domain/framing prefix before any streamed content.
...
export const isWellFormedMidgardBlake2b256TraceControlV1 = (
  control: MidgardBlake2b256TraceControlV1,
): boolean => {
  if (
    control.version !== MIDGARD_BLAKE2B_256_TRACE_V1_VERSION ||
    !Number.isSafeInteger(control.stage) ||
    control.stage < MidgardBlake2b256TraceStagesV1.Ready ||
    control.stage > MidgardBlake2b256TraceStagesV1.Terminal ||
  ...
  if (
    ...
    return (
      ...
      (control.stage === MidgardBlake2b256TraceStagesV1.Ready
        ? control.cursor < control.totalLength &&
          control.cursor % MIDGARD_BLAKE2B_256_BLOCK_BYTES === 0
        : control.cursor === control.totalLength)
    );
  }
  ...
  return (
    ...
    (control.stage === MidgardBlake2b256TraceStagesV1.Round
      ? control.round >= 0 && control.round < MIDGARD_BLAKE2B_256_ROUNDS
      : control.round === MIDGARD_BLAKE2B_256_ROUNDS)
  );
};

export const initialMidgardBlake2b256TraceControlV1 = (
  totalLength: number,
): MidgardBlake2b256TraceControlV1 => {
  const control = {
    version: MIDGARD_BLAKE2B_256_TRACE_V1_VERSION,
    stage: MidgardBlake2b256TraceStagesV1.Ready,
    cursor: 0,
    totalLength,
    chainingValue: initialChainingValue(),
    ...emptyActiveState,
  } satisfies MidgardBlake2b256TraceControlV1;
  if (!isWellFormedMidgardBlake2b256TraceControlV1(control)) {
    throw new Error("Invalid V1 BLAKE2b-256 trace total length");
  }
  return control;
};

export const encodeMidgardBlake2b256TraceControlV1 = (
  control: MidgardBlake2b256TraceControlV1,
): Buffer => {
  if (!isWellFormedMidgardBlake2b256TraceControlV1(control)) {
    throw new Error("Invalid V1 BLAKE2b-256 trace control");
  }
...
export const advanceMidgardBlake2b256TraceV1 = ({
  control,
  block,
}: {
  readonly control: MidgardBlake2b256TraceControlV1;
  readonly block?: Uint8Array | null;
}): MidgardBlake2b256TraceControlV1 | null => {
  try {
    if (!isWellFormedMidgardBlake2b256TraceControlV1(control)) return null;
    if (control.stage === MidgardBlake2b256TraceStagesV1.Ready) {
      const expectedLength = Math.min(
        MIDGARD_BLAKE2B_256_BLOCK_BYTES,
        control.totalLength - control.cursor,
      );
      ...
      const activeBlock = Buffer.alloc(MIDGARD_BLAKE2B_256_BLOCK_BYTES);
      activeBlock.set(block);
      const bytesCompressed = control.cursor + expectedLength;
      const next = {
        ...control,
        stage: MidgardBlake2b256TraceStagesV1.Round,
        activeBlock,
      ...
      return isWellFormedMidgardBlake2b256TraceControlV1(next) ? next : null;
    }
    if (block !== null && block !== undefined) return null;
    if (control.stage === MidgardBlake2b256TraceStagesV1.Round) {
      const round = control.round + 1;
      const next = {
        ...control,
        stage:
          round === MIDGARD_BLAKE2B_256_ROUNDS
            ? MidgardBlake2b256TraceStagesV1.Finish
            : MidgardBlake2b256TraceStagesV1.Round,
    ...
    if (control.stage === MidgardBlake2b256TraceStagesV1.Finish) {
...
export const digestMidgardBlake2b256TraceV1 = (
  control: MidgardBlake2b256TraceControlV1,
): Buffer | null =>
  isWellFormedMidgardBlake2b256TraceControlV1(control) &&
  control.stage === MidgardBlake2b256TraceStagesV1.Terminal
    ? control.chainingValue.subarray(0, MIDGARD_BLAKE2B_256_DIGEST_BYTES)
    : null;

export const buildMidgardBlake2b256TraceV1 = (
  message: Uint8Array,
): readonly MidgardBlake2b256TraceStepV1[] => {
  const bytes = Buffer.from(message);
  let control = initialMidgardBlake2b256TraceControlV1(bytes.length);
  const steps: MidgardBlake2b256TraceStepV1[] = [];
  while (control.stage !== MidgardBlake2b256TraceStagesV1.Terminal) {
    const block =
      control.stage === MidgardBlake2b256TraceStagesV1.Ready
        ? bytes.subarray(
            control.cursor,
            control.cursor + MIDGARD_BLAKE2B_256_BLOCK_BYTES,
          )
        : null;
    const next = advanceMidgardBlake2b256TraceV1({ control, block });
    if (next === null || !isWellFormedMidgardBlake2b256TraceControlV1(next)) {
      throw new Error("V1 BLAKE2b-256 trace failed closed");
    }
