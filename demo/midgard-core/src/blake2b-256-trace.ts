...
export const MIDGARD_BLAKE2B_256_TRACE_VERSION = 1 as const;
...
export const MidgardBlake2b256TraceStages = Object.freeze({
  Ready: 0,
  Round: 1,
  Finish: 2,
  Terminal: 3,
...
export type MidgardBlake2b256TraceStage =
  (typeof MidgardBlake2b256TraceStages)[keyof typeof MidgardBlake2b256TraceStages];

const WORD_MASK = 0xffff_ffff_ffff_ffffn;
const PARAMETER_BLOCK = 0x0101_0020n;
...
export type MidgardBlake2b256TraceControl = {
  readonly version: typeof MIDGARD_BLAKE2B_256_TRACE_VERSION;
  readonly stage: MidgardBlake2b256TraceStage;
...
export type MidgardBlake2b256TraceStep = {
  readonly control: MidgardBlake2b256TraceControl;
  readonly block: Buffer | null;
  readonly next: MidgardBlake2b256TraceControl;
};
...
export const isWellFormedMidgardBlake2b256TraceControl = (
  ...
  if (
    control.version !== MIDGARD_BLAKE2B_256_TRACE_VERSION ||
    !Number.isSafeInteger(control.stage) ||
    control.stage < MidgardBlake2b256TraceStages.Ready ||
    control.stage > MidgardBlake2b256TraceStages.Terminal ||
...
export const initialMidgardBlake2b256TraceControl = (
  totalLength: number,
): MidgardBlake2b256TraceControl => {
  const control = {
    version: MIDGARD_BLAKE2B_256_TRACE_VERSION,
    stage: MidgardBlake2b256TraceStages.Ready,
    cursor: 0,
    totalLength,
    chainingValue: initialChainingValue(),
    ...emptyActiveState,
  } satisfies MidgardBlake2b256TraceControl;
  if (!isWellFormedMidgardBlake2b256TraceControl(control)) {
    throw new Error("Invalid V1 BLAKE2b-256 trace total length");
  }
  return control;
};

export const encodeMidgardBlake2b256TraceControl = (
  control: MidgardBlake2b256TraceControl,
): Buffer => {
  if (!isWellFormedMidgardBlake2b256TraceControl(control)) {
...
export const advanceMidgardBlake2b256Trace = ({
  control,
  block,
}: {
  readonly control: MidgardBlake2b256TraceControl;
  readonly block?: Uint8Array | null;
}): MidgardBlake2b256TraceControl | null => {
  try {
    if (!isWellFormedMidgardBlake2b256TraceControl(control)) return null;
    if (control.stage === MidgardBlake2b256TraceStages.Ready) {
      const expectedLength = Math.min(
        MIDGARD_BLAKE2B_256_BLOCK_BYTES,
        control.totalLength - control.cursor,
      );
      ...
      const next = {
        ...control,
        stage: MidgardBlake2b256TraceStages.Round,
        activeBlock,
      ...
      return isWellFormedMidgardBlake2b256TraceControl(next) ? next : null;
    }
    if (block !== null && block !== undefined) return null;
    if (control.stage === MidgardBlake2b256TraceStages.Round) {
      const round = control.round + 1;
      const next = {
        ...control,
        stage:
          round === MIDGARD_BLAKE2B_256_ROUNDS
            ? MidgardBlake2b256TraceStages.Finish
            : MidgardBlake2b256TraceStages.Round,
    ...
    if (control.stage === MidgardBlake2b256TraceStages.Finish) {
      const cursor = control.cursor + control.activeBlockLength;
      const next = {
        ...control,
        stage:
          cursor === control.totalLength
            ? MidgardBlake2b256TraceStages.Terminal
...
export const digestMidgardBlake2b256Trace = (
  control: MidgardBlake2b256TraceControl,
): Buffer | null =>
  isWellFormedMidgardBlake2b256TraceControl(control) &&
  control.stage === MidgardBlake2b256TraceStages.Terminal
    ? control.chainingValue.subarray(0, MIDGARD_BLAKE2B_256_DIGEST_BYTES)
    : null;

export const buildMidgardBlake2b256Trace = (
  message: Uint8Array,
): readonly MidgardBlake2b256TraceStep[] => {
  const bytes = Buffer.from(message);
  let control = initialMidgardBlake2b256TraceControl(bytes.length);
  const steps: MidgardBlake2b256TraceStep[] = [];
  while (control.stage !== MidgardBlake2b256TraceStages.Terminal) {
    const block =
      control.stage === MidgardBlake2b256TraceStages.Ready
        ? bytes.subarray(
            control.cursor,
        ...
        : null;
    const next = advanceMidgardBlake2b256Trace({ control, block });
    if (next === null || !isWellFormedMidgardBlake2b256TraceControl(next)) {
