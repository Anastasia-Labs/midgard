...
- [x] `midgard-validation/src/native-tx-bridge.ts` — new file. Lifts `midgardTsToNativeFull` / `nativeFullToMidgardTs` from lucid-midgard's `builder.ts` and exposes `decodeMidgardTxBytesToNativeFull` + `midgardTxIdFromNativeFull`. phase-a uses these in place of `decodeMidgardNativeTxFull` / `computeMidgardNativeTxIdFromFull`; phase-b uses the decode helper at three call sites. This fixes phase-a/phase-b decoding the new wire format (without which `runPhaseAValidation` / `runPhaseBValidationWithPatch` would have rejected every submitted tx with `E_CBOR_DESERIALIZATION`). ⚠️ NOTE: `nativeFullToMidgardTs` is now duplicated between `lucid-midgard/src/builder.ts` and `midgard-validation/src/native-tx-bridge.ts`. Phase 5-main (below) should delete both copies once the consumers go midgard-ts-native.
