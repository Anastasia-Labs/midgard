...
- [x] Refiled the 15 remaining `it.skip(TODO(Phase 6))` markers (2026-05-14). Unskipped + fixed 1 (`recovers native script identity from explicit versioned native script bytes` — the test was just feeding the wrong envelope format). The other 14 are re-tagged:
  - **`WONTFIX(post-Phase-5-main)` × 11** — test OLD-codec error paths impossible to construct in the midgard-ts wire format. Categories: (a) malformed mint/observer/signer CBOR preimages (Mint/Hash28[] are structured, no preimages); (b) datum-hash outputs (midgard-ts `TransactionOutput.datum` is inline-only); (c) OLD-CBOR-codec error strings (`"Babbage map-form"`, `"datum hashes"`, `"missing address key 0"`); (d) `midgardNativeTxFullToCardanoTxEncoding` cardano-conversion legacy tests. These should be deleted in a follow-up test cleanup (no live caller exercises the paths).
  - **`DESIGN-TODO(dual-domain script discovery)` × 4** — real design gap: phase-A only counts witness scripts, not reference-script-derived script hashes; and `resolveScriptSource` matches one hash per source so "same UPLC bytes serve both PlutusV3 (prefix 0x03) and MidgardV1 (prefix 0x80) domains" doesn't work. Not a codec issue.
- [x] Test cleanup follow-up (2026-05-15): deleted **14** `WONTFIX(post-Phase-5-main)` skipped tests across `tests/midgard-local-script-eval.test.ts` (3), `tests/midgard-native-tx-codec.test.ts` (2), and `tests/native-transaction-integration.test.ts` (9). Also removed the helpers that only served them: `makeEmptyMintMapPreimage`, `makeEmptyMintPolicyAssetsPreimage`, `makeDatumHashScriptOutput`. Tests went from **128 / 1 / 52** → **129 / 1 / 35**. The 4 `DESIGN-TODO(dual-domain script discovery)` skips are kept (real feature gap, not codec).
...
- [x] **submit-l2-transfer test bridge wiring (2026-05-13):** `tests/submit-l2-transfer.test.ts` switched `decodeMidgardNativeTxFull` / `computeMidgardNativeTxIdFromFull` imports to the validation bridge (these consume midgard-ts wire bytes from the built tx). Dropped an OLD-codec `bytes[0] >> 5 === 5` (CBOR-map-tag) assertion that doesn't apply to midgard-ts binary outputs. **(2026-05-15)** Replaced the bridge import with a direct `decodeTransaction` / `transactionId` from `@al-ft/midgard-ts` — the test now inspects `tx.body.inputs` / `tx.body.outputs` structurally instead of round-tripping through `MidgardNativeTxFull`. Also dropped the now-unused `decodeMidgardTxOutput` import.
...
- [x] `tests/txs/txs_0.json` is Cardano CBOR — unchanged; consumed via the Cardano-ingress path. `tests/benchmarks/fixtures/{initial-ledger-state,tx-sequence}.json` regenerated (2026-05-14) via `pnpm bench:fixtures:generate`. No `*-native-*` fixtures exist.

### Phase 7 — finish
- [ ] `pnpm install` clean
- [x] `pnpm --dir midgard-ts build && pnpm --dir lucid-midgard build && pnpm --dir midgard-sdk build && pnpm --dir midgard-node typecheck` clean (2026-05-14)
- [ ] `pnpm --dir midgard-node test` (emulator) green — currently **132 pass / 2 env logic fail / 30 skipped** after the 2026-05-15 cleanup. Remaining skips: `DESIGN-TODO(dual-domain script discovery)` × 4 + pre-existing non-codec skips; only `database.test.ts` (file-level) and 2 `submit-l2-transfer.test.ts` program tests need a real DB.
- [x] **Test cleanup pushed further (2026-05-15):**
  - Deleted `tests/midgard-native-tx-codec.test.ts` (662 ln) — it tested `@al-ft/midgard-core/codec/native` directly, not midgard-node code. Coverage already lives in `midgard-core/tests/native-codec.test.ts`.
  - Rewrote `tests/validation-parallelization.test.ts` (~330 ln) to construct midgard-ts `Transaction` directly: `OutputReference` for inputs/reference_inputs, `TransactionOutput` for outputs (via `decodeTransactionOutput`), `encodeTransaction`/`transactionId` from midgard-ts. Dropped all `MidgardNativeTxFull`/`encodeMidgardTxBytes`/`midgardTxIdFromNativeFull` references. All 3 tests pass.
  - **Remaining OLD-codec consumer in midgard-node: `tests/native-transaction-integration.test.ts` only** (3700 ln, 150+ OLD-shape touchpoints, 160 `inputOutRef` references). Self-contained but a substantial rewrite — needs its own focused session. Once done, all OLD-codec transitional re-exports can be dropped from `src/midgard-tx-codec/index.ts`.

- [x] **Stale OLD-codec calls in small test files migrated (2026-05-15):** `tests/listen-admission-auth.test.ts`, `tests/merge-error-codes.test.ts`, `tests/phase-a-cardano-signature-bridge.test.ts`, `tests/database.test.ts`, and `tests/validation-parallelization.test.ts` were still calling `cardanoTxBytesToMidgardNativeTxFullBytes` / `decodeMidgardNativeTxFull` / `computeMidgardNativeTxIdFromFull` / `encodeMidgardNativeTxFull` on midgard-ts-shaped data, producing real bugs that were only masked by parallel-vitest test ordering. Swapped to direct midgard-ts (`cardanoTxBytesToMidgardTxBytes` / `decodeTransaction` / `transactionId` / `transactionBodyHash` / `deriveTransactionWitnessSetCompact`) in the 4 read-only sites; `validation-parallelization` builds `MidgardNativeTxFull` literals so it was routed through the validation bridge (`encodeMidgardTxBytes` / `midgardTxIdFromNativeFull`) instead, matching `native-transaction-integration.test.ts`. Also dropped the now-unused `decodeMidgardNativeByteListPreimage` and `decodeMidgardNativeMint` re-exports from `src/midgard-tx-codec/index.ts`.
- [x] `bench:codec:quick` green (2026-05-14, ~67k tx/s serialize, ~370k tx/s deserialize).
- [x] `bench:phase-a:native:quick` green (2026-05-14, ~9.7k tx/s) — rewired to consume midgard-ts wire bytes via `cardanoTxBytesToMidgardTx` + `encodeTransaction` directly, bypassing the OLD-codec `MidgardNativeTxFull` intermediate. Dropped `EMPTY_LIST_ROOT`/`EMPTY_CBOR_LIST` constants + the `computeHash32` import that only served OLD-codec literal-builder fixtures.
- [x] `bench:validation:quick` green (2026-05-14, ~4–8k tx/s combined phase-A+B across sizes 600/700/800/1000/2000/4000). Fix: relaxed `midgard-ts/src/cardano.ts::cmlOutputToMidgard` to also accept Alonzo-format (array-form) outputs alongside Conway-format. Alonzo outputs map to `{address, value, datum: undefined, script_ref: undefined}`; a datum-hash field is still rejected as unrepresentable in Midgard. This lets lucid-evolution Emulator output (which still emits Alonzo array-form) flow through the cardano-ingress path the bench uses.
- [ ] consider deleting now-dead `@al-ft/midgard-core/codec/native.ts` (and its `MidgardNativeTx*` exports) — still load-bearing inside lucid-midgard's builder, so blocked on the Phase 4 "operate on midgard-ts structurally" follow-up.
- [x] Grep stragglers (2026-05-14): `MidgardNativeTx` / `PreimageCbor` / `spendInputsRoot` references are confined to (1) the bridge file itself, (2) lucid-midgard's builder (Phase 4 follow-up), (3) midgard-node's transitional `midgard-tx-codec/index.ts` re-exports, (4) test files exercising the OLD codec as a regression layer, (5) one stale comment in `phase-b.ts:620` (cleaned up).
