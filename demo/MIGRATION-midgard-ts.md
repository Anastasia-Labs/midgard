...
- [ ] **Phase 5-main**: rewrite `phase-a.ts` (689) / `phase-b.ts` (**1,298**) / `script-context.ts` (290) / `midgard-redeemers.ts` (209) / `types.ts` / `ledger.ts` to operate on midgard-ts `Transaction` directly (eliminating the round-trip through `MidgardNativeTxFull`), **or** adopt `@al-ft/midgard-ts/validation` (already has phase-A/phase-B + reject codes against the new types, but a thinner surface — would lose Plutus eval, mint validation, observer hashes, `processedTx` output that the node currently relies on). The current bridged setup works end-to-end but is lossy (`validity` enum → `is_valid: bool` collapses 6 codes into 2) and does a needless materialize→re-encode round-trip on each decode.
- [ ] `midgard-node/src/validation/*.ts` (thin re-export shims) — once phase-a/phase-b are rewritten, repoint.
...
- [ ] **Remaining runtime test status (2026-05-13):** `pnpm test` now reports ~66 tests passing (was ~22 before this session), ~13 failing. Remaining failures cluster around tests in `native-transaction-integration.test.ts` that pre-encode malformed `MidgardVersionedScript` bytes (raw `CML.Script.to_cbor_bytes()` rather than `encodeMidgardVersionedScript({language, bytes})`). With the new bridge, `nativeFullToMidgardTs` re-parses those bytes during encode and rejects them early; the old codec used to pass the bytes verbatim and let phase-B's structural rejection logic catch them. Fixes need either: rewrite the fixtures to use proper `encodeMidgardVersionedScript(...)` envelopes, or have the bridge tolerate / pass-through malformed script bytes. `mpt.test.ts` and `submit-l2-transfer.test.ts` failures need similar inspection (look like test-fixture format drift, not codec bugs).
...
- [ ] `pnpm --dir midgard-node test` (emulator) green — currently ~66 pass / ~13 fail; see Phase 6 entry on the remaining `native-transaction-integration.test.ts` / `submit-l2-transfer.test.ts` / `mpt.test.ts` fixtures
