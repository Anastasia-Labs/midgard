...
- [ ] Test cleanup follow-up: delete the 11 `WONTFIX(post-Phase-5-main)` tests in a focused PR.
...
- [ ] `pnpm --dir midgard-node test` (emulator) green — currently **129 pass / 1 env logic fail / 51 skipped** (the 51 are `WONTFIX(post-Phase-5-main)` × 11 + `DESIGN-TODO(dual-domain script discovery)` × 4 + 36 other pre-existing skips); only `database.test.ts` / `mpt.test.ts` / one `submit-l2-transfer.test.ts` program test need a real DB.
...
- [ ] `bench:validation:quick` — wiring updated to midgard-ts (same approach as phase-a bench), but the fixture generator (`scripts/generate-validation-fixtures.mjs`) emits pre-Conway-format CBOR outputs via lucid-evolution, which `cardanoTxBytesToMidgardTx` rejects (`ConversionError: Only Conway-format (post-Alonzo) transaction outputs are supported in Midgard`). This is a midgard-ts ingress strictness vs. lucid-evolution Emulator output-format mismatch, not a codec migration issue. Two options to fix: (a) relax midgard-ts `cmlOutputToMidgard` to also accept Alonzo-array-form outputs; (b) coerce the fixture generator to emit Conway-format outputs explicitly. Deferred.
