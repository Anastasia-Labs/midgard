...
- [ ] Still TODO: **(a)** rewrite `phase-a.ts` (689) / `phase-b.ts` (**1,298**) / `script-context.ts` (290) / `midgard-redeemers.ts` (209) / `types.ts` / `ledger.ts` to operate on midgard-ts `Transaction` directly, **or (b)** adopt `@al-ft/midgard-ts/validation` (already has phase-A/phase-B + reject codes against the new types) and delete the bespoke modules. Current state works because the boundary-adapter in lucid-midgard converts to/from the core `MidgardNativeTxFull` shape phase-a/phase-b still expect — but the conversion (and the `validity` enum collapse) is lossy/circuitous.
