...
- [ ] `pnpm --dir midgard-node test` (emulator) green — currently **129 pass / 1 env logic fail / 35 skipped** after the WONTFIX cleanup (2026-05-15). Remaining skips: `DESIGN-TODO(dual-domain script discovery)` × 4 + pre-existing non-codec skips; only `database.test.ts` / `mpt.test.ts` / one `submit-l2-transfer.test.ts` program test need a real DB.
