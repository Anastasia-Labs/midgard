...
- [ ] **Phase 5-main / phase-b**: rewrite `phase-b.ts` (**1,298**) / `script-context.ts` (290) / `midgard-redeemers.ts` (209) on midgard-ts `Transaction` directly. Phase-b still calls `decodeMidgardTxBytesToNativeFull` from the bridge at three sites. Reference-script discovery (currently broken under the bridge — drops typed-PlutusV3 / MidgardV1 reference-script context per the `TODO(Phase 6)` skipped tests) should be picked up here. Once phase-b is migrated and the ~16 `it.skip(TODO(Phase 6))` tests are unskipped or deleted, the bridge's `decodeMidgardTxBytesToNativeFull` / `encodeMidgardTxBytes` / `midgardTxIdFromNativeFull` / `midgardTsToNativeFull` exports can be deleted (the body-hash helpers stay until lucid-midgard's builder stops constructing `MidgardNativeTxFull` internally — that's the Phase 4 "operate on midgard-ts structurally" follow-up).
- [ ] `midgard-node/src/validation/*.ts` (thin re-export shims) — already pass through; no change needed once phase-b is rewritten.
