...
export type CommittedFieldShapeCatalogueCategoryV1 = {
...
export const committedFieldShapeStepLabelV1 = (stepIndex: 0 | 1): string =>
  `${COMMITTED_FIELD_SHAPE_CATEGORY_LABEL} step 0${(stepIndex + 1).toString()}`;

export const requireCommittedFieldShapeThreadUtxoV1 = async ({
  lucid,
  contracts,
  categoryId,
  stepIndex,
  ...
  const label = committedFieldShapeStepLabelV1(stepIndex);
...
export const requireCommittedFieldShapeReferenceScriptV1 = ({
  utxo,
  expectedScriptHash,
  stepIndex,
}: {
  ...
  if (utxo.scriptRef == null) {
    throw committedFieldShapeSubmitError(
      `reference UTxO ${outRefLabel(utxo)} for ${committedFieldShapeStepLabelV1(stepIndex)} carries no reference script.`,
    );
  }
  const actual = validatorToScriptHash(utxo.scriptRef);
  if (actual !== expectedScriptHash) {
    throw committedFieldShapeSubmitError(
      `reference script at ${outRefLabel(utxo)} hashes to ${actual}, not the ${committedFieldShapeStepLabelV1(stepIndex)} validator ${expectedScriptHash}.`,
    );
  }
  return utxo;
};

export const requireCommittedFieldShapeStepStateV1 = <State>({
  threadUtxo,
  signer,
  schema,
  stepIndex,
