...
import {
  type InputSetUniquenessCatalogueCategoryV1,
  inputSetUniquenessSubmitError,
} from "./submit-common-v1.js";

type LucidDataSchema = Parameters<typeof Data.to>[1];

export type SubmitInputSetUniquenessInitResult = {
...
export const submitInputSetUniquenessInit = async ({
  ...
  readonly category: InputSetUniquenessCatalogueCategoryV1;
  ...
  const [catalogueUtxo, hubOracleUtxo, fraudulentBlockUtxo] = await Promise.all(
    [
      requireSingletonUtxo({
        ...
        label: `${INPUT_SET_UNIQUENESS_CATEGORY_LABEL} init fraud-proof catalogue`,
      }),
      requireSingletonUtxo({
        lucid,
        ...
        label: `${INPUT_SET_UNIQUENESS_CATEGORY_LABEL} init hub oracle`,
      }),
      fetchUtxoByOutRef({
        lucid,
        outRef: parseOutRef(
        ...
        label: `${INPUT_SET_UNIQUENESS_CATEGORY_LABEL} fraudulent block UTxO`,
      }),
    ],
  );
  ...
  const computationThreadMintCarriage = witnessMintingPolicyCarriageV1({
    ...
    label: `${INPUT_SET_UNIQUENESS_CATEGORY_LABEL} init computation-thread mint`,
  });
  const phasMembershipCarriage = witnessWithdrawalValidatorCarriageV1({
    script: phasMembershipScript,
    referenceUtxo: witnessReferenceScripts?.phasMembershipWithdraw,
    label: `${INPUT_SET_UNIQUENESS_CATEGORY_LABEL} init PHAS membership`,
  });
  ...
  const computationThreadMintRedeemer = ((ctx) => {
    requireOwnMintPurpose(
      ctx,
      contracts.computationThread.policyId,
      `${INPUT_SET_UNIQUENESS_CATEGORY_LABEL} init computation-thread mint`,
    );
    const outputIndex = requireUniqueOutputIndex(
      ctx.outputs,
      firstStepOutputMatches,
      `${INPUT_SET_UNIQUENESS_CATEGORY_LABEL} init first step`,
    );
    ...
    return Data.to(
      {
        Init: {
          ...
          fraud_proof_catalogue_ref_input_index: requireReferenceInputIndex(
            ctx,
            catalogueUtxo,
            `${INPUT_SET_UNIQUENESS_CATEGORY_LABEL} init fraud-proof catalogue`,
          ),
          inclusion_proof_script_redeemer_index: requireWithdrawalRedeemerIndex(
            ctx,
            phasRewardAddress,
            `${INPUT_SET_UNIQUENESS_CATEGORY_LABEL} init PHAS membership`,
          ),
          hub_oracle_ref_input_index: requireReferenceInputIndex(
            ctx,
            hubOracleUtxo,
            `${INPUT_SET_UNIQUENESS_CATEGORY_LABEL} init hub oracle`,
          ),
          fraudulent_block_ref_input_index: requireReferenceInputIndex(
            ctx,
            fraudulentBlockUtxo,
            `${INPUT_SET_UNIQUENESS_CATEGORY_LABEL} init fraudulent block`,
          ),
        },
      },
      FraudProofComputationThreadRedeemer,
  ...
  const resolvedClaimRegistryMutation = requirePreparedClaimRegistryMutationV1({
    ...
    label: `${INPUT_SET_UNIQUENESS_CATEGORY_LABEL} init`,
  });
