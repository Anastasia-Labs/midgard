...
export type SubmitCanonicalDecodabilityInitResult = {
...
export const submitCanonicalDecodabilityInit = async ({
  ...
  const [catalogueUtxo, hubOracleUtxo, fraudulentBlockUtxo] = await Promise.all(
    [
      requireSingletonUtxo({
        ...
        label: `${CANONICAL_DECODABILITY_CATEGORY_LABEL} init fraud-proof catalogue`,
      }),
      requireSingletonUtxo({
        lucid,
        ...
        label: `${CANONICAL_DECODABILITY_CATEGORY_LABEL} init hub oracle`,
      }),
      fetchUtxoByOutRef({
        lucid,
        outRef: parseOutRef(
        ...
        label: `${CANONICAL_DECODABILITY_CATEGORY_LABEL} fraudulent block UTxO`,
      }),
    ],
  );
  ...
  const computationThreadMintRedeemer = ((ctx) => {
    requireOwnMintPurpose(
      ctx,
      contracts.computationThread.policyId,
      `${CANONICAL_DECODABILITY_CATEGORY_LABEL} init computation-thread mint`,
    );
    const outputIndex = requireUniqueOutputIndex(
      ctx.outputs,
      firstStepOutputMatches,
      `${CANONICAL_DECODABILITY_CATEGORY_LABEL} init first step`,
    );
    ...
    return Data.to(
      {
        Init: {
          ...
          fraud_proof_catalogue_ref_input_index: requireReferenceInputIndex(
            ctx,
            catalogueUtxo,
            `${CANONICAL_DECODABILITY_CATEGORY_LABEL} init catalogue`,
          ),
          inclusion_proof_script_redeemer_index: requireWithdrawalRedeemerIndex(
            ctx,
            phasRewardAddress,
            `${CANONICAL_DECODABILITY_CATEGORY_LABEL} init PHAS membership`,
          ),
          hub_oracle_ref_input_index: requireReferenceInputIndex(
            ctx,
            hubOracleUtxo,
            `${CANONICAL_DECODABILITY_CATEGORY_LABEL} init hub oracle`,
          ),
          fraudulent_block_ref_input_index: requireReferenceInputIndex(
            ctx,
            fraudulentBlockUtxo,
            `${CANONICAL_DECODABILITY_CATEGORY_LABEL} init fraudulent block`,
          ),
        },
      },
      FraudProofComputationThreadRedeemer,
