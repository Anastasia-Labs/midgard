/**
 * `missing-signature` thread cancellation (offchain plan §7.1).
 *
...
export type SubmitMissingSignatureCancelResult = {
...
export const submitMissingSignatureCancel = async ({
  lucid,
  contracts,
  categoryId,
  signer,
  ...
  const resolvedClaimRegistryMutation = requirePreparedClaimRegistryMutationV1({
    ...
    kind: "cancel",
    claimId: threadToken.assetName,
    label: `${MISSING_SIGNATURE_CATEGORY_LABEL} cancel`,
  });
