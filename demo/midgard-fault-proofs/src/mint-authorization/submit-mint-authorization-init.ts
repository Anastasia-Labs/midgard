/**
 * `mint-authorization` thread init.
 *
 ...
 * production `submitInit` category union entry is parent-owned and lands at
 * registration.
 */
import {
...
export type SubmitMintAuthorizationInitResult = {
...
export const submitMintAuthorizationInit = async ({
  lucid,
  blueprint,
  network,
  contracts,
  ...
  const [catalogueUtxo, hubOracleUtxo, fraudulentBlockUtxo] = await Promise.all(
    [
      requireSingletonUtxo({
        ...
        label: `${MINT_AUTHORIZATION_CATEGORY_LABEL} init fraud-proof catalogue`,
      }),
      requireSingletonUtxo({
        lucid,
        ...
        label: `${MINT_AUTHORIZATION_CATEGORY_LABEL} init hub oracle`,
      }),
      fetchUtxoByOutRef({
        lucid,
        outRef: parseOutRef(
        ...
        label: `${MINT_AUTHORIZATION_CATEGORY_LABEL} fraudulent block UTxO`,
      }),
    ],
  );
  ...
  const computationThreadMintRedeemer = ((ctx) => {
    requireOwnMintPurpose(
      ctx,
      contracts.computationThread.policyId,
      `${MINT_AUTHORIZATION_CATEGORY_LABEL} init computation-thread mint`,
    );
    const outputIndex = requireUniqueOutputIndex(
      ctx.outputs,
      firstStepOutputMatches,
      `${MINT_AUTHORIZATION_CATEGORY_LABEL} init first step`,
    );
    ...
    return Data.to(
      {
        Init: {
          ...
          fraud_proof_catalogue_ref_input_index: requireReferenceInputIndex(
            ctx,
            catalogueUtxo,
            `${MINT_AUTHORIZATION_CATEGORY_LABEL} init fraud-proof catalogue`,
          ),
          inclusion_proof_script_redeemer_index: requireWithdrawalRedeemerIndex(
            ctx,
            phasRewardAddress,
            `${MINT_AUTHORIZATION_CATEGORY_LABEL} init PHAS membership`,
          ),
          hub_oracle_ref_input_index: requireReferenceInputIndex(
            ctx,
            hubOracleUtxo,
            `${MINT_AUTHORIZATION_CATEGORY_LABEL} init hub oracle`,
          ),
          fraudulent_block_ref_input_index: requireReferenceInputIndex(
            ctx,
            fraudulentBlockUtxo,
            `${MINT_AUTHORIZATION_CATEGORY_LABEL} init fraudulent block`,
          ),
        },
      },
      FraudProofComputationThreadRedeemer,
  ...
  const resolvedClaimRegistryMutation = requirePreparedClaimRegistryMutationV1({
    ...
    label: `${MINT_AUTHORIZATION_CATEGORY_LABEL} init`,
  });
