...
export type DistinctAssetFoldAction =
  | Readonly<{ kind: "skip" }>
  | Readonly<{
      kind: "authenticate";
      evidence: Readonly<{
...
export const submitDistinctAssetAccumulationFold = async ({
  lucid,
  contracts,
  categoryId,
  signer,
  ...
  readonly action: DistinctAssetFoldAction;
  ...
  const state = requireLinearFaultStepState<FoldState>({
    threadUtxo,
    signer,
    schema: DistinctAssetStep03DatumSchema as never,
    family: "distinct-asset-accumulation-limit",
    stepIndex,
  });
