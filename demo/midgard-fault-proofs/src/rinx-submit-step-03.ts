...
// Step 03 binds the *producing* transaction of the offending reference input by
// the block's native transaction root — the same inclusion path as step 01.
export type RinxStep03TxInclusion = SubmitStep01TxInclusion;
export const parseRinxStep03TxInclusion = parseSubmitStep01TxInclusion;

export type RinxSubmitStep03CliConfig = SubmitProviderConfig & {
...
export type RinxSubmitStep03Result = {
...
type Step03DatumWithState = ReferenceInputNoIdxStep03Datum & {
...
const requireStep03Datum = ({
  ...
  if (datum.data === null) {
    throw new Error(
      "Step 03 input datum must carry bad-reference-input tx-id/output-index state data.",
    );
  }
  return datum as Step03DatumWithState;
};

export const rinxSubmitStep03 = async ({
  lucid,
  blueprint,
  deploymentInfo,
  network,
  ...
  readonly threadOutRef: string;
  readonly stateQueueBlockOutRef: string;
  readonly txInclusion: RinxStep03TxInclusion;
  readonly awaitConfirmation?: boolean;
}): Promise<RinxSubmitStep03Result> => {
  ...
  if (txInclusion.nativeTxId !== inputDatum.data.bad_reference_input_tx_id) {
    throw new Error(
      `Producing tx inclusion is for ${txInclusion.nativeTxId}, but step-03 datum expects the bad reference input's producing tx ${inputDatum.data.bad_reference_input_tx_id}.`,
    );
  }
  ...
  let resolvedLayout: RinxStep03Layout | undefined;
  const redeemer = ((ctx) => {
    requireOwnSpendPurpose(ctx, threadUtxo, "reference-input-no-idx step 03");
    const layout: RinxStep03Layout = {
  ...
  return {
    ...
    badReferenceInputOutputIndex: Number(
      inputDatum.data.bad_reference_input_output_index,
    ),
...
export const rinxSubmitStep03FromFiles = async (
  config: RinxSubmitStep03CliConfig,
): Promise<RinxSubmitStep03Result> => {
  ...
  return await rinxSubmitStep03({
    ...
    txInclusion: parseRinxStep03TxInclusion(txInclusionJson),
