...
import {
  type DoubleWithdrawCatalogueCategoryV1,
...
export const submitDoubleWithdrawInit = async ({
  ...
  readonly category: DoubleWithdrawCatalogueCategoryV1;
  readonly catalogue: {
  ...
  const [catalogueUtxo, hubOracleUtxo, fraudulentBlockUtxo] = await Promise.all(
    [
      requireSingletonUtxo({
        ...
        label: `${DOUBLE_WITHDRAW_CATEGORY_LABEL} init fraud-proof catalogue`,
      }),
      requireSingletonUtxo({
        lucid,
        ...
        label: `${DOUBLE_WITHDRAW_CATEGORY_LABEL} init hub oracle`,
      }),
      fetchUtxoByOutRef({
        lucid,
        outRef: parseOutRef(
        ...
        label: `${DOUBLE_WITHDRAW_CATEGORY_LABEL} fraudulent block UTxO`,
      }),
    ],
  );
  ...
  const computationThreadMintCarriage = witnessMintingPolicyCarriageV1({
    ...
    label: `${DOUBLE_WITHDRAW_CATEGORY_LABEL} init computation-thread mint`,
  });
  const phasMembershipCarriage = witnessWithdrawalValidatorCarriageV1({
    script: phasMembershipScript,
    referenceUtxo: witnessReferenceScripts?.phasMembershipWithdraw,
    label: `${DOUBLE_WITHDRAW_CATEGORY_LABEL} init PHAS membership`,
  });
  ...
  const mintRedeemer = ((ctx) => {
    requireOwnMintPurpose(
      ctx,
      contracts.computationThread.policyId,
      `${DOUBLE_WITHDRAW_CATEGORY_LABEL} init computation-thread mint`,
    );
    const outputIndex = requireUniqueOutputIndex(
      ctx.outputs,
      outputMatches,
      `${DOUBLE_WITHDRAW_CATEGORY_LABEL} init first step`,
    );
    ...
    return Data.to(
      {
        Init: {
          ...
          fraud_proof_catalogue_ref_input_index: requireReferenceInputIndex(
            ctx,
            catalogueUtxo,
            `${DOUBLE_WITHDRAW_CATEGORY_LABEL} init catalogue`,
          ),
          inclusion_proof_script_redeemer_index: requireWithdrawalRedeemerIndex(
            ctx,
            phasRewardAddress,
            `${DOUBLE_WITHDRAW_CATEGORY_LABEL} init PHAS membership`,
          ),
          hub_oracle_ref_input_index: requireReferenceInputIndex(
            ctx,
            hubOracleUtxo,
            `${DOUBLE_WITHDRAW_CATEGORY_LABEL} init hub oracle`,
          ),
          fraudulent_block_ref_input_index: requireReferenceInputIndex(
            ctx,
            fraudulentBlockUtxo,
            `${DOUBLE_WITHDRAW_CATEGORY_LABEL} init fraudulent block`,
          ),
        },
      },
      FraudProofComputationThreadRedeemer,
