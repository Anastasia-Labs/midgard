 ...
 * module re-derives, from the **on-chain** state-queue block header, the
 * counted `transactions_root` over the supplied raw PHAS root and the header's
 * own `l2TransactionCount`, and requires it to equal the committed
 * `transactionsRoot`. The bad transaction is also re-decoded from its canonical
 * compact CBOR, so the `spend_inputs_hash` forwarded to step 02 is the
...
export type SubmitInputNoIdxStep01Result = {
...
export const submitInputNoIdxStep01 = async ({
  lucid,
  blueprint,
  deploymentInfo,
  network,
  ...
  let resolvedLayout: InputNoIdxStep01Layout | undefined;
  const redeemer = ((ctx) => {
    requireOwnSpendPurpose(ctx, threadUtxo, "input-no-idx step 01");
    const layout: InputNoIdxStep01Layout = {
  ...
  const expectedTxHash = await reachFraudProofPreSubmitBoundary({
    ...
    referenceScripts: workflowReferenceScriptsUsedByTransaction({
      ...
      candidates: [
        {
          role: "V1 fraud-proof non-existent-input-no-index step-01",
          utxo: referenceScriptUtxo,
          expectedScript:
            contracts.nonExistentInputNoIndex.steps[0].spendingScript,
        },
  ...
  if (txHash !== expectedTxHash) {
    throw new Error(
      `input-no-idx step-01 provider returned ${txHash}, expected ${expectedTxHash}.`,
    );
  }
...
export const submitInputNoIdxStep01FromFiles = async (
  config: SubmitInputNoIdxStep01CliConfig,
): Promise<SubmitInputNoIdxStep01Result> => {
  ...
  return await submitInputNoIdxStep01({
    lucid,
    blueprint,
    deploymentInfo,
