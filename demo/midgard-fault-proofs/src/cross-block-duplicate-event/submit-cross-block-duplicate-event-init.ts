/**
 * `cross-block-duplicate-event` thread init (offchain plan §5).
 *
 * Uses the generic catalogue/thread init protocol of `src/submit-init.ts` — catalogue, hub-oracle
...
export const submitCrossBlockDuplicateEventInit = async ({
  lucid,
  blueprint,
  network,
  contracts,
  ...
  const [catalogueUtxo, hubOracleUtxo, fraudulentBlockUtxo] = await Promise.all(
    [
      requireSingletonUtxo({
        ...
        label: `${CROSS_BLOCK_DUPLICATE_EVENT_CATEGORY_LABEL} init fraud-proof catalogue`,
      }),
      requireSingletonUtxo({
        lucid,
        ...
        label: `${CROSS_BLOCK_DUPLICATE_EVENT_CATEGORY_LABEL} init hub oracle`,
      }),
      fetchUtxoByOutRef({
        lucid,
        outRef: parseOutRef(
        ...
        label: `${CROSS_BLOCK_DUPLICATE_EVENT_CATEGORY_LABEL} fraudulent block UTxO`,
      }),
    ],
  );
  ...
  const computationThreadMintRedeemer = ((ctx) => {
    requireOwnMintPurpose(
      ctx,
      contracts.computationThread.policyId,
      `${CROSS_BLOCK_DUPLICATE_EVENT_CATEGORY_LABEL} init computation-thread mint`,
    );
    const outputIndex = requireUniqueOutputIndex(
      ctx.outputs,
      firstStepOutputMatches,
      `${CROSS_BLOCK_DUPLICATE_EVENT_CATEGORY_LABEL} init first step`,
    );
    ...
    return Data.to(
      {
        Init: {
          ...
          fraud_proof_catalogue_ref_input_index: requireReferenceInputIndex(
            ctx,
            catalogueUtxo,
            `${CROSS_BLOCK_DUPLICATE_EVENT_CATEGORY_LABEL} init fraud-proof catalogue`,
          ),
          inclusion_proof_script_redeemer_index: requireWithdrawalRedeemerIndex(
            ctx,
            phasRewardAddress,
            `${CROSS_BLOCK_DUPLICATE_EVENT_CATEGORY_LABEL} init PHAS membership`,
          ),
          hub_oracle_ref_input_index: requireReferenceInputIndex(
            ctx,
            hubOracleUtxo,
            `${CROSS_BLOCK_DUPLICATE_EVENT_CATEGORY_LABEL} init hub oracle`,
          ),
          fraudulent_block_ref_input_index: requireReferenceInputIndex(
            ctx,
            fraudulentBlockUtxo,
            `${CROSS_BLOCK_DUPLICATE_EVENT_CATEGORY_LABEL} init fraudulent block`,
          ),
        },
      },
      FraudProofComputationThreadRedeemer,
  ...
  const resolvedClaimRegistryMutation = requirePreparedClaimRegistryMutationV1({
    ...
    label: `${CROSS_BLOCK_DUPLICATE_EVENT_CATEGORY_LABEL} init`,
  });
