...
export const buildExecutionSourceMachineAuthenticationV1 = async ({
  ...
  if (stateIndex < 0)
    throw new Error(
      "missingScriptSource replay has no native execution descriptor",
    );
  ...
  if (
    ...
    throw new Error("missingScriptSource selected source is not native");
  ...
  if (selected.length !== 1 || !selected[0]!.value.equals(valueCbor))
    throw new Error(
      "missingScriptSource retained validation descriptor changed",
    );
  ...
  if (
    ...
    throw new Error("missingScriptSource retained validation root changed");
