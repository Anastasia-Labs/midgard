...
import type { PreparedNoReferenceInputWrongfulRejection } from "./wrongful-rejection.js";

export type NoReferenceInputForcedContracts = LinearFaultContracts & {
...
export const noReferenceInputForcedFieldPlan = (
  prepared: PreparedNoReferenceInputWrongfulRejection,
  owner: string,
) =>
  planFaultProofFieldOpening({
    fieldIndex: 1,
    ...
    owner,
    label: "noReferenceInput forced field one",
  });

export const noReferenceInputForcedStates = (
  prepared: PreparedNoReferenceInputWrongfulRejection,
) => {
...
export const submitNoReferenceInputForcedStep = async ({
  lucid,
  contracts,
  categoryId,
  signer,
  ...
  contracts: NoReferenceInputForcedContracts;
  categoryId: string;
  ...
  await admitNoReferenceInputForcedArtifact(
    noReferenceInputForcedArtifact(prepared),
  );
  const family = "no-reference-input";
  ...
  const states = noReferenceInputForcedStates(prepared);
  const datumSchemas = [
    SDK.NoReferenceInputStep02ThreadDatum,
    SDK.NoReferenceInputStep03ThreadDatum,
    SDK.NoReferenceInputStep04ThreadDatum,
  ] as const;
  if (stepIndex === 0) requireInitialStepDatum({ threadUtxo, signer });
    ...
    if (
      ...
      throw new Error("noReferenceInput: thread state changed");
  }
  ...
  if (stepIndex !== 3) {
    ...
    const redeemer = ((ctx) => {
      ...
      if (stepIndex === 0)
        return Data.to(
          ...
          SDK.NoReferenceInputStep01SpendRedeemer as never,
        );
      if (stepIndex === 1)
        return Data.to(
          {
          ...
          SDK.NoReferenceInputStep02ThreadSpendRedeemer,
        );
      return Data.to(
        {
          Continue: [
        ...
        SDK.NoReferenceInputStep03ThreadSpendRedeemer,
      );
    ...
    if (outputIndex === undefined)
      throw new Error("noReferenceInput: unresolved output");
  ...
  const spend = ((ctx) => {
    ...
    return Data.to(
      ...
      SDK.NoReferenceInputStep04ThreadSpendRedeemer,
    );
  }) satisfies BuildTxWithRedeemer;
  const proofMint = ((ctx) => {
  ...
  if (txHash !== expected)
    throw new Error("noReferenceInput: submitted transaction hash differs");
