...
import {
  type NoReferenceInputForcedSource,
...
export const NO_REFERENCE_INPUT_FORCED_ARTIFACT =
  "midgard-no-reference-input-forced-artifact-v1";
export const noReferenceInputForcedArtifact = (
  prepared: PreparedNoReferenceInputWrongfulRejection,
): JournalJsonObject =>
  normalizeJournalJson({
    schemaVersion: NO_REFERENCE_INPUT_FORCED_ARTIFACT,
...
export const admitNoReferenceInputForcedArtifact = async (
  input: unknown,
): Promise<PreparedNoReferenceInputWrongfulRejection> => {
  if (typeof input !== "object" || input === null || Array.isArray(input))
    throw new Error("noReferenceInput: malformed artifact");
  ...
  if (
    ...
    artifact.schemaVersion !== NO_REFERENCE_INPUT_FORCED_ARTIFACT
  )
    throw new Error("noReferenceInput: artifact shape changed");
  const hex = (key: string) => {
    const value = artifact[key];
    if (typeof value !== "string" || !/^(?:[0-9a-f]{2})+$/u.test(value))
      throw new Error(`noReferenceInput: malformed ${key}`);
    return value;
  };
  const forcedSource = Data.from(
    hex("source"),
    SDK.NoReferenceInputForcedSourcePayloadSchema as never,
  ) as NoReferenceInputForcedSource;
  ...
  if (headerHash !== hex("headerHash") || header.protocolVersion !== 1n)
    throw new Error("noReferenceInput: header changed");
  ...
  const counted = async <K, V>(
    ...
    if (
      ...
      throw new Error("noReferenceInput: counted root changed");
    verifyMpf({
  ...
  if (
    ...
    throw new Error("noReferenceInput: event/transition coordinate changed");
  ...
  if (material.selectedInput === null) {
    if (artifact.ledger !== null)
      throw new Error(
        "noReferenceInput: impossible index carries unrelated witness",
      );
  } else {
    if (
      ...
      throw new Error("noReferenceInput: ledger membership absent");
    ...
    if (
      ...
      throw new Error("noReferenceInput: malformed ledger witness");
    ledgerMembership = {
      value: ledger.value,
