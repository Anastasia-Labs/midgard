...
export type WithdrawnReferenceInputCatalogueCategoryV1 = {
...
export const withdrawnReferenceInputStepLabelV1 = (stepIndex: 0 | 1 | 2) =>
  `${WITHDRAWN_REFERENCE_INPUT_CATEGORY_LABEL} step 0${(stepIndex + 1).toString()}`;

export const requireWithdrawnReferenceInputThreadUtxoV1 = async ({
  lucid,
  contracts,
  categoryId,
  stepIndex,
  ...
  const label = withdrawnReferenceInputStepLabelV1(stepIndex);
...
export const requireWithdrawnReferenceInputReferenceScriptV1 = ({
  utxo,
  expectedScriptHash,
  stepIndex,
}: {
  ...
  if (utxo.scriptRef == null) {
    throw withdrawnReferenceInputSubmitError(
      `reference UTxO ${outRefLabel(utxo)} for ${withdrawnReferenceInputStepLabelV1(stepIndex)} carries no reference script.`,
    );
  }
  const actual = validatorToScriptHash(utxo.scriptRef);
  if (actual !== expectedScriptHash) {
    throw withdrawnReferenceInputSubmitError(
      `reference script at ${outRefLabel(utxo)} hashes to ${actual}, not the ${withdrawnReferenceInputStepLabelV1(stepIndex)} validator ${expectedScriptHash}.`,
    );
  }
  return utxo;
};

export const requireWithdrawnReferenceInputStepStateV1 = <State>({
  threadUtxo,
  signer,
  schema,
  stepIndex,
