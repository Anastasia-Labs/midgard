...
type BoundConfigV1 = Readonly<{
  ...
  claimRegistry: ProductionClaimRegistryPrerequisiteV1<"nativeScriptInvalid">;
...
export const createManifestBoundNativeScriptInvalidWorkflowV1 = async (
  ...
  const claimRegistry = createProductionClaimRegistryPrerequisiteV1({
    ...
    mutationForAction: ({ action, artifact }) => {
      ...
      if (input.stage === "init") return { kind: "open" };
      if (input.stage === "step_05") return { kind: "close" };
      if (input.stage === "step_03") {
        return isDirect(admitProductionNativeScriptInvalidArtifactV1(artifact))
          ? { kind: "close" }
          : null;
      }
      return null;
