...
// Reference-input-no-idx step 01 binds the bad transaction by the block's
// native transaction root — the same inclusion path as input-no-idx step 01 —
// so the tx-inclusion material is identical to `submit-step-01`'s. The only
// difference is which body hash is threaded forward: the reference-inputs hash
// instead of the spend-inputs hash.
export type RinxStep01TxInclusion = SubmitStep01TxInclusion;
export const parseRinxStep01TxInclusion = parseSubmitStep01TxInclusion;

export type RinxSubmitStep01CliConfig = SubmitProviderConfig & {
...
export type RinxSubmitStep01Result = {
...
export const rinxSubmitStep01 = async ({
  lucid,
  blueprint,
  deploymentInfo,
  network,
  ...
  readonly threadOutRef: string;
  readonly stateQueueBlockOutRef: string;
  readonly txInclusion: RinxStep01TxInclusion;
  readonly awaitConfirmation?: boolean;
}): Promise<RinxSubmitStep01Result> => {
  ...
  let resolvedLayout: RinxStep01Layout | undefined;
  const redeemer = ((ctx) => {
    requireOwnSpendPurpose(ctx, threadUtxo, "reference-input-no-idx step 01");
    const layout: RinxStep01Layout = {
...
export const rinxSubmitStep01FromFiles = async (
  config: RinxSubmitStep01CliConfig,
): Promise<RinxSubmitStep01Result> => {
  ...
  return await rinxSubmitStep01({
    ...
    txInclusion: parseRinxStep01TxInclusion(txInclusionJson),
