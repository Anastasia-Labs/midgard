...
export const submitMissingNativeScriptTxInit = async ({
  lucid,
  blueprint,
  network,
  contracts,
  ...
  const [catalogueUtxo, hubOracleUtxo, fraudulentBlockUtxo] = await Promise.all(
    [
      requireSingletonUtxo({
        ...
        label: `${MISSING_NATIVE_SCRIPT_TX_CATEGORY_LABEL} init catalogue`,
      }),
      requireSingletonUtxo({
        lucid,
        ...
        label: `${MISSING_NATIVE_SCRIPT_TX_CATEGORY_LABEL} init hub oracle`,
      }),
      fetchUtxoByOutRef({
        lucid,
        outRef: parseOutRef(
        ...
        label: `${MISSING_NATIVE_SCRIPT_TX_CATEGORY_LABEL} fraudulent block`,
      }),
    ],
  );
  ...
  const computationThreadMintRedeemer = ((ctx) => {
    requireOwnMintPurpose(
      ctx,
      contracts.computationThread.policyId,
      `${MISSING_NATIVE_SCRIPT_TX_CATEGORY_LABEL} init thread mint`,
    );
    const outputIndex = requireUniqueOutputIndex(
      ctx.outputs,
      firstStepOutputMatches,
      `${MISSING_NATIVE_SCRIPT_TX_CATEGORY_LABEL} init first step`,
    );
    ...
    return Data.to(
      {
        Init: {
          ...
          fraud_proof_catalogue_ref_input_index: requireReferenceInputIndex(
            ctx,
            catalogueUtxo,
            `${MISSING_NATIVE_SCRIPT_TX_CATEGORY_LABEL} init catalogue`,
          ),
          inclusion_proof_script_redeemer_index: requireWithdrawalRedeemerIndex(
            ctx,
            phasRewardAddress,
            `${MISSING_NATIVE_SCRIPT_TX_CATEGORY_LABEL} init PHAS membership`,
          ),
          hub_oracle_ref_input_index: requireReferenceInputIndex(
            ctx,
            hubOracleUtxo,
            `${MISSING_NATIVE_SCRIPT_TX_CATEGORY_LABEL} init hub oracle`,
          ),
          fraudulent_block_ref_input_index: requireReferenceInputIndex(
            ctx,
            fraudulentBlockUtxo,
            `${MISSING_NATIVE_SCRIPT_TX_CATEGORY_LABEL} init block`,
          ),
        },
      },
      FraudProofComputationThreadRedeemer,
  ...
  const resolvedClaimRegistryMutation = requirePreparedClaimRegistryMutationV1({
    ...
    label: `${MISSING_NATIVE_SCRIPT_TX_CATEGORY_LABEL} init`,
  });
