...
import type {
  ...
  FieldItemWidthJournal,
  FieldItemWidthJournalEntry,
  FieldItemWidthStage,
} from "./field-item-width-illegal-v1.js";

type SubmitAction = Exclude<FieldItemWidthAction, "done">;
...
export const createFieldItemWidthIllegalCentralJournalAdapter = ({
  store,
  ...
  const familyJournal: FieldItemWidthJournal = {
    load: async (familyIdentity) => {
      const current = await entries();
      const result: FieldItemWidthJournalEntry[] = [];
