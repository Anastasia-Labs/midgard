...
afterAll(async () => {
  ...
  const evidence = {
    category: "validationTraceDispute/ScriptSources discovery",
  ...
  writeFileSync(
    "/tmp/nip-script-sources-discovery-measurements.json",
    JSON.stringify(
  ...
  await writeVanRossemFitLedger(
    fileURLToPath(
      new URL(
        "../../../docs/fault-proofs/size-plans/validation-trace-script-sources-discovery-fit-ledger.json",
        import.meta.url,
      ),
    ),
...
const slots = [16, 17, 18, 23, 24, 27] as const;
