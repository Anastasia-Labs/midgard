...
import { createResolvedOutputNonCanonicalCentralJournalAdapter } from "../src/resolved-output-non-canonical/central-journal-v1.js";
import type {
...
const adapter = (
  ...
  createResolvedOutputNonCanonicalCentralJournalAdapter({
    store: value,
...
describe("resolvedOutputNonCanonical central journal adapter", () => {
  ...
  it("reconciles an exact submitted transaction after a directory-backed process restart", async () => {
    ...
    try {
      ...
      await expect(
        ...
        expect.objectContaining({ stage: "step01", txHash }),
      ]);
    } finally {
