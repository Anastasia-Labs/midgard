...
import {
  ...
  detectMintItemNonCanonicalCompleteReplay,
...
const references = (): MintItemNonCanonicalReferenceScripts => ({
  step01: utxo("1", 0),
  step02: utxo("2", 1),
  step03: utxo("3", 2),
  step04: utxo("4", 3),
...
const suppliedByName = (
  supplied: MintItemNonCanonicalReferenceScripts,
): ReadonlyMap<string, UTxO> =>
  new Map([
    [MINT_ITEM_NON_CANONICAL_MANIFEST_CONTRACTS.step01, supplied.step01],
    [MINT_ITEM_NON_CANONICAL_MANIFEST_CONTRACTS.step02, supplied.step02],
    [MINT_ITEM_NON_CANONICAL_MANIFEST_CONTRACTS.step03, supplied.step03],
    [MINT_ITEM_NON_CANONICAL_MANIFEST_CONTRACTS.step04, supplied.step04],
    [
      MINT_ITEM_NON_CANONICAL_MANIFEST_CONTRACTS.computationThreadMint,
      supplied.witnesses.computationThreadMint,
    ],
    [
      MINT_ITEM_NON_CANONICAL_MANIFEST_CONTRACTS.fraudProofMint,
      supplied.witnesses.fraudProofMint,
    ],
    [
      MINT_ITEM_NON_CANONICAL_MANIFEST_CONTRACTS.phasMembershipWithdraw,
      supplied.witnesses.phasMembershipWithdraw,
    ],
    [
      MINT_ITEM_NON_CANONICAL_MANIFEST_CONTRACTS.fieldPreimageCertificateMint,
...
describe("mintItemNonCanonical production workflow", () => {
  it("runs only its own catalogue category, and refuses a foreign one before loading any runtime configuration", async () => {
    ...
    const runner = createMintItemNonCanonicalWorkflowRunnerSurface({
    ...
    await expect(
      ...
      /mintItemNonCanonical production runner category mismatch: unusedRedeemer/u,
    );
  ...
  it("names the deployment registry's step contracts in step order", () => {
    ...
    expect([
      MINT_ITEM_NON_CANONICAL_MANIFEST_CONTRACTS.step01,
      MINT_ITEM_NON_CANONICAL_MANIFEST_CONTRACTS.step02,
      MINT_ITEM_NON_CANONICAL_MANIFEST_CONTRACTS.step03,
      MINT_ITEM_NON_CANONICAL_MANIFEST_CONTRACTS.step04,
    ]).toEqual(FRAUD_PROOF_DEPLOYMENT_ENTRIES_BY_CATEGORY.mintItemNonCanonical);
  });
  ...
  it("binds reference out-refs and script identities against the manifest", () => {
    ...
    expect(() =>
      ...
      /fraudProofMintItemNonCanonicalStep02 reference UTxO script differs from finalized manifest identity/u,
    );
  ...
  it("scans every retained field-5 mint item and derives the sole exact violation", () => {
    ...
    const canonical = Buffer.from(`82581c${"11".repeat(28)}a14001`, "hex");
    const malformed = Buffer.from(`82581c${"22".repeat(28)}a14000`, "hex");
    const blockWithOutputs = (outputs: readonly Buffer[]) => {
      const fullTx = materializeMidgardNativeTxFromCanonical({
        ...material.fullTx!,
        body: {
          ...material.fullTx!.body,
          mintPreimageCbor: encodeMidgardFieldPreimage([...outputs]),
        },
      });
    ...
    const detections = detectMintItemNonCanonicalCompleteReplay(block as never);
    ...
    expect(detections[0]).toEqual({
      detectionId: `${MINT_ITEM_NON_CANONICAL_VIOLATION_ID}:0:${nodeTxId}:1`,
      headerHash: "a".repeat(56),
      violationId: MINT_ITEM_NON_CANONICAL_VIOLATION_ID,
      position: 0n,
      diagnostic: `transaction ${nodeTxId} mint policy item 1 violates the field-5 grammar or policy ordering`,
    });

    const evidence = deriveMintItemNonCanonicalEvidenceFromCanonicalBlock(
      block as never,
    );
    expect(evidence.itemIndex).toBe(1);
    expect(evidence.fieldIndex).toBe(5);
    ...
    const honest = blockWithOutputs([canonical]);
    expect(
      detectMintItemNonCanonicalCompleteReplay(honest.block as never),
    ).toEqual([]);
    expect(() =>
      deriveMintItemNonCanonicalEvidenceFromCanonicalBlock(
        honest.block as never,
      ),
    ).toThrow(/contains no finding/u);

    const pair = blockWithOutputs([malformed, malformed]);
    expect(
      detectMintItemNonCanonicalCompleteReplay(pair.block as never),
    ).toHaveLength(2);
    expect(
      deriveMintItemNonCanonicalEvidenceFromCanonicalBlock(pair.block as never)
        .itemIndex,
    ).toBe(0);
  });
  ...
  it("resolves each physical checkpoint against the authenticated raw-L1 stage", async () => {
    ...
    ): ReturnType<typeof createMintItemNonCanonicalRawL1StageResolver> =>
      createMintItemNonCanonicalRawL1StageResolver({
