...
afterAll(async () => {
  ...
  const evidence = {
    category: "validationTraceDispute/ScriptSources middle yields",
  ...
  writeFileSync(
    "/tmp/nip-script-sources-middle-yields-measurements.json",
    JSON.stringify(
  ...
  await writeVanRossemFitLedger(
    fileURLToPath(
      new URL(
        "../../../docs/fault-proofs/size-plans/validation-trace-script-sources-middle-yields-fit-ledger.json",
        import.meta.url,
      ),
    ),
