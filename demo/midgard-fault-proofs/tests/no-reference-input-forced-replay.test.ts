...
import { prepareNoReferenceInputWrongfulRejection } from "../src/no-reference-input/wrongful-rejection.js";
...
describe("noReferenceInput installed forced replay", () => {
  it("uses the forced event pre-state after an earlier forced spend", async () => {
    ...
    expect(
      (
        await NO_REFERENCE_INPUT_COMPLETE_CANONICAL_REPLAY.replay(
          evidence,
          context,
        )
      ).detections,
    ...
    await expect(
      prepareNoReferenceInputWrongfulRejection({
        block: evidence,
        forcedIndex: 1,
        predecessor,
      }),
    ...
    async (index) => {
      ...
      expect(
        await admitNoReferenceInputForcedArtifact(
          JSON.parse(JSON.stringify(noReferenceInputForcedArtifact(prepared))),
        ),
      ).toEqual(prepared);
    },
  );
