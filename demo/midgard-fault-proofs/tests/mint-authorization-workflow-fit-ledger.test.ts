...
describe("mintAuthorization installed workflow fit ledger", () => {
  it("binds complete maximum evidence and publication margins to the current blueprint", async () => {
    ...
    for (const stage of [
      ...
      expect(
        [...names].some((name) =>
          name.startsWith(`maximum-selected-assets/${stage}/`),
        ),
      ).toBe(true);
    for (let index = 0; index < 13; index++)
      expect(names.has(`maximum-selected-assets/publish/${index}`)).toBe(true);
    expect(
      ledger.entries.filter((row) =>
        row.name.startsWith("maximum-selected-assets/step_02/"),
      ).length,
    ).toBe(258);
