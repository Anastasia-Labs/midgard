...
describe("distinctAssetAccumulationLimit signed publication fit", () => {
  it("publishes all six applied scripts below the reliability reserve", async () => {
    ...
    expect(
      DISTINCT_ASSET_ACCUMULATION_LIMIT_BLUEPRINT_TITLES.filter(
        (title) =>
          !blueprint.validators.some((validator) => validator.title === title),
      ),
      "distinctAssetAccumulationLimit validators missing from the rebuilt blueprint",
    ).toEqual([]);
    ...
    expect(steps, "applied distinctAssetAccumulationLimit steps").toHaveLength(
      expectedStepCount,
    );
