...
import { createProtectedOutputSignerMissingCentralJournalAdapterV1 } from "../src/protected-output-signer-missing/central-journal-v1.js";
import type {
...
describe("protectedOutputSignerMissing central journal adapter", () => {
  ...
  it("reconciles an exact submitted transaction after a directory-backed process restart", async () => {
    const directory = await mkdtemp(
      join(tmpdir(), "midgard-protected-output-signer-journal-"),
    );
    try {
      const first = adapter(
