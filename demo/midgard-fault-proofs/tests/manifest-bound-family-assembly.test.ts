...
import {
  ...
  type MigratedLinearFamilyCategory,
} from "../src/workflow/linear-family-definitions.js";
...
const categories = Object.keys(
  LINEAR_FAMILY_DEFINITIONS,
) as MigratedLinearFamilyCategory[];
 ...
 * stay real.
 */
const fixture = (category: MigratedLinearFamilyCategory) => {
  const definition = LINEAR_FAMILY_DEFINITIONS[category];
  const spec = linearFamilySpec(category);
  const address = credentialToAddress("Preprod", { type: "Key", hash: OWNER });
  const names = [
    ...spec.steps.map((step) => step.manifestContractName),
  ...
  const steps = spec.steps.map(
    (step) => references.get(step.manifestContractName)!,
  );
...
describe("manifest-bound family assembly", () => {
  it("covers every migrated linear definition", () => {
    expect(categories.length).toBeGreaterThan(0);
    for (const category of categories) {
      expect(LINEAR_FAMILY_DEFINITIONS[category].category).toBe(category);
    }
  });
  ...
  describe.each(categories)("%s", (category) => {
    ...
    it("binds every spec step and declared witness reference before provider startup", async () => {
      ...
      const expected = [
        ...f.spec.steps.map((step) => step.manifestContractName),
    ...
    describe.each(LINEAR_FAMILY_DEFINITIONS[category].witnessRoles)(
      "witness %s",
      (role) => {
  ...
  it("refuses a definition that is not a frozen product of defineLinearFamily", async () => {
  ...
  it("refuses a definition whose step datum schemas disagree with its spec", async () => {
    ...
    await expect(
      ...
      `${category!} definition declares ${(f.definition.stepDatumSchemas.length - 1).toString()} step datum schemas for a ${f.spec.steps.length.toString()}-step spec`,
    );
    expect(f.observe).not.toHaveBeenCalled();
  });
});
