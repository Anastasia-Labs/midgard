...
afterAll(async () => {
  ...
  const evidence = {
    category: "validationTraceDispute/ScriptSources descriptor",
  ...
  writeFileSync(
    "/tmp/nip-script-sources-descriptor-measurements.json",
    JSON.stringify(
  ...
  await writeVanRossemFitLedger(
    fileURLToPath(
      new URL(
        "../../../docs/fault-proofs/size-plans/validation-trace-script-sources-descriptor-fit-ledger.json",
        import.meta.url,
      ),
    ),
