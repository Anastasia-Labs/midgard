...
afterAll(async () => {
  ...
  const evidence = {
    category: "validationTraceDispute/ScriptSources descriptor maximum",
  ...
  writeFileSync(
    "/tmp/nip-script-sources-descriptor-max-measurements.json",
    JSON.stringify(
  ...
  await writeVanRossemFitLedger(
    fileURLToPath(
      new URL(
        "../../../docs/fault-proofs/size-plans/validation-trace-script-sources-descriptor-max-fit-ledger.json",
        import.meta.url,
      ),
    ),
...
it.each(slots)(
  "proves ScriptSources slot %s %s through permanent proof and removal",
  async (scriptSourcesSemanticIndex, scriptSourcesDescriptorAction) => {
...
it.each(slots)(
  "refuses forged ScriptSources slot %s %s against an honest trace",
  async (scriptSourcesSemanticIndex, scriptSourcesDescriptorAction) => {
    await expect(
...
it.each(slots)(
  "cancels and restarts ScriptSources slot %s %s",
  async (scriptSourcesSemanticIndex, scriptSourcesDescriptorAction) => {
    const build = ({
      operatorVkey,
      now,
