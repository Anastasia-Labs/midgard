...
describe("prepare-invalid-signature", () => {
  it("selects the transaction carrying a signature that does not verify", async () => {
    ...
    expect(output.tx.badAddressWitnessIndex).toBe(2);
    expect(output.tx.badAddressWitnessVerificationKey).toBe(
  ...
  it("derives witness preimages that reproduce the committed hashes", async () => {
    ...
    // This is exactly the chain step 02 walks on-chain: witness-set preimage to
    // witness_set_hash, then address witnesses to addr_tx_wits_hash.
    expect(computeWitnessSetHash(output.tx.witnessSetPreimage)).toBe(
      output.tx.badTxWitsHash,
    );
    expect(output.tx.badTxWitsHash).toBe(output.tx.nativeTx.witness_set_hash);
    expect(output.tx.badAddressWitnessIndex).toBe(0);
  });
  ...
  it("reports the first failing witness when several are corrupt", async () => {
    ...
    expect(output.tx.badAddressWitnessIndex).toBe(1);
  });
  ...
  it("writes the artifacts the two submit steps consume", async () => {
    ...
    await withTempDir(async (dir) => {
      ...
      const witnessSet = JSON.parse(
        await readFile(output.files!.witnessSetPreimagePath, "utf8"),
      );
      expect(witnessSet.addrTxWitsHash).toBe(
        output.tx.witnessSetPreimage.addr_tx_wits_hash,
      );
      ...
      expect(plan.badAddressWitnessIndex).toBe(1);
