...
const measuredBlueprintSha256 = createHash("sha256")
  ...
  .digest("hex");
afterAll(async () => {
  const path = process.env.TRANSITION_TRACE_FIT_LEDGER_PATH;
  if (path)
...
} from "./support/transition-trace-final-fixtures.js";
...
describe("fault-proof emulator integration", () => {
  it.each([
    ...
    async ({
      ...
      let padding = 0;
      while (outputBytes > 0 && makeOutput(padding, true).length < outputBytes)
        padding++;
      ...
      if (fieldBytes > 0) {
        ...
        expect(encodeMidgardFieldPreimage(outputCbors).length).toBe(fieldBytes);
      }
      ...
      if (kind === "deposit") {
        ...
        if (assetCount === 1295)
          expect(
            assetsToValue(additionalReferenceInputs[0]!.assets).to_cbor_bytes()
              .length,
          ).toBe(5000);
        proverSigner.selectWallet(proverLucid);
      ...
      if (honest) {
        await expect(submitProof()).rejects.toThrow(/failed script execution/u);
        await expect(
          proverLucid.utxosAtWithUnit(
            contracts.fraudProof.spendingScriptAddress,
            toUnit(
