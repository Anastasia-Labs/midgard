...
// submit-init-emulator-value-and-mint.test.ts: one emulator journey per file,
// on purpose. The split was made while `@lucid-evolution/uplc` (through 0.2.22)
// leaked wasm linear memory on every script evaluation and vitest isolates per
// FILE; that leak is fixed upstream, and the split is kept so each file runs in
