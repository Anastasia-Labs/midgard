...
const ledgerPath = new URL(
  "../../../docs/fault-proofs/size-plans/invalid-range-wrongful-rejection-v1-fit-ledger.json",
  import.meta.url,
);
describe("invalidRange wrongful-rejection fit ledger", () => {
  it("reproduces its digest and every Van Rossem margin", async () => {
    ...
    expect(parsed.category).toBe("invalidRange");
    expect(parsed.categoryId).toBe("00000003");
    expect(
