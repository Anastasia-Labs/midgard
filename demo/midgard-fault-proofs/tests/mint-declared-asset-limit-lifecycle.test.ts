...
const crossing = (policyByte: number, padding: number) =>
  Buffer.concat([
...
describe("mintDeclaredAssetLimit local-catalogue maximum lifecycle", () => {
  it("runs Init, grammar and fold resumes, permanent mint, and removal", async () => {
    ...
    expect(staged.grammar).toHaveLength(3);
    ...
    expect(carriageCapture.measurements).toHaveLength(3);
    ...
    const submitActuatorAction = async (
      ...
      if (next === undefined)
        throw new Error(
          "mint declared actuator omitted its next thread output",
        );
      return {
        txHash,
    ...
    const grammarActions = [
      ...
      { kind: "grammar_resume", nextOrdinal: 1 } as const,
      { kind: "grammar_resume", nextOrdinal: 2 } as const,
      { kind: "grammar_finish" } as const,
    ];
    for (const [index, action] of grammarActions.entries()) {
      const result = await captureEmulatorSubmission(harness.emulator, () =>
        submitActuatorAction(
          ...
          applied[index === grammarActions.length - 1 ? 2 : 1]
            .spendingScriptAddress,
        ),
      );
    ...
    for (let ordinal = 0; ordinal < staged.walk.length; ordinal += 1) {
      ...
      captures.push([`fold${ordinal.toString()}`, result]);
      threadOutRef = result.result.nextThreadOutRef;
    }
    const final = await captureEmulatorSubmission(
      ...
      async () => {
        ...
        if (proof === undefined)
          throw new Error(
            "mint declared actuator omitted permanent proof token",
          );
        return {
          txHash,
    ...
    if (process.env.MIDGARD_PRINT_FIT === "1") {
      console.info(
        `[mint-declared-asset-limit-ledger] ${JSON.stringify(
      ...
      console.info(
        `[mint-declared-asset-limit-carriage-ledger] ${JSON.stringify(
  ...
  it("runs the exact forced wrongful-rejection arm through permanent mint", async () => {
    ...
    const reason = {
      MintDeclaredAssetLimit: { policy_index: 0n },
    } as const;
      ...
      references.push(
        (
          await publishPlainReferenceScriptUtxo({
            lucid: harness.funderLucid,
            script: step.spendingScript,
            label: `mint-declared-forced-${index.toString()}`,
          })
        ).utxo,
      );
    const captures: Array<
    ...
    const step03 = await measured("forced-complete-fold", () =>
    ...
    if (process.env.MIDGARD_PRINT_FIT === "1")
      console.info(
        `[mint-declared-asset-limit-forced-ledger] ${JSON.stringify(
