...
describe("invalidSignature wrongful-rejection real lifecycle", () => {
  it.each([
    ...
    async (scenario) => {
      ...
      const deepMembership = "deepMembership" in scenario;
      ...
      const references = [
        ...
        harness.faultProofReferenceScripts.fraudProofInvalidSignatureStep02!
          .utxo,
      ] as const;
      ...
      const evidence = {
        ...
        witnessSetHash:
          invalid.compact.transactionWitnessSetHash.toString("hex"),
      ...
      if (accused === "honest")
        ...
        await expect(
          admitInvalidSignatureForcedArtifact(artifact),
      ...
      if (opening.plan.tier === "Certified") {
        ...
        const certified = await captureEmulatorSubmission(
          harness.emulator,
          () =>
            certifyFaultProofFieldCarriage({
              lucid: harness.proverLucid,
