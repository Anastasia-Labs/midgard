...
import { createWitnessScriptDecodingCentralJournalAdapterV1 } from "../src/witness-script-decoding/central-journal-v1.js";
import type {
...
const adapter = (
  ...
  createWitnessScriptDecodingCentralJournalAdapterV1({
    store: value,
...
describe("witnessScriptDecoding central journal adapter", () => {
  ...
  it("reuses an identical crash intent and rejects transaction substitution", async () => {
    const memory = store();
    const bridge = adapter(memory.value);
    await bridge.begin("submitStep02", "family-evidence", "step02", "scan");
  ...
  it("refuses a confirmed hash paired with a substituted authenticated stage", async () => {
    ...
    await expect(bridge.reconcile("scan")).rejects.toThrow(
  ...
  it("reconciles an exact submitted transaction after a directory-backed process restart", async () => {
    const directory = await mkdtemp(
      join(tmpdir(), "midgard-field-item-width-journal-"),
    );
    try {
      const first = adapter(
      ...
      await expect(
        ...
        expect.objectContaining({ stage: "step01", transactionId: txHash }),
      ]);
    } finally {
