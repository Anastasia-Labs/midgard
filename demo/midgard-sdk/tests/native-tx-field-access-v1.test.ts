...
describe("§4/§8.6 values a builder must reproduce", () => {
  ...
  it("records the zero-input residual #569 and #579 close together", () => {
    // The `fraud_proofs/zero_input/step_02` *source* requires the flat §4
    // commitment; the blueprint on disk still carries the counted one, so the
    // emulator-backed suites still see the old script.
    // `EMPTY_SPEND_INPUTS_HASH` is the value the *codec* computes, because the
    // nine per-field producers have not swapped yet (#569) and the blueprint
    // has not been regenerated (#579) — see the constant's own note for why
    // moving it before both would break the fault-proof emulator. Asserting the
    // gap keeps it visible instead of letting a builder quietly emit a datum
