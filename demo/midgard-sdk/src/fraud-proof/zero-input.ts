import { computeHash32, encodeCborArrayRaw } from "@al-ft/midgard-core";
...
export const ZeroInputStep01DatumSchema = faultProofStepDatumSchema(
  Data.Any(),
);
 ...
 * nothing: the hash of an empty definite-length CBOR array. Derived from the
 * same codec that hashes real input preimages, so it cannot drift from the
 * `empty_spend_inputs_hash` constant the step-02 validator compares against.
 */
export const EMPTY_SPEND_INPUTS_HASH: string = computeHash32(
  encodeCborArrayRaw([]),
).toString("hex");
