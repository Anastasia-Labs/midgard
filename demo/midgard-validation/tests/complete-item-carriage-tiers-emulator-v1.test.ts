...
const buildTraceWithOutputs = async (
  ...
  const spentOutput = makeOutput(BigInt(10 * outputs.length));
 ...
 * `complete-item-proof-fit-v1.test.ts` keeps.
 */
 ...
 * `complete-item-proof-fit-emulator-v1.test.ts`, whose harness is tier-1 only.
...
describe("complete-item §8 carriage tiers 2 and 3 (emulator, applied validators)", () => {
  ...
  it("carries one complete item at the applied publication maximum through the tier-2 door", async () => {
    // Moved here from `complete-item-proof-fit-emulator-v1.test.ts` (owner
