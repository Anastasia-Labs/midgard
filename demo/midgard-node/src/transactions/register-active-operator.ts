...
import { withStubbedProviderEvaluation as withSharedStubbedProviderEvaluation } from "@/cml-redeemers.js";
import {
...
const withRegisteredOperatorStubbedProviderEvaluation = async <A>(
  ...
  const resolveStubExUnits = async (
    ...
    const memShare = protocolParameters.maxTxExMem / safeRedeemerCount;
    const stepsShare = protocolParameters.maxTxExSteps / safeRedeemerCount;
    const mem = Number(
  ...
  return withSharedStubbedProviderEvaluation(lucid, run, resolveStubExUnits);
};
  ...
  Effect.gen(function* () {
    ...
    let activationUnsignedTx = yield* Effect.tryPromise({
      try: () =>
        completeWithLocalEvaluation((options) =>
          mkActivateTx(activateLayout).complete(
            activationCompleteOptions(options),
          ),
        ),
      catch: (cause) =>
        new SDK.LucidError({
