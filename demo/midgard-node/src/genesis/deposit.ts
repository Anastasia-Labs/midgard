 ...
 */
export const buildAndSubmitGenesisDeposit = (): Effect.Effect<
  void,
  SDK.Utils.LucidError | GenesisDepositError,
  AlwaysSucceedsContract | NodeConfig | Lucid
> =>
  Effect.gen(function* () {
    ...
    if (config.GENESIS_UTXOS.length <= 0) {
      yield* Effect.fail(
        new GenesisDepositError({
          message:
            "No GENESIS_UTXOS provided, abort buildAndSubmitGenesisDeposit",
          cause: "",
        }),
      );
      return;
