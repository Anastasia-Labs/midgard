...
This document defines the phase-1 validation rules that `midgard-node` should enforce before a transaction is accepted into Midgard state and eligible for inclusion in a committed block.
...
Only `accepted` transactions are allowed to affect local ledger state and root computation.
...
Rule:
Every reference input must exist in current UTxO state at validation time and must not be spent by the same transaction.
...
Rule:
Value conservation must hold under Midgard's phase-1 accounting constraints.
...
Rule:
Required VKey signatures must verify against the transaction body hash.
...
## Midgard-Specific Custom Rules

## R16. `is_valid` Must Be `true`
...
## R19. Withdrawals Must Be Zero/Absent
...
## R20. No Minting in L2 User Transaction Path
...
The validation pipeline is intended to run in two stages:

1. Stateless checks (parallel-safe).
2. Stateful checks (deterministic ordering, conflict-aware).

Deterministic ordering is required so validation outcomes are reproducible and consistent across runs.

## Explicit Non-Goals for This Phase

- Plutus phase-2 script execution.
- Validation rules for unsupported governance/staking features beyond rejecting their use.
- Any reward-balance withdrawal semantics (Midgard treats withdrawals as zero/absent in this scope).
