...
import { LucidEvolution, TxHash } from "@lucid-evolution/lucid";
...
// Probe L1 directly for the first output of the submitted tx. If the
// indexer-derived state_queue scan has not yet surfaced our pending block
// (indexer lag), the tx may still already be on chain - and `utxosByOutRef`
// is the authoritative witness of that. Treat any non-empty return as
// "tx is confirmed on chain, just not yet visible to state_queue scan".
const probeSubmittedTxOnChain = (
  ...
  return Effect.tryPromise({
    try: () =>
      lucid.utxosByOutRef([{ txHash: submittedTxHash, outputIndex: 0 }]),
    catch: () => null,
  }).pipe(
    Effect.match({
      onFailure: () => false,
      onSuccess: (utxos) => utxos.length > 0,
    }),
  );
};
...
const awaitPendingBlockResolution = (
  ...
  Effect.gen(function* () {
    ...
    while (Date.now() - startedAt < timeoutMs) {
      ...
      // Before declaring the submission stale (either because the canonical
      // tip moved past our block-end time, or wall-clock did), confirm the
      // submitted tx really isn't on chain. The state_queue scan above is
      // indexer-derived and can briefly trail the L1 tip; if our tx was
      // included in the last few seconds we may not see it yet. A direct
      // out-ref probe is the authoritative check - if the submission's
      // first output exists, the tx IS on chain and we keep polling so
      // that the next iteration's hash match succeeds.
      const wouldDeclareStale =
        latestEndTimeMs >= pendingBlock.blockEndTimeMs ||
        Date.now() > pendingBlock.blockEndTimeMs;
      if (wouldDeclareStale) {
        const submittedOnChain = yield* probeSubmittedTxOnChain(
          lucid,
        ...
        yield* Effect.logInfo(
          `🔍 Submitted tx ${pendingBlock.submittedTxHash} is on chain; awaiting state_queue indexer to surface header ${pendingBlock.expectedHeaderHash}.`,
        );
      }
