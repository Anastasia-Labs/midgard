...
} from "@/commands/phase4-genesis-ledger.js";
import {
...
} from "@/commands/phase4-t1-recovery.js";
import { loadDotenvFile } from "@/e2e/env.js";
import type { PipelinedCommitCrashCheckpoint } from "@/e2e/pipelined-commit-crash-checkpoint.js";
import {
...
const resetAndPreflight = async ({
  ...
  const genesisOutput = await runRequiredProcess(
    {
      command: process.execPath,
      args: [dist, "phase4-genesis-ledger", "--verify-only"],
      cwd,
      env: {
        ...env,
...
const runT1RecoveryCase = async ({
  ...
  const [recoveryPath, repositoryRecoveryPath] = await Promise.all([
    ...
    realpath(resolve(cwd, "devnet/phase4-process/scripts/t1-recover.sh")),
  ]);
