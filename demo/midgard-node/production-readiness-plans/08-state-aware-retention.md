# P0 Blocker 8: State-Aware Retention For Bridge Events And Replay Evidence

## Problem Statement

The current retention sweeper still performs broad age-driven pruning when
`RETENTION_DAYS > 0`. It now covers `tx_rejections`, `address_history`,
`deposits_utxos`, and `withdrawal_utxos`. That is not safe for a production L2
because deposit and withdrawal rows are bridge evidence, not ordinary cache
rows. A bridge event row may be old while still being live, projectable,
assigned to an unconfirmed block, needed for local-finalization recovery,
needed for merge replay, or needed to prove the correctness of a prior bridge
inclusion.

The fix is to make retention state-aware and audit-aware. Bridge-event
retention must only physically prune records that are terminal and already
exported into a durable audit archive with checksums. Live records, replay
records, records referenced by pending/finalized-but-unmerged block journals,
and records whose archive cannot be verified must remain in PostgreSQL. Age can
be a secondary condition; it must never be the primary proof of prune safety.

This document is the active P0 blocker 8 plan in the current tree. The original
production-readiness report snapshot referenced by older versions of this file
is no longer present in `demo/midgard-node`.

## Current Behavior

No state-aware archive/prune policy has landed yet. The current implementation
has only coarse retention enablement plus table-specific `pruneOlderThan`
helpers. A repository search for `retention_state`, `deposit_retention`,
`retention_sweep`, and `pruneArchivedTerminal` under `src/` and `tests/` finds
no implemented retention lifecycle, archive tables, sweep audit rows, or
tombstone lookups.

Current retention code:

- `RETENTION_DAYS` is read from config with default `0`:
  [`src/services/config.ts:228`](../src/services/config.ts#L228), then exposed
  on `NodeConfig`:
  [`src/services/config.ts:394`](../src/services/config.ts#L394). The config
  shape still has a single broad `RETENTION_DAYS` field:
  [`src/services/config.ts:59`](../src/services/config.ts#L59).
- `.env.example` sets `RETENTION_DAYS=0`, so pruning is disabled unless
  operators opt in:
  [`.env.example:66`](../.env.example#L66).
...
- The current unit tests assert exactly that `RETENTION_DAYS <= 0` disables
  pruning, `RETENTION_DAYS > 0` enables pruning, and the cutoff subtracts whole
  days:
  [`tests/retention-policy.test.ts:5`](../tests/retention-policy.test.ts#L5).
- The node starts `retentionSweeperFiber` with
  `WAIT_BETWEEN_RETENTION_SWEEPS` alongside block, deposit, merge, and tx
  workers:
  [`src/commands/listen.ts:191`](../src/commands/listen.ts#L191).
- The sweeper imports `WithdrawalsDB` in addition to transaction rejections,
  address history, and deposits:
  [`src/fibers/retention-sweeper.ts:1`](../src/fibers/retention-sweeper.ts#L1).
- When pruning is enabled, it computes one cutoff from `RETENTION_DAYS` and
  calls `TxRejectionsDB.pruneOlderThan`, `AddressHistoryDB.pruneOlderThan`,
  `DepositsDB.pruneOlderThan`, and `WithdrawalsDB.pruneOlderThan` in one
  unbounded `Effect.all`:
  [`src/fibers/retention-sweeper.ts:27`](../src/fibers/retention-sweeper.ts#L27),
  [`src/fibers/retention-sweeper.ts:38`](../src/fibers/retention-sweeper.ts#L38).
- The sweeper log reports all four deleted counts:
  [`src/fibers/retention-sweeper.ts:48`](../src/fibers/retention-sweeper.ts#L48).
...
- Deposit statuses are exactly `awaiting`, `projected`, and `consumed`:
...
- The table accepts only those three statuses and requires `awaiting` rows to
  ...
  [`src/database/deposits.ts:71`](../src/database/deposits.ts#L71).
- Deposit ingestion inserts rows from visible L1 deposit UTxOs with status
  `awaiting` and full event/ledger payload:
  [`src/fibers/fetch-and-insert-deposit-utxos.ts:191`](../src/fibers/fetch-and-insert-deposit-utxos.ts#L191).
...
- Awaiting deposits are selected by inclusion time for projection:
  [`src/database/deposits.ts:270`](../src/database/deposits.ts#L270).
- `projectDepositsToMempoolLedger` reconciles already projected deposits and
  projects due awaiting deposits:
  [`src/fibers/project-deposits-to-mempool-ledger.ts:28`](../src/fibers/project-deposits-to-mempool-ledger.ts#L28),
  [`src/fibers/project-deposits-to-mempool-ledger.ts:79`](../src/fibers/project-deposits-to-mempool-ledger.ts#L79).
- Projection inserts/reconciles `mempool_ledger` rows and then marks deposits
  `projected`:
  [`src/fibers/project-deposits-to-mempool-ledger.ts:93`](../src/fibers/project-deposits-to-mempool-ledger.ts#L93).
...
- A mempool transaction that spends deposit-origin UTxOs clears those UTxOs and
  ...
  [`src/database/mempool.ts:91`](../src/database/mempool.ts#L91).
- Block construction resolves deposits pending header assignment and may
  ...
  [`src/database/deposits.ts:287`](../src/database/deposits.ts#L287).
- Block construction refuses to skip an overdue awaiting deposit:
  [`src/database/deposits.ts:306`](../src/database/deposits.ts#L306).
- Pending block finalization stores included deposit event ids in
  ...
  [`src/database/pendingBlockFinalizations.ts:329`](../src/database/pendingBlockFinalizations.ts#L329).
...
- Local block finalization assigns the header hash to included deposits:
  [`src/workers/utils/commit-submission.ts:298`](../src/workers/utils/commit-submission.ts#L298).
- Block confirmation also assigns/retains the header hash when a pending block
  is observed on L1:
  [`src/fibers/block-confirmation.ts:107`](../src/fibers/block-confirmation.ts#L107).
- Abandoned pending blocks clear the matching deposit header assignment:
  [`src/fibers/block-confirmation.ts:138`](../src/fibers/block-confirmation.ts#L138).
- Merge local finalization updates `confirmed_ledger` and clears the merged
  ...
  [`src/transactions/state-queue/merge-to-confirmed-state.ts:1440`](../src/transactions/state-queue/merge-to-confirmed-state.ts#L1440).
- Deposit-only commits can create pending-finalization journals with deposit
  members and no `BlocksDB` transaction members:
  [`src/workers/commit-block-header.ts:623`](../src/workers/commit-block-header.ts#L623),
  [`src/workers/commit-block-header.ts:708`](../src/workers/commit-block-header.ts#L708).
  ...
  `BlocksDB` rows.
- Current deposit pruning deletes by `inclusion_time < cutoff` only for rows
  already marked `consumed`, with a non-null `projected_header_hash`, and with
  no pending-finalization deposit member:
  [`src/database/deposits.ts:653`](../src/database/deposits.ts#L653). This is a
  shallow operational guard, not a terminal/archive retention policy. It does
  not prove the containing block was merged, the event was archived, or a
  queryable tombstone was retained.

Relevant withdrawal table behavior:

- `withdrawal_utxos` has the current bridge-withdrawal event payload, L1 order
  identity, L2 outref/value/owner fields, settlement event info, validity
  classification, `projected_header_hash`, and `status`:
  [`src/database/withdrawals.ts:112`](../src/database/withdrawals.ts#L112).
- Withdrawal statuses are exactly `awaiting`, `projected`, and `finalized`:
  [`src/database/withdrawals.ts:36`](../src/database/withdrawals.ts#L36).
- The table check constraint accepts only those three statuses and requires
  non-awaiting rows to have settlement info and validity classification:
  [`src/database/withdrawals.ts:134`](../src/database/withdrawals.ts#L134).
- Withdrawal ingestion reconciles visible withdrawal order UTxOs into
  `withdrawal_utxos`:
  [`src/fibers/fetch-and-insert-withdrawal-utxos.ts:136`](../src/fibers/fetch-and-insert-withdrawal-utxos.ts#L136).
- Pending block finalization stores included withdrawal event ids in
  `pending_block_finalization_withdrawals`, with a foreign key back to
  `withdrawal_utxos` and `ON DELETE RESTRICT`:
  [`src/database/pendingBlockFinalizations.ts:349`](../src/database/pendingBlockFinalizations.ts#L349).
- Block confirmation and local-finalization recovery mark withdrawals
  `finalized` once the containing block is finalized locally:
  [`src/fibers/block-confirmation.ts:118`](../src/fibers/block-confirmation.ts#L118),
  [`src/workers/utils/commit-submission.ts:417`](../src/workers/utils/commit-submission.ts#L417).
- Current withdrawal pruning deletes by `inclusion_time < cutoff` only for rows
  already marked `finalized`, with a non-null `projected_header_hash`, and with
  no pending-finalization withdrawal member:
  [`src/database/withdrawals.ts:835`](../src/database/withdrawals.ts#L835). Like
  deposit pruning, this does not prove archive durability, auditability, or
...
1. A bridge-event row is never physically deleted because of age alone.
2. Deposit rows in `awaiting`, `projected`, `consumed`, header-assigned,
   pending-finalization, submitted, confirmed-but-unmerged,
   abandoned-but-replayable, and archive-unverified states are not pruneable.
3. Withdrawal rows in `awaiting`, `projected`, `finalized`,
   header-assigned, pending-finalization, submitted, confirmed-but-unmerged,
   abandoned-but-replayable, and archive-unverified states are not pruneable.
   `finalized` is an operational withdrawal status, not retention proof.
4. A bridge event is pruneable only after a deterministic state transition
   marks it terminal for retention and links it to a verified durable archive
   export.
5. The archive must contain enough data to prove and replay the bridge-event
   lifecycle: L1 event identity, event payload, inclusion time, L1 transaction
   hash, derived L2 ledger or settlement output, address/owner/value data,
   header assignment, block/merge evidence, final lifecycle state, and hashes
   of any related tx/block records.
6. Pruning must be idempotent. Re-running a sweep must not change retained state
   except for deleting rows already marked pruneable.
7. Pruning must be auditable. Every prune batch has a durable audit row with
   ...
   fails.
8. Pruning must be transactional for PostgreSQL state. The archive verification
...
9. If archive verification, SQL deletion, audit logging, or invariant checking
   fails, the sweeper fails closed for that table and deletes nothing from
   `deposits_utxos` or `withdrawal_utxos`.
10. Foreign-key `ON DELETE RESTRICT` constraints remain defense in depth, not the
   ...
   pruning impossible.
11. Retention does not create compatibility modes or legacy ID fallbacks in
    `demo/midgard-node`. Schema changes are explicit migrations.
12. Rejection and address-history pruning must not remove the last copy of
...
13. Readiness/observability must expose blocked retention, unarchived terminal
    backlog, and failed prune attempts so operators can distinguish safe
    retention delay from a broken sweeper.
14. Pruning a hot deposit or withdrawal row must leave a durable, queryable
    identity/tombstone keyed by `event_id`. Existing and future status/audit
    lookups must be able to distinguish `archived_pruned` from `not_found`.

## Bridge Event Lifecycle States

### Existing Deposit States
...
### Existing Withdrawal States

The existing `withdrawal_utxos.status` is also too coarse for retention:

- `awaiting`: L1 withdrawal order was ingested and has not been classified and
  projected for settlement.
- `projected`: withdrawal settlement event info exists and the withdrawal can
  be included in a block.
- `finalized`: the containing block has been finalized locally for the
  withdrawal lifecycle.

These states describe withdrawal processing, not archival safety. In
particular, `finalized` is not terminal for retention. A finalized withdrawal
may still be part of a pending journal, an unmerged block, a recovery path, or
an audit investigation until merge-boundary evidence and archive verification
exist.

### Target Lifecycle Model

Implement a separate retention lifecycle rather than overloading the existing
projection/finalization status. This keeps the current operational state
machines readable and lets retention answer a narrower question: "May this
evidence leave the hot database?"

Recommended bridge-event retention states:

- `live_unprojected`: existing deposit or withdrawal row is `awaiting`; never
  prune.
- `live_projected`: existing row is `projected` with no terminal block/merge
  evidence; never prune.
- `live_spent_or_finalized`: existing deposit row is `consumed`, or existing
  withdrawal row is `finalized`, but it is not yet proven merged and archived;
  never prune.
...
- `block_abandoned_replayable`: header assignment was cleared or abandoned and
  the bridge event must remain available for re-inclusion; never prune.
...
- `pruned`: logical marker recorded in audit tables after the row has been
  physically deleted from `deposits_utxos` or `withdrawal_utxos`.
...
rows. Clearing `BlocksDB` during merge is not sufficient proof that bridge
event members are terminal, especially for deposit-only or withdrawal-only
blocks. Add or reuse an explicit block-retention evidence record keyed by
header hash that records:

- header hash and block end time;
- deposits root and ordered deposit event ids;
- withdrawals root and ordered withdrawal event ids;
...
bridge-event pruning.

### Archive Contents

Each archived bridge-event record must include:

- `event_id`, event kind, and raw event payload.
- `inclusion_time`.
- For deposits: `deposit_l1_tx_hash`, `ledger_tx_id`, `ledger_output`, and
  `ledger_address`.
- For withdrawals: `withdrawal_l1_tx_hash`, `withdrawal_l1_output_index`,
  settlement event info, validity classification, L2 owner/outref/value, L1
  payout/refund address data, and payout datum data.
...
- Block membership evidence: header hash, event ordinal, deposits or
  withdrawals root, block end time, and confirmation/merge boundary where
  available.
- L1 observation evidence from bridge-event ingestion, including fetch
  barrier/cursor data once the startup fail-closed work makes it durable.
...
- Tombstone metadata after pruning: prune sweep id, prune timestamp, prune
  ...
  after the hot bridge-event row is gone.
...
Add explicit migrations for bridge-event retention. The schema below shows the
deposit side explicitly; withdrawals need an analogous `withdrawal_retention_*`
schema, or the implementation may choose a generic `bridge_event_retention_*`
schema keyed by `(event_kind, event_id)` if that is cleaner and preserves the
same invariants.
...
`ON DELETE RESTRICT` FK to `deposits_utxos` or `withdrawal_utxos`, because that
makes `archived_terminal` pruning impossible. Active live references, such as
`mempool_ledger.source_event_id`, should continue to block deletion directly.

## Prune Query Changes

Replace `DepositsDB.pruneOlderThan(cutoff)` and
`WithdrawalsDB.pruneOlderThan(cutoff)` with retention-class-specific
operations, for example `DepositsDB.pruneArchivedTerminal(cutoff, sweepId)` and
`WithdrawalsDB.pruneArchivedTerminal(cutoff, sweepId)`.

The deposit delete must be shaped like this:
...
- Keep `ON DELETE RESTRICT` foreign keys in `mempool_ledger` and
  ...
  audit-only membership FKs pointed at `deposits_utxos` or `withdrawal_utxos`.
- Rename log fields from `deposits_utxos=<count>` to
  `deposits_archived_terminal_pruned=<count>` and from
  `withdrawal_utxos=<count>` to
  `withdrawals_archived_terminal_pruned=<count>` so logs describe what was
  actually eligible.

The sweeper must sequence bridge-event archive verification before bridge-event
pruning. It may continue pruning `tx_rejections` or `address_history` only
after their own safety rules are separated from bridge evidence. Bridge-event
deletion must not run in `Effect.all(..., { concurrency: "unbounded" })` with
unrelated pruning if that makes audit ordering unclear.
...
This P0 is bridge-event-driven, but the current sweeper prunes four tables
together. The implementation must avoid turning non-bridge pruning into hidden
evidence loss.
...
- `WITHDRAWAL_RETENTION_ENABLED=false` by default until archive verification,
  lifecycle migration, tests, and rollout checks pass.
...
- `WITHDRAWAL_ARCHIVE_REQUIRED=true` always in production. Do not provide a
  production mode that prunes without archive verification.
- `DEPOSIT_ARCHIVED_TERMINAL_RETENTION_DAYS=<positive integer>` controls only
  `archived_terminal.archived_at`, not `inclusion_time`.
- `WITHDRAWAL_ARCHIVED_TERMINAL_RETENTION_DAYS=<positive integer>` controls
  only `archived_terminal.archived_at`, not `inclusion_time`.
...
- `TX_REJECTION_RETENTION_DAYS` and `ADDRESS_HISTORY_RETENTION_DAYS` are
  separate from bridge-event retention and require their own archive/source
  guarantees.
- Invalid combinations fail startup, for example deposit or withdrawal pruning
  enabled while archive required is false, archive storage URI is unset,
  retention days is zero/negative, or migrations are not at the required schema
  version.
...
with clear remediation. Do not silently map it to deposit or withdrawal
terminal retention.
...
1. Add explicit schema migrations.
   ...
   tables; add deposit and withdrawal retention metadata; add indexes for
...
2. Add a retention state classifier.
   ...
   retention state from `deposits_utxos.status`, `withdrawal_utxos.status`,
   `projected_header_hash`, `mempool_ledger.source_event_id` where applicable,
   active/finalized pending-finalization rows, archived block membership, and
   merge boundary evidence. Unknown rows are live/non-pruneable.

3. Backfill retention state in the migration.
   Backfill bridge-event identity tables first, then retention state and
...
4. Wire lifecycle transitions into existing bridge-event paths.
   Update deposit and withdrawal ingestion to create `live_unprojected`,
   projection/classification to move to `live_projected`, deposit spending and
   withdrawal finalization to move to `live_spent_or_finalized`, pending-block
   creation to move included events to `block_pending`, abandonment to move
...
6. Replace bridge-event prune queries.
   Remove age-driven `DepositsDB.pruneOlderThan` and
   `WithdrawalsDB.pruneOlderThan` from the sweeper path. Add
   `pruneArchivedTerminal` operations with live-reference joins, archive
   verification, row locks, bounded batches, tombstone insertion, and sweep
   audit rows.

7. Split sweeper table classes.
   Make the sweeper call separate table-specific retention policies with
   separate config and audit entries. Do not run bridge-event pruning as a generic
...
10. Update bridge-event status/audit lookups.
    Make deposit and withdrawal status resolution check hot rows first, then
    tombstone plus archive membership. A pruned terminal bridge event must
    return an explicit archived/pruned status with archive id, prune sweep id,
    and enough identity data to audit the event; it must not be reported as
    `404 not_found`.
...
- Classifier maps consumed rows without merge/archive proof to
  `live_spent_or_finalized`.
- Classifier maps finalized withdrawal rows without merge/archive proof to
  `live_spent_or_finalized`.
...
- `archived_terminal` without archive member row is rejected by constraints or
  prune query.
- A bridge event cannot be marked pruneable unless its identity table
...
- Age-old `awaiting`, `projected`, and `finalized` withdrawals are not deleted
  without verified archive and terminal retention state.
...
- Age-old withdrawals referenced by active
  `pending_block_finalization_withdrawals` journals are not deleted.
...
- Pruning inserts bridge-event tombstones in the same transaction as the
  hot-row delete.
...
- A direct audit-only FK from finalized pending membership to `deposits_utxos`
  or `withdrawal_utxos` is rejected by migration/startup verification.

End-to-end tests:

- Deposit and withdrawal ingestion, projection/classification, block inclusion,
  confirmation, merge, archive, and prune leave enough archive data to answer
  status/audit queries.
- Deposit-only and withdrawal-only block inclusion follow the same archive and
  prune path even when there are no `BlocksDB` transaction rows for the header.
...
2. Backfill bridge-event retention metadata in dry-run/report mode on
   representative data.
...
7. Enable `DEPOSIT_RETENTION_ENABLED=true` and
   `WITHDRAWAL_RETENTION_ENABLED=true` with a small batch size in a non-prod
   environment.
8. Run deposit and withdrawal flow emulator tests through ingestion, commit,
   confirmation, merge, archive, prune, and audit lookup.
...
- Merge boundary definition: the implementation must define the exact point at
  which a bridge event no longer needs hot replay evidence. The likely boundary
  is after the containing block has been merged into confirmed state and local
  ...
  3.
- Status API after pruning: decide whether `GET /deposit-status` and
  `withdrawal-status` should query the archive/audit index for pruned terminal
  events or return a specific archived/pruned response. This plan requires the
  latter behavior to be explicit and non-ambiguous; implementation should decide
  the exact JSON shape while preserving current selector validation.
...
- [ ] Add explicit migrations for deposit and withdrawal retention metadata,
      archive tables, identity/tombstone tables, audit tables, indexes, and
      constraints.
- [ ] Add deterministic bridge-event retention classifier and migration
      backfill.
- [ ] Add lifecycle transition updates to deposit and withdrawal ingestion,
      projection/classification, mempool spending, pending block preparation,
      abandonment, confirmation, merge boundary, archive, and prune paths.
...
- [ ] Replace `DepositsDB.pruneOlderThan` and
      `WithdrawalsDB.pruneOlderThan` usage with
...
- [ ] Update deposit/withdrawal status and audit lookups to report
      archived/pruned bridge events from tombstone and archive indexes instead
...
- [ ] Add end-to-end deposit and withdrawal flow coverage through archive and
      prune.
