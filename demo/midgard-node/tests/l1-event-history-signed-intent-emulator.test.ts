...
it("retains the exact durable signed commitment after an accepted submission loses its response, then confirms and merges it", async () => {
  ...
  try {
    ...
    fixture.emulator.submitTx = observingSubmit;
    try {
      const path = process.env.MIDGARD_SIGNED_INTENT_EVIDENCE_PATH;
      if (path !== undefined) {
        ...
        writeFileSync(
          ...
          JSON.stringify(
            {
              scope:
                "Actual emulator acceptance followed by one lost provider response; independent durable pre-provider SQL read and production canonical confirmation/native finalization/mature merge. Synthetic transport ancestry. No restart or state-queue continuation coverage.",
