...
const measureBlockSize = async (
  ...
  const nowMillis = 0n;
  const baselinePhaseB = runPhaseBValidation(
    baselinePhaseA.accepted,
    preState,
    { nowMillis },
  );
  assertOrThrow(
  ...
  for (let i = 0; i < warmupRuns; i++) {
    ...
    const warmupPhaseB = runPhaseBValidation(warmupPhaseA.accepted, preState, {
      nowMillis,
    });
    assertOrThrow(
  ...
  for (let i = 0; i < measuredRuns; i++) {
    ...
    const phaseBResult = runPhaseBValidation(phaseAResult.accepted, preState, {
      nowMillis,
    });
  ...
  return {
    ...
    throughputTps: {
      ...
      mean: throughputTps.reduce((sum, value) => sum + value, 0) /
...
describe("validation benchmark", () => {
  ...
  it(
    "measures phase-1 block validation throughput on generated fixtures",
    async () => {
      ...
      for (const size of BENCHMARK_SIZES) {
        ...
        results.push(await measureBlockSize(queuedBlock, preState, size));
      }
