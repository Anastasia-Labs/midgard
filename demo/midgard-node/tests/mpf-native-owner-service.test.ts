...
// The plain `test` run builds this in tests/global-setup.ts; the binary can
// still be absent when cargo is unavailable (or MIDGARD_SKIP_NATIVE_BUILD=1).
// Skip LOUDLY rather than fail on a missing optional toolchain — and never
// pass silently (#642).
const nativeOwnerBinaryPresent = existsSync(binaryPath);
if (!nativeOwnerBinaryPresent) {
  console.warn(
    `[mpf-native-owner-service] SKIPPING: native binary absent at ${binaryPath} — build it with \`pnpm run native:mpf-owner:build\` (requires cargo)`,
  );
}
...
describe.skipIf(!nativeOwnerBinaryPresent)(
  "production native MPF owner service",
  () => {
    ...
    it("supervises a killed native child and rejects handles from its stale epoch", async () => {
      const root = await mkdtemp(
        join(tmpdir(), "midgard-native-owner-restart-"),
      );
      temporaryPaths.push(root);
      const levelPath = join(root, "ledger");
