...
describe("DA attestation configuration", () => {
  ...
  it("fails closed when no second key is configured", async () => {
    const failure = await daParamsFailure(BASE);
    expect(failure.message).toBe("Invalid DA committee configuration");
    expect(failure.cause).toMatch(/DA_COSIGNER_SEED_PHRASE/);
  });

  it("fails closed when the configured committee has a single member", async () => {
    ...
    expect(failure.message).toBe("Invalid DA committee configuration");
    expect(failure.cause).toMatch(/DA_COMMITTEE_HEX must list at least 2/);
  });

  it("fails closed when the configured owner set has a single member", async () => {
    const failure = await daParamsFailure({
      ...BASE,
    ...
    expect(failure.cause).toMatch(/DA_OWNERS_HEX must list at least 2/);
  });
