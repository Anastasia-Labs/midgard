...
jq -n \
  ...
  --arg toolsSourceSha256 "$tools_source_sha" --arg toolsDistSha256 "$tools_dist_sha" \
...
jq -e \
  'def exact($required): type == "object" and keys == ($required | sort);
   ...
   ([.artifacts[] | strings | test("^[a-f0-9]{64}$")] | length == 10 and all) and
