...
jq -e \
  ...
  'def exact($required): type == "object" and keys == ($required | sort);
   ...
   (.artifacts | exact(["sourceSha256","distSha256","toolsSourceSha256","toolsDistSha256","genesisSha256","configSha256","acceptanceEnvSha256","composeSha256","phase4AssetsSha256","phasRegistrationProofSha256"])) and
...
[ -f "$node_root/dist/index.js" ] || die "built Midgard node CLI is missing"
# The probe and canonical-advance commands are tooling, so they run from the
# snapshot-bound tooling distribution rather than the operator binary.
dist="$tools_root/dist/index.js"
[ -f "$dist" ] || die "built Midgard node tools CLI is missing"
current_source_sha=$(tree_sha256 "$node_root/src")
current_dist_sha=$(tree_sha256 "$node_root/dist")
current_tools_source_sha=$(tree_sha256 "$tools_root/src")
current_tools_dist_sha=$(tree_sha256 "$tools_root/dist")
...
jq -e \
  ...
  --arg toolsSourceSha256 "$current_tools_source_sha" --arg toolsDistSha256 "$current_tools_dist_sha" \
  ...
  '.artifacts.sourceSha256 == $sourceSha256 and .artifacts.distSha256 == $distSha256 and
   .artifacts.toolsSourceSha256 == $toolsSourceSha256 and .artifacts.toolsDistSha256 == $toolsDistSha256 and
  ...
  || die "source/dist/tools source/tools dist/genesis/config/acceptance/Phase4 assets do not match the frozen snapshot identity"
