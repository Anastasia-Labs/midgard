...
/** Parse Cardano transaction CBOR bytes and convert to a Midgard Transaction. */
export function cardanoTxBytesToMidgardTx(txBytes: Uint8Array): Transaction {
  let cmlTx: CML.Transaction;
  try {
    cmlTx = CML.Transaction.from_cbor_bytes(txBytes);
  } catch (e) {
    throw new ConversionError(`Invalid Cardano transaction bytes: ${String(e)}`);
  }
  return cmlToMidgard(cmlTx);
}

/** Parse Cardano transaction CBOR bytes and encode as Midgard binary bytes. */
export function cardanoTxBytesToMidgardTxBytes(txBytes: Uint8Array): Uint8Array {
  return encodeTransaction(cardanoTxBytesToMidgardTx(txBytes));
}
